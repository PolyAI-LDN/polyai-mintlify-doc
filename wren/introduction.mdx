...
Wren is PolyAI's natural-language assistant for Agent Studio. Describe what you want, and it reads your project, proposes a plan, and applies changes when you approve. Every edit lands on a Branch, not on Live.
...
Wren can also look back over your project's own history (past reviews, earlier chats, and published Live versions), so questions like *"what changed last week?"* have real answers behind them.
...
The first time you ask for a change, Wren creates a Branch automatically and names it after the request, prefixed `wren/`. Live stays untouched. Wren always cuts its Branch from Live, so it finishes one piece of work before starting the next. In Simple mode Wren works in your single Branch instead, so no new Branch is created.

**Wren never merges or publishes.** When you're ready, you [publish](/environments-and-versions/introduction#publishing) the Branch to Live yourself with the **Publish** button in the top-right corner.

### What it won't do

- **Edit anything other than your Branch.** It never publishes to Live or moves a Branch to [Staging](/environments-and-versions/environments#staging), and it can't change Live directly.
...
New to Wren? The walkthrough runs one build end to end, from first prompt to published Branch, with screenshots at every step.

<Card title="A full session, step by step" icon="route" href="/wren/walkthrough">
  Describe a change, answer the questions, approve the plan, review what changed, publish.
</Card>
...
Every change Wren makes appears on your Branch just like a manual edit. Inspect it, tweak it, or revert it before publishing.
