...
On self-serve accounts, the **Agent wizard** entry under the Build suggestions is a separate guided builder that bootstraps a brand-new agent from a single company URL. Use it when you're starting from scratch; for incremental additions to an existing agent, prompt directly.
