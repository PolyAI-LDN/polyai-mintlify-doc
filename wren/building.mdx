...
Wren works on a copy of your project, then commits in one step:

1. **It reads your project** into a working copy at the start of the session, and re-syncs whenever you switch branches.
2. **It drafts the change** across however many files the change touches.
3. **It applies the change** to your branch in a single commit once you approve, and shows you a summary card listing every resource that changed, each with a link to open it.

If applying fails, Wren retries once and then tells you plainly rather than reporting success. Nothing reaches `Main` — see [Branches](/wren/introduction#branches).
...
Wren can list, create, and edit [custom metrics](/analytics/custom-metrics/introduction) from chat — useful when you're building a flow and want the metric that measures it registered at the same time.
