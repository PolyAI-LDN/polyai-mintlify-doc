...
They are attached during configuration in the Knowledge area fields later, and are at runtime, when the agent decides to escalate the call, where they are are logged alongside the handoff action.</Note>
