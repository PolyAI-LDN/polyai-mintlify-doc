...
- **Auto-tracked language changes** – multilingual projects now automatically register a `CONVERSATION_LANGUAGE_CHANGED` metric. Every time the conversation language switches, the new locale is recorded so you can filter, dashboard, and ask Smart Analyst about mid-call language changes without declaring the metric yourself.
