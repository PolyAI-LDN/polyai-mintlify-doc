...
Agent Builder can now author and manage [simulation tests](/analytics/test-suite/introduction) for your agent in the same chat where you build it. Ask it to cover a scenario and it creates the test cases, runs them, and updates them as your agent changes — no hand-writing every case to keep coverage in step with the build.
...
See [Test suite](/analytics/test-suite/introduction) for the workspace and [Prompting Agent Builder](/agent-builder/prompting#test) for prompt patterns.
...
- **Works with existing chat features** — pairs with the [Chat channel](/webchat/introduction) configuration and [multichannel agents](/webchat/multichannel) for projects that serve both.
...
<Accordion title="Stronger abuse protection for self-serve accounts" icon="shield" id="plg-abuse-protection">

For self-serve (PLG) accounts, Agent Builder now automatically blocks accounts that repeatedly trigger PolyAI's content-safety filters. This protects the platform and legitimate users from misuse, with no impact on enterprise projects or accounts using Agent Builder within normal limits.
