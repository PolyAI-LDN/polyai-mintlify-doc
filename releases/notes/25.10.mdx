...
<Accordion title="Email from Functions" icon="envelope" id="email-from-functions">

You can now send emails directly from functions using `conv.send_email()`, enabling templated notifications, confirmations, or alerts as part of your conversation logic.

**What's new:**
- Use `conv.send_email()` inside functions to send emails during a conversation.
- Supports **HTML content**, **CC/BCC recipients**, and **file attachments**.
- Useful for sending booking confirmations, follow-up summaries, or internal alerts triggered by conversation events.
