...
<Update label="April 2026">
  ...
  - **[Multi-language support](./notes/26.04#multilingual)**: Configure a single agent to handle multiple languages from the UI — add languages, assign per-language voices with automatic switching, use conditional content tags, and test in any language from Agent Chat.
  - **[Translations](./notes/26.04#translations)**: A new Translations page under Response Control lets you manage auto-translated content, manually override specific phrases, and insert translation cards anywhere in your agent's configuration.
  - **[MCP integrations](./notes/26.04#mcp-integrations)**: Connect external tool servers to your agent from **Configure > Integrations > MCP**. Agent Studio auto-discovers available tools and lets you toggle them on or off — no custom code required.
</Update>
