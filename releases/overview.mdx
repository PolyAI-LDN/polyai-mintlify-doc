...
<Update label="Coming soon">
  **New Agent Studio layout.** We're rolling out a cleaner Studio layout that goes from 29 pages down to 16 by grouping related settings together. No functionality is added, removed, or changed — agents behave exactly as before. Highlights:

  - **Studio Assistant** is the new homepage — our prompt-based tool for building agents in natural language.
  - **Analytics** consolidates KPIs, custom metrics, custom dashboards, conversations, PolyScore, Smart analyst, agent analysis, and CSAT.
  - **Behavior** merges Agent settings and general configuration into one page with **General**, **Language**, and **Guardrails** tabs. Safety filters move behind an **Advanced** button.
  - **Knowledge** renames Managed topics → **FAQs**, Connected knowledge → **Sources**, and pulls Variants in as a tab.
  - **Simulation testing** replaces Test suite (which is being sunset). See [Simulation testing](/simulation-testing/introduction).
  - **Real-time config** combines the real-time config and the configuration builder.
  - **Voice** consolidates 8 separate pages — Settings, Numbers, Handoffs, Message templates, Audio library — onto one page, with Model, Call, and Speech settings behind an **Advanced** button.
  - **Messaging** follows the same pattern for chat settings.
  - **Integrations** becomes its own section (App, API, MCP).
  - **Widgets** moves out of Messaging into its own standalone page for managing widgets across channels.

  While the rollout is in progress, a toggle in Studio lets you switch between the old and new interface at any time.
</Update>

<Update label="June 2026">
  <Card>![june-release-main-image](/images/agent-settings/guardrails.png)</Card>

  Click the **bolded** titles for a breakdown of what's new, or go to the [notes tab](./notes/26.06) for full details.

  - **[Platform Guardrails](./notes/26.06#guardrails)**: Five pre-built safety protections – jailbreak defence, scope and hallucination control, AI identity, emergency escalation, and tool call integrity – are now applied automatically to every conversation, observable in transcripts, and toggleable per project in [Configure > General](/behavior/guardrails/introduction).
  - **[A/B testing (Beta)](./notes/26.06#ab-testing)**: Promote a second version to Live alongside the current one with a configurable traffic split, compare real-traffic metrics in your dashboards, and pick a winner – available behind a feature flag on US and UK enterprise clusters. See [A/B testing](/environments-and-versions/ab-testing).
  - **[Agent Builder writes simulation tests](./notes/26.06#agent-builder-test-suite)**: [Agent Builder](/studio-assistant/introduction) can now author, run, and maintain [simulation test cases](/simulation-testing/introduction) alongside the agent it's building, keeping coverage in step with your changes.
...
<Update label="May 2026">
  <Card>![may-release-main-image](/images/analytics/conv-review-side-panel.png)</Card>
...
<Update label="April 2026">
  <Card>![april-release-main-image](/images/release-notes/2604/add-languages.png)</Card>
  ...
  - **[MCP integrations](./notes/26.04#mcp-integrations)**: Connect [external tool servers](/integrations/mcp) to your agent from **Configure > Integrations > MCP**.
</Update>

<Update label="March 2026">
  <Card>![march-release-main-image](/images/release-notes/2603/source-hub-file-preview.png)</Card>
...
<Update label="June 2025">
  ...
  - **[Test suite](./notes/25.06#test-suite)**: Save real conversations as [test cases](/simulation-testing/introduction) and re-run them against draft or sandbox versions to check for changes in behavior.
