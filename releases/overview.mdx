...
<Update label="May 2025">
  ...
  - **[Duplicate and set default variants](./notes/25.05#duplicate-and-set-default-variants)**: From the 3-dot menu on the variant page, duplicate an existing variant or assign a new default with a simple toggle.
  - **[Build KBs with URL and PDF upload](./notes/25.05#build-kbs-with-url-and-pdf-upload)**: Create knowledge base topics directly from a website or uploaded PDF. Some URLs may not be supported due to site restrictions.
  - **[Latency metadata tag](./notes/25.05#latency-metadata-tag)**: Calls with high latency turns are now flagged for deeper analysis—identify slowness across conversations.
  ...
  - **[LLM annotations](./notes/25.05#llm-annotations)**: Flag good or bad LLM outputs using thumbs up/down. Future improvements will allow detailed correction suggestions.
  - **[Handoff reason and utterance in templates](./notes/25.05#handoff-reason-and-utterance-in-templates)**: You can now include structured `reason` and `utterance` fields when triggering a built-in handoff, ensuring clarity in logs.
</Update>
