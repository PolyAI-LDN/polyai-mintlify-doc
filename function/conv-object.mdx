---
title: "Conversation"
description: "Comprehensive reference for the Conversation (conv) object, its attributes, and methods."
---

The `Conversation` object (`conv`) provides access to critical conversation data and utilities for controlling the assistant's behavior during
...
<AccordionGroup>
  ...
  <Accordion title="env">
    **Description**: Identifies the current [environment](/environments-and-versions/introduction) in which the assistant is running.
  ...
  <Accordion title="state">
    **Description**: A dictionary-like object for storing and persisting variables throughout the conversation. Use it to manage contextual data and share information between function calls.
...
Here’s a comprehensive example utilizing multiple attributes and methods of the `conv` object:

```python
def manage_appointment(conv: Conversation):
    ...
    caller_id = conv.sip_headers.get("From", "Unknown Caller")
    conv.state.caller_id = caller_id
    print(f"Caller ID: {caller_id}")

    # Step 3: Retrieve and store branch metadata
    branch_name = conv.sip_headers.get("Branch", "Default Branch")
    conv.state.branch_name = branch_name
    print(f"Branch: {branch_name}")

    # Step 4: Transition to a new flow if needed
    if branch_name == "Priority Branch":
        conv.goto_flow("priority_flow")
    else:
        conv.goto_flow("default_flow")

    # Step 5: Send confirmation SMS
    conv.send_sms_template("appointment_confirmation", {"branch": branch_name, "caller": caller_id})
    return "Appointment managed successfully."
```
