...
The `Conversation` object (`conv`) provides access to critical conversation data and utilities for controlling the assistant's behavior during interactions. It centralizes functionality for managing states, flows, SMS communication, and the current environment.
