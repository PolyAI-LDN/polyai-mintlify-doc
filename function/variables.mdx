...
<Note>
`conv.state` supports both **dot notation** and **dictionary-style access**. Both are equivalent:

```python
# Dot notation
conv.state.user_name = "Alice"
name = conv.state.user_name

# Dictionary-style access
conv.state["user_name"] = "Alice"
name = conv.state.get("user_name", "Unknown")
```

Use `.get(key, default)` when a variable may not be set yet, to avoid errors.
</Note>
