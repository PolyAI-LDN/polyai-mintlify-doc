...
<Warning>
**`eleven_v3` and `optimize_streaming_latency`:** Do not set `optimize_streaming_latency` when using `eleven_v3` — it is not supported and will cause a 400 error, resulting in a silent fallback to Google TTS.
</Warning>
...
<Note>
Azure's `role` and `style` parameters are only supported by a limited set of voices and locales. Unsupported values fail silently. Check [Azure's documentation](https://learn.microsoft.com/en-us/azure/ai-services/speech-service/speech-synthesis-markup-voice#use-speaking-roles) for supported voices before using either parameter.
</Note>
...
<Warning>
There is a known issue where voice settings configured in Agent Studio may not match what is actually used in TTS requests, even after manually updating. If you experience unexpected voice behavior after changing settings, try the cache-busting workaround above or contact support.
</Warning>
