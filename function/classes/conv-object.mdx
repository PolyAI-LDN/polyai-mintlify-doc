...
<Accordion title="channel_type">
  ...
  <Accordion title="caller_number">
    **Description**: The caller's identifier.
    For SIP calls, this is the phone number in [E.164](http://twilio.com/docs/glossary/what-e164) format, where +1 is the area code for the USA.
    For chat channels, this may be an email address or integration-provided user ID. The value is not guaranteed to be None.
    **Example**:
    ```python
