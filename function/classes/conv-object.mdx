...
<AccordionGroup>
  ...
  <Accordion title="randomize_voice">
    ...
    **Available voices**: Import from `polyai.voice` module (e.g., `ElevenLabsVoice`, `CartesiaVoice`, `PlayHTVoice`, `RimeVoice`). See [Voice classes](/function/classes/voice) for the full list of available voices and their IDs.
    
    **Example**:
    ```python
    from polyai import VoiceWeighting
  ...
  <Accordion title="utils">
    ...
    **Note**: Some utilities require activation. If a method raises a `NotImplementedError`, contact [PolyAI support](mailto:support@poly.ai) to enable it for your account.
  ...
  <Accordion title="send_email">
    ...
    **SMTP configuration**: Emails are sent through a managed delivery service. For custom email delivery requirements, contact [PolyAI support](mailto:support@poly.ai).
