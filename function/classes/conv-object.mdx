...
<AccordionGroup>
  ...
  <Accordion title="set_voice">
    **Description**: Change the assistant voice for future replies.

````
**Example**:
```python
conv.set_voice(voice=Voices.en_female_warm)
```
````
...
The method supports optional `reason` and `utterance` fields:
