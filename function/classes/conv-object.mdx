...
<Accordion title="Quick reference — all attributes and methods">

| Name | Type | Description |
|------|------|-------------|
| `id` | attribute | Unique conversation identifier |
| `account_id` | attribute | PolyAI account ID |
| `project_id` | attribute | Project ID of the current agent |
| `env` | attribute | Current environment (sandbox, pre-release, live) |
| `channel_type` | attribute | Channel type (VOICE, sms, webchat, etc.) |
| `attachments` | attribute | List of file attachments |
| `sip_headers` | attribute | SIP headers from the inbound call |
| `integration_attributes` | attribute | Metadata from external integrations (start function only) |
| `integration_data` | attribute | Additional integration payload |
| `caller_number` | attribute | Caller's phone number |
| `callee_number` | attribute | Dialled number |
| `state` | attribute | Conversation state dictionary |
| `current_flow` | attribute | Name of the active flow |
| `current_step` | attribute | Name of the active step |
| `variant_name` | attribute | Active variant name |
| `variant` | attribute | Active variant object |
| `language` | attribute | Current language code |
| `history` | attribute | Conversation turn history |
| `handoffs` | attribute | Configured handoff destinations |
| `memory` | attribute | Persistent agent memory |
| `entities` | attribute | Extracted entities |
| `api` | attribute | API integrations interface |
| `say` | method | Speak or display a message |
| `randomize_voice` | method | Randomly select from weighted voices |
| `goto_flow` | method | Start a named flow |
| `exit_flow` | method | Exit the current flow |
| `set_variant` | method | Switch to a different variant |
| `call_handoff` | method | Transfer to a handoff destination |
| `send_sms` | method | Send an SMS message |
| `send_email` | method | Send an email |
| `write_metric` | method | Write a custom metric |
| `set_voice` | method | Set the TTS voice |
| `set_language` | method | Set the conversation language |
| `set_asr_biasing` | method | Configure ASR biasing |
| `goto_csat_flow` | method | Route to the CSAT survey flow |
| `utils` | method | Access conversation utilities |
