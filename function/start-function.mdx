...
<AccordionGroup>
  <Accordion title="Detect channel type" icon="display">
    Use [`conv.channel_type`](/function/classes/conv-object#channel_type) to determine whether the conversation is voice, webchat, or another channel — then branch accordingly. Disable call transfers for webchat, set a different persona, or inject channel-specific prompts.
  </Accordion>
  ...
  <Accordion title="Set the active variant" icon="location-dot">
    Use [`conv.set_variant()`](/function/classes/conv-object#set_variant) to route the conversation to a specific [variant](/variant-management/introduction) based on SIP headers, callee number, or other metadata. This is the standard way to configure multi-site agents.
  </Accordion>
