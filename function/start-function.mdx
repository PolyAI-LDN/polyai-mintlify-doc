...
<Warning>
The start function is **synchronous** — it must complete before the greeting plays. Long-running API calls or heavy computation will delay the greeting and create silence for the caller. Keep execution fast.
</Warning>
...
* Choose an alternative TTS system using [PlayHT](https://docs.play.ht/reference/api-getting-started), [Rime](https://docs.rime.ai/api-reference/voices), or [Cartesia](https://docs.cartesia.ai/api-reference/tts/tts).
...
A single deployment can use multiple voice agents. A range of voices acts as a retention boost for repeat users, because each voice has its own personality and will handle enquiries in subtly different ways.
