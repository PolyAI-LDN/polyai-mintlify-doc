...
For outbound calls, PolyAI can detect whether a human, IVR, or voicemail system answered the call. Using an `outbound_check_recipient` flow, the agent classifies what it hears at the start of the call — before the conversation begins — and can take different actions depending on the result.

This is useful for:

- **Skipping voicemail** — End the call if a voicemail system picks up
- **Leaving a message** — Deliver a pre-configured message to voicemail
- **Navigating IVR menus** — Handle automated systems before reaching a live person

<Note>
Voicemail detection is a project-level configuration. Contact your PolyAI representative to enable it for your outbound calls.
</Note>
