...
Outbound calls need to determine who — or what — answered before the conversation begins. PolyAI uses an entity detection flow on the first turn to classify the recipient into categories such as:

| Classification | Description | Typical action |
| --- | --- | --- |
| `human` | A live person answered | Proceed to greeting and main conversation |
| `ivr` | An automated phone system answered | Enter IVR traversal (DTMF, hold loops) |
| `voicemail` | A voicemail system answered | Leave a message or hang up |
| `number_not_in_service` | The number is disconnected | End the call with an exit code |

### How it works

Detection runs on the first step of the outbound flow with:

- A dedicated **ASR model** (`voicemail`) configured via experimental JSON to optimize transcription for machine-generated speech
...
- An **LLM-based classification** function that evaluates the transcript to determine whether the response is from a human or a machine

The agent uses both the ASR transcript and common voicemail keywords (e.g., "forwarded", "voicemail", "leave a message") to make the classification. Once detected, the agent routes to the appropriate path — human greeting, IVR navigation, voicemail message, or call termination.
