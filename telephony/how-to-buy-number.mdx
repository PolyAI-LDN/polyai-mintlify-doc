...
<Steps>
  ...
  <Step title="Pick an area code">
    For US numbers, the area code picker shows each available area code alongside its **region** (state) and **locality** (city), so you can match the number to the geography your callers expect.

    Area codes in the same region as your project's first existing phone number appear at the top of the list. The rest are sorted alphabetically. If your project has no existing US numbers yet, all area codes are sorted alphabetically.
  </Step>
  <Step title="Confirm">
...
<Tip>Keeping all of a project's numbers in the same region makes the local-first sort more useful — every new purchase surfaces nearby area codes first.</Tip>
