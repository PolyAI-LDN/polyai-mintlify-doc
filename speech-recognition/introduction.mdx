...
3. **Per-flow** – biasing applied to every step within a flow (configured by PolyAI)
4. **Per-language** – biasing applied only when the conversation is in a specific language (configured by PolyAI; see [Diacritics](#diacritics))
5. **Global** – biasing configured on this Speech Recognition page

If the same phrase appears at multiple levels, the highest-priority setting takes precedence. For example, dynamic biasing overrides per-step, per-step overrides per-flow, and so on down to global.
