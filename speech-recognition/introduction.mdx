...
| `NatWest` | Brand name frequently misheard as "not West" | Maximum |
...
<Warning>
Transcript corrections are powerful but can introduce errors if the pattern is too broad. A correction like `not West` → `NatWest` will also fire on legitimate uses of "not west" in other contexts. Use specific regex patterns and test corrections against a range of real transcripts before deploying.
</Warning>
...
| `/\b(not West\|Nat West)\b/i` | `NatWest` | Partial transcript | Brand name correction |
| `/\bDr\.?\s?Patel\b/i` | `Dr. Patel` | Partial transcript | Proper noun / doctor name |
...
If your agent operates in a language that uses [diacritics](https://www.sussex.ac.uk/informatics/punctuation/misc/diacritics) — such as **č, ć, š, ž, đ** — you may need to configure transcript corrections to handle these characters.

These characters are common in languages like **Croatian**, **Serbian**, **Bosnian**, **Slovak**, **Czech**, and **Slovenian**. Some ASR models may strip or misinterpret diacritical marks, reducing accuracy for names and other proper nouns.

### What you can do

- **Add transcript corrections** for commonly affected words. For example, if the ASR model transcribes `Zeljko` instead of `Željko`, create a correction rule to fix it.
- **Test ASR output** in sandbox with native-language speakers to identify which terms are being affected.
- **Contact your PolyAI representative** if you need ASR language model changes (such as switching ASR language codes or enabling a different model for your target language). These changes are not self-serve and require configuration by the PolyAI team.

<Note>
ASR accuracy for non-English languages varies by model and language. If your agent handles multiple languages, ensure the correct ASR language is configured for each — an English-biased model may fail to detect non-English speech entirely. Reach out to your PolyAI contact for multilingual ASR configuration support.
</Note>
