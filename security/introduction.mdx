---
title: Security and privacy
sidebarTitle: Overview
description: How PolyAI protects data, meets compliance standards, and supports enterprise procurement requirements.
keywords:
  - security
  - privacy
  - compliance
  - GDPR
  - HIPAA
  - PCI
  - SOC 2
  - ISO 27001
  - data protection
  - enterprise
---

PolyAI is built for enterprise voice AI deployments where data security, regulatory compliance, and operational trust are non-negotiable. This section documents the standards we meet, how data flows through the platform, and what artefacts are available for procurement and security teams.

## Certifications and standards

| Standard | Status | Scope |
|---|---|---|
| **ISO 27001** | Certified | Information security management system (ISMS) |
| **SOC 2 Type II** | Achieved | Security, availability, processing integrity, confidentiality, privacy |
| **HIPAA** | Supported (where relevant) | Protected health information handling |
| **PCI-DSS** | Supported (where relevant) | Payment card data security |
| **GDPR** | Compliant | EU personal data protection |
| **Cyber Essentials Plus** | Certified | UK NCSC cyber threat protection |

For details on each certification, see [compliance certifications](/legal/compliance).

<Tip>
Contact your PolyAI account manager to request certification reports or discuss your specific compliance requirements.
</Tip>

## Data flow

The following diagram shows how data flows through the PolyAI platform during a voice call:

```mermaid
flowchart TD
    A[Caller] -->|Voice/SIP| B[PolyAI Telephony]
    B -->|Audio stream| C[Speech Recognition]
    C -->|Transcript| D[Raven LLM]
    D -->|Response text| E[Text-to-Speech]
    E -->|Audio stream| B
    D <-->|Knowledge retrieval| F[Knowledge Base]
    D <-->|API calls| G[Your External APIs]
    D -->|Conversation data| H[Data Storage]
    H -->|Export| I[Your S3 Bucket]
    H -->|API| J[Conversations API]
```

All data is encrypted in transit and at rest. Voice streams support SRTP encryption — see [handoff encryption options](/call-handoff/introduction) for configuration details.

## Data retention

Data retention is configurable per deployment. By default:

- **Transcripts and metadata** — retained for the duration of your contract, configurable on request
- **Audio recordings** — retained for the duration of your contract, configurable on request
- **Real-time conversation context** — held in memory for the duration of the call only

You can configure automatic export to your own infrastructure using the [S3 integration](/call-data/s3-to-s3) for long-term compliance archival.

## Deployment regions

PolyAI operates in three deployment regions. Your data stays in the region you choose.

| Region | API base URL |
|---|---|
| **US** | `api.us-1.platform.polyai.app` |
| **UK** | `api.uk-1.platform.polyai.app` |
| **EU West** | `api.euw-1.platform.polyai.app` |

## What artefacts can you request?

Enterprise procurement teams typically need the following. Contact your PolyAI account manager for any of these:

| Document | Description |
|---|---|
| **SOC 2 Type II report** | Independent auditor's report covering security controls |
| **ISO 27001 certificate** | Current certification from an accredited body |
| **Data Processing Agreement (DPA)** | For GDPR compliance |
| **Business Associate Agreement (BAA)** | For HIPAA-covered entities |
| **Security questionnaire** | Pre-filled responses available on request |
| **Architecture and data flow diagram** | Detailed infrastructure diagram for your security review |

## Related pages

<CardGroup cols={2}>
  <Card title="GDPR" icon="shield-halved" href="/security/gdpr">
    How PolyAI meets GDPR requirements for EU personal data
  </Card>
  <Card title="HIPAA" icon="hospital" href="/security/hipaa">
    Protected health information handling
  </Card>
  <Card title="Data handling" icon="database" href="/security/data-handling">
    What data PolyAI processes, where it's stored, and how to control it
  </Card>
  <Card title="Compliance certifications" icon="certificate" href="/legal/compliance">
    Full list of certifications and standards
  </Card>
</CardGroup>
