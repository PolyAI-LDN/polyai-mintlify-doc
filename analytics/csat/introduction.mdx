---
title: 'Surveys'
description: 'Measure customer satisfaction with post-conversation surveys'
---
...
Add `conv.goto_csat_flow()` to your hang-up function in **Build > Tools**. This instructs the agent to route users into the survey flow before ending the conversation.

<Note>If you are not the developer managing your agent's code, ask your technical team to set this up.</Note>
...
![goodbye_and_hang_up function definition in Build > Tools](/images/csat.png)
...
#### Custom utterance (optional)

If you want a custom final message before the CSAT survey, you can optionally set the utterance in `conv.state.csat_output_args` before calling `conv.goto_csat_flow()`:
...
<Note>
The custom utterance is **not required**. Without it, the agent proceeds directly into the CSAT survey flow using the default lead-in message configured in your CSAT settings.
</Note>
