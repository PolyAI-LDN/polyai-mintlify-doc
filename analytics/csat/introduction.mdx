...
CSAT surveys collect feedback from customers at the end of their conversation. The agent asks customers to rate their experience, and responses are tracked in your analytics dashboards.
...
<Note>
When using `conv.goto_csat_flow()` in your end function, there should be no return values. The function should end with `conv.goto_csat_flow()` and not return anything.
</Note>
...
The following features are under development. Availability is subject to change.

- **Webchat support** — CSAT surveys for webchat channels
- **Enhanced analytics** — Additional reporting and insights for CSAT data

<Note>
Contact your PolyAI representative for the latest availability and to discuss your CSAT requirements.
</Note>
