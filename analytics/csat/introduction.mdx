...
```python
@func_description("Routes the caller to the CSAT survey before ending the call")
...
```python
@func_description("Routes the caller to the CSAT survey with a custom message")
def goodbye_and_hang_up(conv: Conversation):
