...
## Integrating testing into your deployment workflow

The test suite is most effective when used as part of your [deployment pipeline](/environments-and-versions/introduction):

<Steps>
  <Step title="Make changes in Draft">
    Edit knowledge, flows, functions, or rules.
  </Step>
  <Step title="Run test sets against Draft">
    Execute your test sets to catch regressions before publishing. Focus on sets covering the areas you changed.
  </Step>
  <Step title="Fix failures">
    If tests fail, review transcripts to identify what broke. Common causes: topic changes that altered routing, function logic updates, or flow modifications.
  </Step>
  <Step title="Publish to Sandbox">
    Once tests pass in Draft, publish to Sandbox for broader testing.
  </Step>
  <Step title="Run test sets against Sandbox">
    Re-run tests in Sandbox to confirm behavior matches Draft. Environment-specific configuration (topic activation, phone numbers) can cause differences.
  </Step>
  <Step title="Promote through Pre-release to Live">
    With test results confirming no regressions, promote through the pipeline with confidence.
  </Step>
</Steps>
...
- **Build a core regression set** -- maintain a "Critical paths" set that covers your most important flows and run it before every promotion to Live
- **Test across environments** -- run the same set against both Draft and Sandbox to catch environment-specific issues
