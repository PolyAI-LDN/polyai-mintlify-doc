...
By default, test cases run on the **Agent Studio chat** channel (`chat.polyai`). For multichannel agents that behave differently per channel — for example, [behavior rules](/troubleshoot/faq-rules) wrapped in `<channel:webchat>` or `<channel:voice>` tags — you can pin a test case to a specific channel so the run reproduces that channel's behavior.

Supported channel values match those exposed on [`conv.channel_type`](/tools/classes/conv-object#channel_type):

| Value | Channel |
| --- | --- |
| `chat.polyai` | Agent Studio in-browser chat (default) |
| `webchat.polyai` | Webchat widget |
| `sip.polyai` | Voice (telephony/SIP) |
| `sms.twilio` | SMS |

When `channel` is unset, the runner falls back to `chat.polyai`, so existing test cases keep working unchanged.

<Tip>
Use this when validating channel-scoped rules or formatting. For example, pin a case to `webchat.polyai` to confirm webchat-only responses render correctly, and a sibling case to `sip.polyai` to confirm the voice variant.
</Tip>
