...
Save conversations as **Test Cases** and group them into **Test Sets** (**Build > Test suite**). Run against Draft or Sandbox to verify agent behavior before going live.
...
- **Test Case**
  A single scenario captured from a real conversation (user messages, agent replies, and the [functions](/function/introduction) that ran). Each case tracks its **Last run** and **Outcome**.
...
<Step title="[Optional] Edit test values">
...
<Tabs>
  ...
  The set displays an aggregated view with pass/fail counts and trend charts so you can see whether the area is ready to go live.
