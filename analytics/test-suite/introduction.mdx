...
**Test Suite** (found under **Build > Test suite** in the sidebar) lets builders save real conversations and re-run them to check whether the agent continues to behave as expected. It’s designed for teams working with generative AI, where responses may vary — making repeatable testing essential.
...
Test Sets can also be configured to run automatically when you publish or promote an agent, ensuring regression checks happen before deployment.
