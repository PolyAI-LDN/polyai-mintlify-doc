...
<Note>The Live conversations panel requires both **Conversations read** and [**PII access**](/tools/classes/conv-log#pii). Users without PII permission cannot list or open in-progress conversations, because live records expose un-redacted turns and contact details. If the banner and panel are missing for an account that has Conversations read, ask an admin to grant PII access.</Note>
