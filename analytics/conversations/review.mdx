...
### Diagnostics and AI summary on PII conversations

For PII-restricted users, additional surfaces are locked down on any conversation that contains PII so sensitive data can't leak through side channels:

- **Diagnosis layers are hidden** — the [Diagnosis](/analytics/conversations/diagnosis) toggle group is removed for PII-restricted users when the conversation contains PII. Users with PII access continue to see all diagnostic layers as normal.
- **AI summary is hidden** — when a conversation contains PII, its AI-generated [call summary](/analytics/polyscore) is hidden from PII-restricted users in both the side panel and the full-page review.

Redaction itself runs on PolyAI's LLM data pipeline, with detection tuned to catch PII such as Social Security Numbers across transcripts and call recordings.
