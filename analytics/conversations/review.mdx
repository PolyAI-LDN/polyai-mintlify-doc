...
Redaction is fail-closed for safety. When you request redacted audio, PolyAI bleeps segments that match PII detected in the redacted transcript for that call. If the redacted transcript is not yet available — for example, immediately after a call ends, before post-call processing completes — the request returns no audio rather than falling back to the unredacted recording. Retry after a short delay once processing is complete.
