...
- **Caller identity**: Displays the user email address if available (for [Webchat](/webchat/introduction)), or phone number if applicable. Personally identifiable details are shown only to users with [PII-level access](/tools/classes/conv-log#pii). For users without PII access, caller phone numbers are automatically redacted as `REDACTED` in the conversations list. By default, inbound calls hide the `from_number` and outbound calls hide the `to_number`, so the opposite number is preserved and you can still tell which line was used. Projects can override which phone-number fields are redacted per call direction — contact your PolyAI representative if you need a different redaction policy.
