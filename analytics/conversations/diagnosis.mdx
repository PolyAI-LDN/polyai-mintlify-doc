...
Shows the full LLM request made by the agent for a given turn. Click the **Request** button on a turn to open a side panel with the complete request payload, including conversation history, system prompt, and model metadata.
...
<Info>
Latency visualization includes detailed breakdowns to help identify performance bottlenecks.
</Info>
...
Displays [function logs](/tools/classes/conv-log) and any structured `conv.log` entries emitted during runtime.
...
4. Use **Sources** to verify the content the agent retrieved from Connected Knowledge.
5. Use **Flows and steps** to verify the agent followed the expected path.
