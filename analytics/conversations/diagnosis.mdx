...
## Diagnosis layers and PII access

Diagnosis layers expose detail captured from the underlying conversation — variable values, tool arguments, retrieved Knowledge chunks, extracted entities, ASR alternatives, and runtime logs — any of which may contain personally identifiable information.

Users without [PII-level access](/tools/classes/conv-log#pii) see the Transcription tab with redacted utterances, but the following layers return no data:
...
Tool calls still appear so you can confirm which tools ran and whether handoff or hangup events fired, but the arguments, return content, logs, and state changes recorded against each call are removed.

Grant PII-level access to reviewers who need the full diagnostic surface for debugging or QA. Reviewers who only need to audit conversation outcomes can stay on standard access without exposing the underlying PII.
