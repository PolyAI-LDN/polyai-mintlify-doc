...
<Note>
**On the Open platform**, Conversations covers **Voice** only — every PolyPhone call is captured here as a transcript and recording. Web chat conversations are enterprise-only at launch. Smart Analyst is built into the [Studio Assistant](/studio-assistant/introduction) home as a chat panel rather than a standalone tab, and the [PolyScore](/analytics/polyscore) overview shows on the standard dashboard; the full PolyScore breakdown view is enterprise-only.
</Note>
</div>
