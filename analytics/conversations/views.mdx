...
- **Filters** – conditions that narrow the conversations table (PolyScore, duration, start date, environment, function calls, SMS events, and so on). You can add up to **10 filters per view**.
...
Click the **+** tab button to the right of your existing view tabs. A dialog appears with two options:
...
Open the **three-dot menu** (⋮) on any Custom View tab to access view actions:
...
Use **Share filters as link** to copy a URL that encodes your current filter configuration. When a teammate opens the link, the filters are applied temporarily to their view. They can dismiss the filters to return to their own view, or save them as a new Custom View of their own.

Sharing a link never silently overwrites someone else's existing setup, and the view itself remains private to your account.
...
- **Visibility** – toggle columns on or off. **Start time** is always visible and cannot be hidden. Other available columns include Contact, PolyScore, Summary, Duration, Language, Call SID, Variant, QA, Handoff To, A/B Group, and Environment.
