...
description: "Automated conversation quality scoring across voice and webchat."
keywords:
  - PolyScore
  - quality score
...
PolyScore is an AI-powered quality score assigned to every eligible conversation – **voice calls and webchat sessions**. It evaluates the transcript and produces a normalized **0–10 rating** along with per-dimension breakdowns, helping you identify strong and weak conversations without manual review.

<Note>
PolyScore now covers webchat as well as voice. The same badge, dimensions, and color coding apply across both channels, so you can compare quality on equal footing.
</Note>
...
PolyScore evaluates every eligible conversation automatically – voice or webchat. The model reads the full transcript and rates six behavioral signals, which are combined into two sub-scores and then normalized to a 0–10 scale.
...
| Requirement | Detail |
|---|---|
| **Channel** | Voice and webchat conversations are scored. SMS-only conversations are not scored. |
| **Minimum turns** | At least 3 turns. Short interactions (for example, immediate hangups or one-line chat sessions) are skipped. |
...
Filter conversations by PolyScore in the Conversations table to build a daily QA workflow – review a sample of low-scoring voice calls and webchat sessions to catch recurring issues early. Each [Conversations](/analytics/conversations/review) scope (Voice, Webchat) keeps its own PolyScore column, so you can run separate QA queues per channel.
