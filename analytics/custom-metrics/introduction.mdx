...
Each call fires in real time as the conversation progresses and is immediately available in [Dashboards](/analytics/dashboards/introduction), [Wren](/wren/analyze), and the [Conversations API](/api-reference/conversations/introduction).
...
You decide what to track. Add `conv.write_metric()` calls anywhere in your flow functions. See [Metrics](/analytics/kpis/introduction) for naming conventions and how metric definitions affect Wren.
...
Writing each extracted field back with `conv.write_metric` is what makes the extracted values available in dashboards, Wren, and API exports alongside your real-time metrics.
...
| [Wren](/wren/analyze) | Built-in AI assistant. Ask *"What are the top reasons callers request a human this week?"* and get answers linked to actual conversations |
...
Once metrics are written, your team can ask Wren directly with questions like:
...
Your agent handles a call → real-time metrics fire during the call → the call ends → post-call extraction runs automatically → all data lands in dashboards, Wren, and the Conversations API.
...
<CardGroup cols={3}>
  ...
  <Card title="Analyze conversations" icon="magnifying-glass-chart" href="/wren/analyze">
    Ask Wren natural-language questions about your metrics and transcripts.
  </Card>
