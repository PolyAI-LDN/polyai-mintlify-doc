...
Each category can be individually enabled or disabled using the toggle, and set to a strictness level:

| Strictness level | Behavior |
|----------|----------|
| **Loose** | Most content allowed — only the most severe violations are filtered |
| **Medium** | Balanced filtering |
| **Strict** | Aggressive filtering — blocks a wider range of potentially harmful content |

<Note>
The safety dashboard displays Azure's upstream severity classifications (Safe, Low, Medium, High) for detected content. The strictness levels above (Loose, Medium, Strict) control what threshold of severity triggers a block in your agent's responses.
</Note>
