...
For outbound calls, PolyAI has built-in voicemail detection. Using an `outbound_check_recipient` flow, the system classifies what it hears at the start of a call — human, IVR, or voicemail — before the conversation begins. This allows the agent to take different actions depending on whether a live person or a voicemail system answered.

See [Outbound calling](/telephony/outbound-calling) for more on outbound call configuration.

### Inbound SIP handoffs

When PolyAI transfers an inbound call to an agent (via SIP REFER, SIP INVITE, or SIP BYE), voicemail detection is **not available by default**. The behavior depends on the handoff method:
...
If your project includes custom flow logic that detects suspected voicemail *during* a call (before transfer), you can flag the handoff reason using [SIP headers](#configuring-sip-headers):
...
This passes voicemail context to your downstream system (e.g., your contact center or CRM) as part of the transfer. This requires custom implementation — contact your PolyAI representative to discuss your specific use case.
