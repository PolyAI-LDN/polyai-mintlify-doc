...
<Info>
**Prerequisites:** Understanding of SIP telephony or your contact center's routing setup. This section covers technical handoff configuration.
</Info>

Define destinations for transferring users to live agents when the automated agent cannot help. For voice, this routes calls via telephony; for webchat, this can connect users to live chat agents.

<Tip>Looking to manage handoff states programmatically? Visit the [Handoff API documentation](/api-reference/handoff/introduction).</Tip>

<Note>
The SIP-based handoff methods described below apply to voice interactions. Webchat handoffs use HTTP-based integrations with your live chat platform.
</Note>
...
<Note>
The UI-based Call Handoff does not support custom metrics. If you need to track handoff-specific metrics (e.g., handoff reason codes for analytics), use the `transfer_call` function with [`conv.write_metric()`](/function/classes/conv-object) instead.
</Note>

<Warning>
If you use `transfer_call`, you must keep destination mappings in sync manually — changes made in the Call Handoff UI are not automatically reflected in function-based transfers. Verify your `transfer_call` destinations match your intended routing whenever you update handoff configuration.
</Warning>
