...
title: Handoff
description: "Set up handoff destinations to transfer users to human agents during conversations."
tag: "Code"
keywords:
  - handoff
  - call transfer
...
## Conversation history on messaging handoffs

When a conversation is handed off to a live agent through a messaging channel (webchat, SMS, or other text-based channels), the platform automatically sends the conversation transcript to the receiving agent. The transcript includes all prior messages formatted as a readable log, giving the live agent full context without requiring the caller to repeat themselves.

This applies to all supported messaging handoff providers and works automatically — no additional configuration is required.

<Note>
Conversation history is only sent for new handoffs. If a handoff session is recovered or restored (for example, after a temporary disconnection), the transcript is not resent.
</Note>
