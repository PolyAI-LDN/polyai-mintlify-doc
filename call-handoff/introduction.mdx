...
For outbound calls, PolyAI has project-level voicemail detection. A detection flow at the start of the call classifies what the agent hears — typically as human, IVR, voicemail, or number not in service — before the conversation begins. Based on the classification, the agent routes into the appropriate path:
...
This detection uses barge-in on the first turn, since voicemail systems and IVRs do not wait for the agent to finish speaking. The flow is configured per-project — contact your PolyAI representative to enable it.
...
This passes voicemail context to your downstream system (e.g., your contact center or CRM) as part of the transfer. This approach requires custom flow logic to classify the caller's response and SIP header injection to pass the result downstream.
