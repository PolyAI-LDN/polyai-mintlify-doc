...
Webchat uses the same conceptual model but exchanges context over WebSocket events rather than SIP. The PolyAI agent emits a `poly_agent_triggered_handoff` event followed by `client_handoff_required`; the live-chat platform handles queueing (`handoff_queue_status`) and acceptance (`handoff_accepted`), then the human appears in the session as `live_agent_joined`. See [Webchat handoff in conversation events](/call-data/conversations-api/handoff-states#live-agent-events) for the full event sequence.
