...
The assistant collects the user’s **first** and **last** name using separate steps, each with its own prompt and transition logic.

- If the user has already provided the name, the assistant calls the relevant function (e.g. `save_first_name`, `save_last_name`) and proceeds.
- If not, the assistant asks for it directly.
...
The assistant attempts to match the user's provided details against active reservations.
