...
Use [Managed Topics](/knowledge/faqs/introduction) for simple question-and-answer interactions. Use flows when the conversation requires a specific sequence of steps, input validation, or branching logic.
...
<Tip>
**Use [Agent Builder](/studio-assistant/introduction) to scaffold a flow from a description** — *"build a booking flow that collects party size, date, time, and name, then confirms"*. Open **Build > Flows** to review, or keep asking Agent Builder to refine (*"add a confirmation step"*, *"skip the email field if it's already on file"*). Every change lands on a branch.
</Tip>
