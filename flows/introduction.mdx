...
<Tip>
To initiate a flow programmatically at the end of a turn, use conv.goto_flow. See How to trigger a flow below.
</Tip>
...
### 1) Trigger a flow from code (most explicit)

Use this when you are in a function and you want to start a flow deterministically.
...
### 2) Trigger a flow from a knowledge base action (LLM-invoked)

Use this when a knowledge base topic should reliably move the user into a structured interaction.

Typical pattern:
- The user asks something that requires multi-step collection or validation
- The knowledge base topic includes an action that starts the flow
- The flow takes over to collect entities, retry, validate, and finish cleanly
