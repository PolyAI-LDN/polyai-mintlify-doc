...
<Note>
**On the Open platform**, [Studio Assistant](/studio-assistant/introduction) scaffolds flows for you from your description: booking, lead capture, FAQ routing, and other common patterns. Once it's built, you can inspect the flow in **Build > Flows** and ask Studio Assistant to tweak it ("add a confirmation step", "skip the email field if it's already on file"). Function steps and Python-driven branching are part of the platform; the no-code path is enough for most self-serve agents.
</Note>
</div>
