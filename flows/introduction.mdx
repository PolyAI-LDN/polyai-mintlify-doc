...
<Note>
**On Agent Builder**, all flows are [no-code flows](/flows/no-code/introduction): Default steps and entity extraction, built visually. [Studio Assistant](/studio-assistant/introduction) can scaffold one from a description (booking, lead capture, FAQ routing, and other common patterns), and you can inspect it in **Build > Flows** and ask the assistant to tweak it ("add a confirmation step", "skip the email field if it's already on file"). Function steps, transition functions, and other code-driven flow paths described on this page aren't part of Agent Builder.
</Note>
</div>
