...
<Note>
Build flows visually with [no-code flows](/flows/no-code/introduction): Default steps and entity extraction. Ask [Studio Assistant](/studio-assistant/introduction) to scaffold one from a description (booking, lead capture, FAQ routing), then open **Build > Flows** and tweak by asking ("add a confirmation step", "skip the email field if it's already on file").
</Note>
</div>
