...
Flows (**Build > Flows**) handle structured interactions requiring input collection, validation, API calls, or specific sequences.
...
2. Any relevant [Managed Topics](/managed-topics/introduction) or [Connected knowledge](/connected-knowledge/introduction) results (if applicable).

### Making Managed Topics functions available in flows

Functions attached to Managed Topics actions are not available inside flows by default. To make them available while a flow is running, add the following to your agent's **Voice configuration** or **Chat configuration** settings (under the advanced LLM configuration section). Contact your PolyAI representative if you need help locating this setting.
