...
<Tip>
**Use [Studio Assistant](/studio-assistant/introduction) to scaffold a flow from a description** — *"build a booking flow that collects party size, date, time, and name, then confirms"*. Open **Flows** to review, or keep asking Studio Assistant to refine (*"add a confirmation step"*, *"skip the email field if it's already on file"*). Every change lands on a branch.
</Tip>
