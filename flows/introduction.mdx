...
A flow (found under **Build > Flows** in the sidebar) is a structured interaction made up of steps, each containing a prompt and one or more actions. You use flows when an interaction requires:

- Collecting user input (e.g. name, phone number, confirmation code)
- Performing validation or retries
- Calling internal or external [APIs](https://en.wikipedia.org/wiki/API)
- Routing, [handoff](/call-handoff/introduction), or escalation
...
2. Any relevant [knowledge base](/managed-topics/introduction) entries (if applicable).
