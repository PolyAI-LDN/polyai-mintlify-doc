...
description: 'Use example-driven prompts to improve accuracy and reduce ambiguity across flow steps, topic actions, and agent behaviour prompts'
---

[Few-shot prompting (FSP)](https://www.ibm.com/think/topics/few-shot-prompting) is a technique for guiding the agent by showing it examples of what users might say — and how the agent should respond. This helps the agent:

- Handle vague or unexpected inputs and choose the right action
...
Few-shot prompting works anywhere the agent reads a prompt. The most common places in Agent Studio are:

- **Flow step prompts** — the step prompt field in the Flow Editor
- **Topic actions** — the action prompt within a managed topic
- **Agent behaviour prompts** — global rules that shape the agent's overall behaviour
...
<Tip>
Because step prompts are inserted last in the LLM input stack, these examples appear right before the agent responds — making them very effective.
</Tip>
...
- A matching **agent response** — often a reply paired with an action
...
![Few-shot examples inside a flow step prompt for collecting a last name](/images/last-name-fsp.png)
...
<Warning>
Too many examples can make the model too rigid or cause it to copy examples too closely instead of adapting. If the agent starts parroting your examples word-for-word instead of generalising, reduce the number of examples or make them more varied.
</Warning>

## Tips for strong few-shot examples

- **Use realistic language** — write examples that sound like actual callers, not idealised or overly formal phrasing.
- **Show both success and edge cases** — include at least one tricky input so the model knows how to handle it.
- **Match the agent's persona** — if the agent has a name and tone, use them consistently in the example responses.
- **Pair responses with function calls** — show the model exactly which function to call and with what arguments.
...
- [Style guide](/get-started/style-guide) — acceptance checklist includes FSP best practices
