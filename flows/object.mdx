...
- **`condition_label`** (optional) — a label for the transition edge. In [Function steps](/flows/no-code/introduction), this label appears on the edge in the visual editor for readability.
...
```python
@func_description("Validates and saves the user's first and last name")
@func_parameter(name="first_name", description="The user's first name")
@func_parameter(name="last_name", description="The user's last name")
...
```python
@func_description("Completes the booking and exits the flow")
...
- [Conversation object](/tools/classes/conv-object) — full reference for `conv.goto_flow()`, `conv.exit_flow()`, and other methods
