...
<Warning>
**Anti-pattern: infinite verification loop when the flow never sets `is_verified`.**

If the protected function calls `conv.goto_flow("Identify & Verify User")` whenever `conv.state.is_verified` is falsy, and the verification flow never writes `is_verified = True` on success, the user is sent back into verification every turn — forever.

```python
# Protected function — re-triggers verification on every call until is_verified is True
def check_account_balance(conv: Conversation, account_type: str):
    if not conv.state.get("is_verified"):
        conv.state.original_topic = "check_account_balance"
        conv.state.pending_account_type = account_type
...
**Two things have to be true to break out of the loop:**

1. **The verification flow must write `is_verified = True` on the success path.** Do this inside a transition function on the verification step that compares user input to the expected value:
...
2. **The protected function must have a terminal branch for repeated failures.** Otherwise, three failed attempts followed by the user returning to the original topic will re-trigger verification a fourth time. Guard on the attempts counter too:
...
See the [Escalating after repeated failed verification](#example-escalating-after-repeated-failed-verification) example below for the in-flow version of this pattern.
</Warning>
