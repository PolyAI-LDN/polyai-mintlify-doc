...
**[Dual-Tone Multi-Frequency](https://en.wikipedia.org/wiki/Dual-tone_multi-frequency_signaling)** (DTMF) is the method telephones use to convert keypad presses into audio tones that telecom systems can detect and process.
You can configure DTMF collection in a flow step to gather numeric input from callers — for example, phone numbers, booking IDs, confirmation codes, or consent responses.

<Warning>DTMF configuration is only available on **Advanced** flow steps. Low-code steps do not support DTMF or custom ASR settings.</Warning>

<Note>DTMF output is not currently supported with the **realtime model**. If your agent uses the realtime model, DTMF collection will not work.</Note>
