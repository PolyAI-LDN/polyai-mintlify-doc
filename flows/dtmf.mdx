---
title: 'DTMF'
description: 'Collect keypad input in flow steps with configurable digit limits, timeouts, and optional recording opt-out.'
tag: "Code"
keywords:
  - DTMF
  - keypad input
...
Use DTMF when callers need to enter numeric input with their phone keypad – phone numbers, account numbers, confirmation codes, or consent responses (e.g., "Press 1 to opt out of recording"). DTMF is more reliable than speech for structured numeric input.
...
![dtmf-main](/images/flows/dtmf.png)
...
![dtmf-timing-diagram](/images/flows/dtmf-opt-out.png)

## Global DTMF

By default, DTMF is configured per step. If you want callers to be able to enter keypad input on **every turn** of a conversation – without configuring DTMF on each individual step – your PolyAI representative can enable **global DTMF** on your agent.

When global DTMF is enabled:

- DTMF collection is active on every conversation turn from the start of the call.
- The agent processes both spoken and keypad responses throughout the conversation.
- Step-level DTMF configuration takes precedence – if a step has DTMF enabled, its settings override the global configuration for that step.

The precedence order is:

1. **Step-level DTMF** – when enabled on a step, the step's configuration applies.
2. **Global DTMF** – when no step-level DTMF is enabled, the global configuration applies.
3. **No DTMF** – when neither is configured, DTMF input is ignored.

Global DTMF supports the same options as step-level DTMF, including maximum digit count, end key, and whether to collect digits while the agent is speaking. Contact your PolyAI representative to enable global DTMF or adjust its settings.
...
![dtmf-opt-out-logic](/images/flows/dtmf-opt-logic.png)
