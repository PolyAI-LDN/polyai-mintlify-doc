...
The DTMF panel sits in the step configuration sidebar, below the [ASR biasing](/flows/asr-biasing) options.
...
![dtmf-opt-out](/images/dtmf-opt-logic.png)
...
<Tip>DTMF processing starts after the `start` function runs. If you want to collect the opt-out keypress *before* any other audio is played, override the greeting in your `start` function with an empty string. </Tip>
...
![dtmf-opt-out](/images/dtmf-opt-out.png)
