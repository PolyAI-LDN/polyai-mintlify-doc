...
In a Default step, entity processing and routing happen **sequentially** on each turn:

1. **Entity extraction and validation** — the agent extracts entity values from the caller's message and validates them against type rules. Invalid values are rejected and the caller is asked to try again.
2. **Condition evaluation** — once entities are validated, the agent evaluates which condition is best met. Conditions with **required entities** only match if those entities have already passed validation.

Validation always completes before condition evaluation begins. An entity that fails validation will not satisfy a "required entities" check — even if the caller provided a value.
...
In [Conversation Review](/analytics/conversations/review), you will see system-generated events for entity validation and condition evaluation. These are useful for debugging entity collection and routing issues.
