...
**No-code Flows** let you build conversational workflows visually — using prompts, entity extraction, and clear branching — without writing code. If you need advanced logic, you can add a Function step.
...
The second argument (`condition_label`) is purely decorative — it labels the edge in the flow editor but has no effect on runtime behaviour. You can pass any string.

<Warning>
There is a known UI bug where calling `flow.goto_step()` with a condition label inside a Function step can prevent edges from rendering in the flow editor. Your flow still works correctly at runtime.
</Warning>
