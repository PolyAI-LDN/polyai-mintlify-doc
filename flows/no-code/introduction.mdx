...
If you reference collected entities inside the prompt, you can use rich text `/entities`. This inserts a template tag in the format `{{entity:entity_name}}` that the system resolves at runtime. Default step prompts also support variant attribute references (`{{va:attributeId}}`), but **cannot** reference transition functions or global functions — use an [Advanced step](#advanced-step-reference) or [Function step](#function-step-low-code-still-visual) for that.

<Tip>
**Auto-transition:** When a Default step has only one outgoing condition and all its required entities are collected and valid, the system skips the LLM condition-selection call and transitions automatically. This reduces latency for simple linear flows.
</Tip>
...
- Allowed characters: letters (including accented characters), numbers, spaces, and `_ & , / . -`
- The **start step** cannot be deleted — reassign the start step to a different node first.
- Condition labels must be **unique within a step** and cannot exceed **50 characters**.
...
## ASR biasing and DTMF on Default steps

Default steps support the same ASR biasing and DTMF configuration as Advanced steps. Access these settings from the right-hand configuration panel when editing a step.

For details on each setting, see [ASR biasing in flows](/flows/asr-biasing) and [DTMF in flows](/flows/dtmf).
...
Both Advanced and Default steps support structured ASR biasing modes for common input types:
