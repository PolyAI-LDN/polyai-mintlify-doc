...
<Frame caption="Example no-code flow rules">
  <img src="/images/no-code-flows/default.png" alt="rules-examples" style={{ maxWidth: '720px', width: '100%', margin: '0 auto', display: 'block' }} />
</Frame>
...
<Note>
`conv.state` stores numeric entities as `int` or `float` based on the entity configuration. `conv.entities.<name>.value` is always a string -- cast to `int()` or `float()` before numeric comparisons. See [Entities -- Accessing entities in code](/flows/no-code/entities#accessing-entities-in-code).
</Note>
