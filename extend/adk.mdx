...
The Agent Development Kit (ADK) is a Python CLI (`polyai-adk` on PyPI) for building PolyAI agents locally. Pull your Agent Studio project as YAML and Python files, edit in your IDE (or with a coding assistant like Claude), version with Git, and push from the command line.

API keys are self-serve — sign up at [studio.poly.ai](https://studio.poly.ai) and generate one from your account.

<Note>
For installation, authentication, and usage instructions, see the [ADK documentation](https://polyai.github.io/adk/).
</Note>
