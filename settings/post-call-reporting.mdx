...
You decide what to track. Add `conv.write_metric()` calls anywhere in your flow functions. See [Metrics](/settings/metrics) for naming conventions and how metric definitions affect Smart Analyst.
...
For each extraction query you plan to expose in dashboards, register a matching [metric](/settings/metrics) and write the result back with `conv.write_metric`. This keeps real-time and post-call data in a single, queryable column per conversation.
...
<CardGroup cols={3}>
  <Card title="Metrics" icon="chart-bar" href="/settings/metrics">
