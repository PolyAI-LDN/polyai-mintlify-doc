...
A small set of metrics are written by the platform itself, not by your functions. They appear alongside custom metrics in dashboards and API responses, and are reserved – `conv.write_metric` cannot overwrite them.

| Metric | Type | Written when |
|--------|------|--------------|
| `RAVEN_OOD` | boolean | [Raven](/agent-settings/raven) flags a user turn as out-of-domain. Written once per conversation, the first time it occurs. Useful for tracking knowledge gaps and tuning agent scope. |
