...
### System-emitted metrics

A small number of metrics are emitted automatically by the platform and don't need to be defined under **Configure > Metrics**. The agent declares them for your project whenever they apply, so they pass validation and flow through to dashboards, Smart Analyst, and the Conversations API like any other metric.

| Metric | Auto-declared when | Value |
| --- | --- | --- |
| `CONVERSATION_LANGUAGE_CHANGED` | The project has one or more [additional languages](/agent-settings/multilingual) configured | The IETF locale code the conversation switched to (for example `"es-ES"`) |

If you define one of these metrics manually – for example to restrict it to a specific list of expected values – your configuration takes precedence and is not overwritten.
