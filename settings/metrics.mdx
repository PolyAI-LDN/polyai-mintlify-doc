...
- **Use a consistent format** — pick one naming style (e.g. `Booking completed` or `BOOKING_COMPLETED`) and apply it across all metrics in a project
...
- For yes/no metrics, state the exact condition that makes the metric `true`
...
| [Conversations API](/api-reference/conversations/introduction) | The `metrics` field on each conversation object contains your custom metric results, available for export to external analytics tools. |
