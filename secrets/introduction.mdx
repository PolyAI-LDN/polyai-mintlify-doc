...
Store API keys, tokens, and credentials securely so your agent's [functions](/tools/introduction) can call external services without exposing secrets in code. Without Secrets, credentials end up hardcoded in function code — a security risk that may expose them in logs or version history.
