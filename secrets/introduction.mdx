...
Secrets (**Build > Tools > Secrets**) let you store sensitive values — API keys, tokens, passwords, and other credentials — so your agent's [functions](/function/introduction) can call external services without exposing credentials in code.
...
3. Functions retrieve the secret at runtime using `secret_vault("secret_name")`.

```python
from agent_v3.deployment_service.agents.functions.builtin import secret_vault

api_key = secret_vault("stripe_api_key")
