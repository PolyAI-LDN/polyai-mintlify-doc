...
PolyAI's **Agentic Dialog Platform** is the platform that brands trust to handle their most complex conversations, now open to every builder across the enterprise. Agents answer questions, make bookings, resolve issues, and anything else you can think of, and run on [Raven](/agent-settings/raven), PolyAI's proprietary LLM with sub-300ms latency across 24+ languages.

Three capabilities are open today:

- **[Agent Builder](/studio-assistant/introduction)** — describe your business needs in natural language. Agent Builder configures the agent and conversation tracks & guardrails automatically in minutes. Test it live, analyze call data, and refine through an ongoing dialog with the platform.
- **[Shareable testing](/widgets/test)** — every agent ships with a zero-setup, shareable test environment. Interact with it live across channels, get stakeholder buy-in, and validate behavior before anything touches production.
- **[Agent Development Kit (ADK)](/extend/adk)** — self-serve API keys, native integrations, and full CLI support. Build in your own IDE, version with Git, and deploy from your terminal.
...
PolyAI's Agentic Dialog Platform is open to every builder. Agent Builder is the self-serve way in: describe what you want, get a working agent in minutes.

# Build a voice agent in minutes

Tell [Agent Builder](https://studio.poly.ai/) what you need. It configures the agent — flows, knowledge, voice, and guardrails — automatically. Test it live in a shareable environment, then drop it on your site. No code, no sales call, just describe and deploy.
...
Copy one into [Agent Builder](/studio-assistant/introduction) and you'll have a working agent in minutes.
