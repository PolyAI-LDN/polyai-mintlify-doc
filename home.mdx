...
These docs help you build a voice or chat agent that completes the customer's request — taking a booking, processing a payment, filing a claim, handling an escalation — rather than routing every call to a human.

The same platform runs production agents at banks, hotels, healthcare providers, and other businesses where accuracy and compliance matter. You're building on it directly.

**What to know before you start:**

- **The platform is built for resolution, not deflection.** [Raven](/agent-settings/raven), [flows](/flows/introduction), and [grounded knowledge](/managed-topics/introduction) work together so the agent finishes the task instead of handing it off.
- **Voice-first and multilingual.** Sub-300ms responses across 24+ languages.
- **Three ways to build the same agent.** Use [Agent Studio](/get-started/introduction) for visual configuration, the [ADK](/extend/adk) for a local Git-based workflow, or the [REST APIs](/api-reference/introduction) for programmatic control. Pick the surface that fits your team.
- **Every change is testable before it ships.** Each edit lands in a [shareable test environment](/widgets/test). Promote to production when you're ready.
