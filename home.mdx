...
<img src="/images/home/front-page-graphic.png" alt="PolyAI platform documentation" noZoom style={{ display: 'block', margin: '0 auto 2rem', maxWidth: '1200px', width: '100%', pointerEvents: 'none' }} />

Build voice and chat agents that take bookings, answer billing questions, and hand off to humans when they need to. It's the same platform powering agents at some of the world's biggest brands, now open to anyone. Under the hood is [Raven](/agent-settings/raven), our own LLM, responding in under 300ms across 24+ languages.

Work the way you want: drag and drop in [Agent Studio](/get-started/introduction), live in the terminal with the [ADK](/extend/adk), or wire everything up through the [REST APIs](/api-reference/introduction). Every change lands in a [shareable test environment](/widgets/test) first, so nothing hits production until you're happy with it.
