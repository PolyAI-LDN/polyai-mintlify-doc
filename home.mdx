...
PolyAI's **Agentic Dialog Platform** is open to every builder across the enterprise. It's the same platform brands use for their most complex customer conversations — booking, billing, escalations, anything you need an agent to actually handle. Agents run on [Raven](/agent-settings/raven), PolyAI's proprietary LLM, with sub-300ms latency across 24+ languages.

Describe what you want to [Agent Builder](/studio-assistant/introduction) and it configures the agent, conversation tracks, and guardrails for you. Every agent ships with a [shareable test environment](/widgets/test) so you can validate behavior across channels before anything goes live. If you'd rather work from your terminal, the [ADK](/extend/adk) gives you self-serve API keys, a CLI, and a Git-native workflow.

Build through [Agent Studio](/get-started/introduction), the [ADK](/extend/adk), or [REST APIs](/api-reference/introduction) — same platform, same agents.
...
Tell [Agent Builder](https://studio.poly.ai/) what you need and it configures the agent for you — flows, knowledge, voice, the lot. Test it in a shareable preview, then drop it on your site. No code, no sales call.
