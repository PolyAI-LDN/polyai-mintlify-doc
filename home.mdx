...
You're here to build a voice or chat agent that can actually handle real customers — bookings, billing questions, escalations, the lot. These docs will get you there.

**What you're working with:**

- **Production-grade out of the box.** The same platform runs agents for some of the world's biggest brands. Now it's open to you.
- **Fast, multilingual conversations.** Powered by [Raven](/agent-settings/raven), our own LLM. Sub-300ms responses across 24+ languages.
- **Three ways to build.** Click together an agent in [Agent Studio](/get-started/introduction), script it from the terminal with the [ADK](/extend/adk), or drive everything programmatically through the [REST APIs](/api-reference/introduction).
- **Safe to experiment.** Every change goes to a [shareable test environment](/widgets/test) first. Nothing reaches production until you say so.

New here? Start with the [quickstart](/get-started/quickstart) and have a working agent in under 10 minutes.
