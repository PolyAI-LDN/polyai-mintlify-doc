...
<Note>
  You're in [no code mode](/platform/simplified-mode). Developer, API, and advanced docs are hidden. Click the **No code — exit** pill in the top-right corner (or visit the no code mode page) to switch back to full docs.
</Note>
...
<Tip>
  **Non-technical user?** Try [no code mode](/platform/simplified-mode) – hides developer, API, and advanced docs so you only see what's relevant for Agent Studio.
</Tip>
