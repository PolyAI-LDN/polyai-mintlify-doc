...
Build a voice or chat agent that actually finishes the call — books the table, moves the money, files the claim, calms the angry customer — instead of handing it back to a human.

This is the same platform behind agents at banks, hotels, healthcare providers, and other places where getting it wrong has real consequences. Now it's yours to build on.

**A few things to know:**

- **We optimise for resolution.** Not deflection, not "I'll connect you to an agent." The whole platform — [Raven](/agent-settings/raven), [flows](/flows/introduction), [grounded knowledge](/managed-topics/introduction) — is wired to complete the task.
- **It's fast and it speaks your customer's language.** Sub-300ms responses, 24+ languages.
- **Build it however you build.** Click around in [Agent Studio](/get-started/introduction), live in the terminal with the [ADK](/extend/adk), or wire it up through [our APIs](/api-reference/introduction). Same agent, your choice of surface.
- **Nothing ships by accident.** Every change lands in a [shareable test environment](/widgets/test) first. Break things there, not on a customer call.
...
Paste any of these into [Agent Builder](/agent-builder/introduction) to scaffold an agent you can immediately edit and test.
...
<CardGroup cols={3}>
  <Card title="Write better prompts" icon="wand-magic-sparkles" href="/agent-builder/prompting">
...
<CardGroup cols={2}>
  <Card title="Build with Agent Builder" icon="wand-magic-sparkles" href="/agent-builder/introduction">
