...
These docs will help you build a voice or chat agent that resolves real customer interactions end to end — bookings, billing, claims, escalations, and the other conversations general-purpose AI gets wrong.

**What you're working with:**

- **Built for the hardest CX problems.** PolyAI runs production agents for banks, healthcare providers, hotels, and other regulated, high-stakes industries where accuracy, compliance, and resolution are non-negotiable.
- **Resolution, not deflection.** Agents are designed to complete the task — backed by deterministic flows, grounded knowledge, and [Raven](/agent-settings/raven), our proprietary LLM, with sub-300ms responses across 24+ languages.
- **Open platform for builders.** Configure visually in [Agent Studio](/get-started/introduction), build like an engineer with the [ADK](/extend/adk), or drive everything programmatically through the [REST APIs](/api-reference/introduction). Same agent, any surface.
