...
The collaborative workflow follows the standard [CLI working pattern](/adk/reference/cli#working-pattern), with each developer working on their own branch. This matches [Branches mode](/environments-and-versions/branches-mode) in Agent Studio, where each person has a Branch cut from Live. A project in Simple mode has one Branch. Switch to Branches or Sub-branches mode under Settings on the Deployments page before the team opens more than one. Create a branch with [`poly branch create`](/adk/reference/cli#poly-branch-create), edit and push, then merge with [`poly branch merge`](/adk/reference/branch_merge) or publish from the Agent Studio UI when ready. On the new deployment system, merging a Branch into `main` publishes it to Live.
...
- a feature branch against `main` (the Live version on the new deployment system) or `sandbox`
...
- publishing to Live still goes through the platform, whether you merge a Branch into `main` from the CLI or use the Publish button in Agent Studio
