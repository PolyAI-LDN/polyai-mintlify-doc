...
`--variant` looks up the variant name in the environment you are chatting against. By default `poly chat` targets the last pushed state of your current Branch. Push `variant_attributes.yaml`, then run `poly chat --variant <name>` to test it before publishing. If you chat against `main` instead (with `--environment sandbox`), the variant names only exist there once the Branch has been merged. On the new deployment system, merging into `main` (with [`poly branch merge`](/adk/reference/branch_merge) or through Agent Studio) publishes the Branch to Live.
...
<CardGroup>
  <Card title="Functions reference" href="/adk/reference/functions">
  ...
  <Card title="Return values reference (platform)" href="https://docs.poly.ai/tools/return-values">
    All supported return shapes — `utterance`, `hangup`, combined dicts, and transition objects.
  </Card>
  <Card title="Variant management (platform)" href="https://docs.poly.ai/variant-management/introduction">
