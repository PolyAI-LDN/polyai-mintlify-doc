...
If you hit this, push your changes with [`poly push`](/adk/reference/cli#poly-push), merge the branch with [`poly branch merge`](/adk/reference/branch_merge) (or publish it in Agent Studio), then chat against `main` instead. The CLI reaches `main` through the `sandbox` environment:

~~~bash
poly chat --environment sandbox
~~~

On the new deployment system, merging a Branch into `main` publishes it to Live. Take this route only when the changes are ready for customers. Otherwise, test the Branch in Agent Studio with an in-app call or chat, which works in [Sandbox](/environments-and-versions/environments#sandbox).
