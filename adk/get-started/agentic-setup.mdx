...
description: "Create your Agent Studio account and API key from Claude Code, Cursor, or Codex by pasting one prompt. The assistant installs the PolyAI ADK and runs `poly onboard`, a non-interactive GitHub sign-in that saves your key and exports POLY_API_KEY."
---
...
Nothing sensitive passes through the assistant — the sign-in and the key stay inside the `poly onboard` command.
...
2. Install the CLI: `uv tool install --python 3.14 polyai-adk`. If `poly` isn't
   ...
   If `poly onboard` isn't recognised, run `uv tool upgrade polyai-adk`.
3. Run `poly onboard`. It prints a sign-in URL and opens it if it can. Show me the
...
3. Run [`poly onboard`](/adk/reference/cli#poly-onboard) and show you the sign-in URL.
...
- `poly onboard` not recognised — run `uv tool upgrade polyai-adk`.
...
`poly onboard` sends anonymous usage events so we can see where setup fails; the key and sign-in token are never included. Set `DO_NOT_TRACK=1` or `POLY_NO_TELEMETRY=1` to opt out.
