...
description: "Install the PolyAI ADK and run poly setup to go from an empty machine to a local Agent Studio project you can edit, push, and deploy."
---

The fastest way to get up and running is entirely from the command line. Two steps — install the ADK, then run `poly setup` — take you from an empty machine to a local project you can edit, push, and deploy.

Using an AI coding assistant? See [Set up with an AI coding assistant](/adk/get-started/agentic-setup) to hand it the whole setup with one prompt.
...
One command handles the rest of onboarding, for both self-serve and enterprise accounts:

```bash
poly setup
```

It runs four steps, skipping any that are already done — so it's safe to re-run at any time:

1. **Sign in** — opens a browser window, fetches (or creates) an API key for your user, and saves it to `~/.poly/credentials.json` so future `poly` commands pick it up automatically. This can be on any device, not just the machine running the CLI.
2. **Shell completion** — installs tab completion for bash, zsh, or fish.
3. **AI agent skills** — installs the ADK's skills into coding agents detected on your machine (Claude Code, Cursor, Codex, and others), so they know the `poly` workflow.
4. **Project** — optionally creates a new Agent Studio project or connects an existing one.

You're prompted to pick a region when signing in. Choose based on your account type, or pass it directly to skip the prompt:
...
If `poly setup` detects an existing API key (from the credential file or an environment variable), it skips straight past authentication to the remaining steps.

</Tip>

### Sign in only — `poly login`

To set up credentials without the rest of `poly setup`, `poly login` runs just the sign-in step:

```bash
poly login --region us-1   # or euw-1, uk-1, studio
```

`poly login`:

1. Opens a browser window so you can sign in.
...
`poly apikey` is a separate, non-interactive command for getting an account API key into `POLY_API_KEY` for use in your own code (the PolyAI APIs, Dialog RSN) — it's not for setting up the ADK itself, and it's the command behind [Set up with an AI coding assistant](/adk/get-started/agentic-setup).

```bash
poly apikey --region studio
```

#### Manual API key export
...
![Generating an API key in Agent Studio — API Keys tab selected, showing the + API key button in the top-right](/adk/assets/api-keys-tab.png)
...
If `poly setup` created a project for you, `cd` into the project directory. Otherwise, connect to an existing project:
...
![Quick setup button Agent Studio](/adk/assets/quick-agent-setup.png)
...
```bash
# Any account type:
poly setup --region studio       # or us-1, euw-1, uk-1 — sign in and save your API key (skip if already done)
