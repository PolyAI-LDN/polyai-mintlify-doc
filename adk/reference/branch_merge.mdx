...
<p className="lead">
Merge a branch into its parent with <code>poly branch merge</code>, including interactive and pre-defined conflict resolution. On the new deployment system, merging a Branch into <code>main</code> publishes it to Live.
</p>

`poly branch merge` is the CLI-native counterpart to merging in the Agent Studio web UI. It brings everything you've changed on the current branch back onto its parent (`main` for a Branch, or the parent Branch for a Sub-branch). It surfaces any conflicts in a structured table and lets you resolve them either interactively or from a JSON file.
...
- `main` is the Live version on projects using the new deployment system. A branch is a Branch, cut from Live. A branch created from another branch is a Sub-branch.
...
4. Merge back to `main` with `poly branch merge '<message>'`, described on this page. On the new deployment system this publishes the Branch to Live.
5. On projects that have not yet moved to the new deployment system, promote the merged state with `poly deployments promote`.

You can also do this in Agent Studio. Open a Branch and click **Publish** in the top-right corner, or open a Sub-branch and click **Merge**. The CLI command and the UI hit the same platform endpoint, so the result is identical.

## Basic usage

`poly branch merge` requires a merge message and merges the **current branch** into its parent (`main` for a Branch, or the parent Branch for a Sub-branch). Switch to the source branch first if you aren't on it.
...
- Run [`poly chat`](/adk/reference/cli#poly-chat) against `main` (which falls back to the sandbox environment) to smoke-test the merged result. On the new deployment system, merging a Branch into `main` publishes it to Live straight away, so test the Branch with `poly chat` before you merge.
- On projects that have not yet moved to the new deployment system, follow up with [`poly deployments`](/adk/reference/cli#poly-deployments) to promote the merged state to a live environment. On the new deployment system the merge has already published the Branch. If you need to undo it, [`poly deployments rollback`](/adk/reference/cli#poly-deployments-rollback) rolls Live back.
...
2. Open the Branch or Sub-branch.
3. Click **Publish** in the top-right corner for a Branch, or **Merge** for a Sub-branch.
...
<CardGroup>
  ...
  <Card title="Tooling" href="/adk/reference/tooling">
