...
**How the CLI maps to the new deployment system**


The CLI keeps its own names. On projects using the new deployment system they map as follows:

- `main` is the Live version. A branch is a Branch, cut from Live. A branch created from another branch is a Sub-branch.
- `poly branch merge` merges into the parent. Merging a Branch into `main` publishes it to Live. Merging a Sub-branch merges it into its Branch.
- `poly branch sync` is Sync. It pulls the parent's changes into your branch. Run it before merging if the parent has moved on.
- `poly branch tag` moves the current Branch to Staging (`pre-release` in the CLI). `poly branch untag` removes it.
- `poly deployments promote`, and the old sequence of promoting from sandbox to pre-release to live, apply only to projects that have not yet moved to the new deployment system.
- `poly deployments rollback` rolls Live back on new-system projects.
...
`poly apikey` exists to get an account API key into `POLY_API_KEY` for the PolyAI APIs and Dialog RSN, and is runnable by an AI coding assistant. One-shot, non-interactive setup: sign-in, creates the account if needed, reuses or creates an account-scoped API key, saves it to `~/.poly/credentials.json`, and exports `POLY_API_KEY` into the shell profile (Windows: user environment). Never prompts. `--region` is required — pass `studio` (PLG) for GitHub-only device sign-in, or any other cluster for the standard sign-in page instead. See [Set up with an AI coding assistant](/adk/get-started/agentic-setup) for the copy-paste prompt that runs this for you. To set up the ADK itself, use `poly setup`/`poly login` instead.
...
poly apikey --region studio --key-name my-key -f
...
| Flag | Description |
|---|---|
| `--region REGION` | Region/cluster to create the key for. Required — no default. Choices match the standard region list. |
...
| `-f`, `--force` | Overwrite an existing `POLY_API_KEY` if it holds a different value. |
| `--json` | Print a single JSON object on stdout. The sign-in URL goes to stderr instead of being suppressed. |

Exit codes: `0` on success, `1` on a sign-in, account, or API failure, `2` when `POLY_API_KEY` is already set to a different value and `--force` was not passed.

<Info>
**Which region should I pick?**

Pass `--region studio` (PLG) — the cluster individual developers sign up on via GitHub — unless you already have an enterprise account provisioned in a production region (`us-1`, `uk-1`, `euw-1`). Those regions require signing in through the standard email/SSO login page rather than GitHub, since the GitHub-only client only exists for `studio`.
</Info>
...
Merge the current branch into its parent via the CLI. A merge message is required. On the new deployment system, merging a Branch into `main` publishes it to Live, and merging a Sub-branch merges it into its Branch.
...
Pull the parent's changes into the current branch. A Branch's parent is `main` (Live on the new deployment system); a Sub-branch's parent is its Branch. Run it before merging if the parent has changed since you created or last synced the branch.
...
Move the current Branch to Staging (`pre-release` in the CLI) so you can test it over a real phone number or widget. Staging is available in Branches and Sub-branches modes, not in Simple mode. `main` cannot be tagged. Use `poly branch untag` to take the Branch out of Staging.
...
Creates a new branch. By default the branch is sourced from the project's `main` branch. On the new deployment system `main` is Live, so the new Branch starts as a copy of Live. On projects that have not yet moved, `main` is the sandbox environment.

| Flag | Description |
|---|---|
| `--env`, `--environment` | Source the new branch from a deployed environment snapshot instead of `main`. Choices: `sandbox`, `pre-release`, `live`. `pre-release` is the Staging environment. |
| `--force`, `-f` | Force branch creation even if there are uncommitted local changes on main. |

When `--env live` or `--env pre-release` (Staging) is specified:
...
Branching from a Live deployment snapshot will overwrite your local project with the Live state. On projects that have not yet moved to the new deployment system, merging this branch back to `main` may roll back changes that were introduced after the snapshot was taken. On the new deployment system, if Live changes after the snapshot, run `poly branch sync` before merging so those changes are kept.

</Warning>

<Info>

**Simple mode allows one Branch at a time**


In Simple mode a project has one Branch, so creating a second returns an error. Switch to Branches or Sub-branches mode under Settings on the Deployments page to open more than one.
...
Use `--push` to push the local project to Agent Studio before starting the chat session. This ensures the branch has your latest local changes before testing, without requiring a separate `poly push` step:
...
| `--environment` | Target environment. Choices: `branch`, `sandbox`, `pre-release`, `live`. Defaults to `branch`. `pre-release` is the Staging environment. `branch` chats against the last **pushed** state of your current branch (not local uncommitted changes); on main it falls back to `sandbox`. Use `--push` to push local changes before chatting. |
...
| Flag | Description |
|---|---|
| `--env` | Environment to list deployments for. Choices: `sandbox`, `pre-release`, `live`. `pre-release` is the Staging environment. Defaults to `live` on projects using the new deployment system, otherwise `sandbox`. |
...
This command applies to projects that have not yet moved to the new deployment system, where publishing a Branch replaces promotion.
...
Roll back to a previous deployment version. On projects using the new deployment system this rolls Live back. On other projects it rolls back sandbox.
...
11. optionally move the Branch to Staging with `poly branch tag` to test it over a real phone number or widget
12. merge the branch with `poly branch merge '<message>'`; on the new deployment system, merging into `main` publishes the Branch to Live
13. on projects that have not yet moved to the new deployment system, promote to pre-release or live with `poly deployments promote`
...
<CardGroup>
  ...
  <Card title="Branch merging" href="/adk/reference/branch_merge">
  ...
  <Card title="Working locally" href="/adk/concepts/working-locally">
