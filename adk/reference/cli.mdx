...
One-shot, non-interactive setup for AI coding assistants: GitHub-only device sign-in, creates the account if needed, reuses or creates an account-scoped API key named `cli-generated-key`, saves it to `~/.poly/credentials.json` (`studio`), and exports `POLY_API_KEY` into the shell profile (Windows: user environment). Never prompts. See [Set up with an AI coding assistant](/adk/get-started/agentic-setup) for the copy-paste prompt that runs this for you.
