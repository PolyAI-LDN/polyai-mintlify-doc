...
One-shot, non-interactive setup for AI coding assistants: sign-in, creates the account if needed, reuses or creates an account-scoped API key, saves it to `~/.poly/credentials.json`, and exports `POLY_API_KEY` into the shell profile (Windows: user environment). Never prompts. By default it targets `studio` (PLG) via GitHub-only device sign-in; `--region` targets any other cluster via the standard sign-in page instead. See [Set up with an AI coding assistant](/adk/get-started/agentic-setup) for the copy-paste prompt that runs this for you.
