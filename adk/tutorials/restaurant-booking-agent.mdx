...
You will use the ADK to define the agent, iterate on it, and push it for testing. You publish to Live by merging into `main`, from the CLI or in Agent Studio.
...
You are now on the `booking-flow` branch. Any changes you push will go to that branch in Agent Studio, leaving `main` untouched. On the new deployment system, `main` is the Live version, so nothing reaches customers until you merge.
...
Your changes are now on the `booking-flow` branch in Sandbox. Live is unaffected.

## Part 13 — Merge and test

By default, `poly chat` connects to your current branch's last pushed state, so run it from `booking-flow` to test the branch on its own. On `main` it falls back to the Sandbox environment. Test the branch before you merge, because on the new deployment system merging into `main` publishes the Branch to Live. When you are happy, merge `booking-flow` into `main`, then run `poly chat` against Sandbox to check the merged project.

<Note>

**Merge from the CLI or publish in Agent Studio**


Merge from the CLI with `poly branch merge '<commit message>'` (run from the `booking-flow` branch), or open the project in Agent Studio, switch to the `booking-flow` Branch, and publish it from the Publish menu at the top right. Either route publishes the Branch to Live. See the [Branch merging reference](/adk/reference/branch_merge) for the full conflict-resolution flow, including `--interactive` and `--resolutions`.
...
After the merge, switch back to `main` locally:
...
<CardGroup>
  <Card title="Flows" href="/adk/reference/flows">
    Full reference for flow configuration, step types, and conditions.
  </Card>
  <Card title="Entities" href="/adk/reference/entities">
  ...
  <Card title="Topics" href="/adk/reference/topics">
    How topics connect caller intent to agent actions.
  </Card>
  <Card title="Speech recognition" href="/adk/reference/speech_recognition">
    Keyphrase boosting, transcript corrections, and ASR settings.
  </Card>
  <Card title="Response control" href="/adk/reference/response_control">
    Pronunciations and phrase filtering for spoken output.
  </Card>
  <Card title="Variants" href="/adk/reference/variants">
