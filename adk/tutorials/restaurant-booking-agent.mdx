...
description: "Tutorial: build a working restaurant voice agent that greets callers, collects the booking details, and confirms the reservation using the PolyAI ADK."
---
...
Merge from the CLI with `poly branch merge '<commit message>'` (run from the `booking-flow` branch), or open the project in Agent Studio, switch to the `booking-flow` branch, and merge through the web UI. See the [Branch merging reference](/adk/reference/branch_merge) for the full conflict-resolution flow, including `--interactive` and `--resolutions`.
