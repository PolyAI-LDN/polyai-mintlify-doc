...
<CardGroup>
  ...
  <Card title="Agent Studio hosts and publishes">
    The generated work is pushed back into Agent Studio, where it can be reviewed and published.
  </Card>
</CardGroup>
...
| **Agent Studio** | Host, preview, review, and publish the agent |
...
`poly chat` connects to the last pushed state of your current Branch (or to the `sandbox` deployment when you are on `main`). Push first, or use `poly chat --push`.
...
Your Branch lives in Sandbox, so you can test it before you publish. Run `poly chat` on the Branch to chat against its last pushed state. Add `--environment sandbox` to chat against the `sandbox` deployment of `main` instead of your Branch.
...
Push your latest changes before chatting, because `poly chat` connects to the last pushed state. Target a specific environment with `--environment sandbox`, `--environment pre-release` (Staging), or `--environment live`.
...
### Step 10 - Publish to Live

Once the changes are pushed and validated, publish the Branch to Live. Publishing merges the Branch's changes onto Live and creates a new version you can roll back to. To try the agent on a real phone number first, move the Branch to Staging (Branches and Sub-branches modes only). See [Environments and versions](/environments-and-versions/introduction).

<Note>

**Publishing from the CLI or the Agent Studio web UI**

From the CLI, run `poly branch merge '<commit message>'` to merge the current Branch into `main`. On the new deployment system, merging into `main` publishes the Branch to Live. In the Agent Studio web UI, open the Branch and click **Publish** in the top-right corner. Test the Branch with `poly chat` before you merge. See the [Branch merging reference](/adk/reference/branch_merge) for the full conflict-resolution flow.
...
<CardGroup>
  ...
  <Card title="You review and publish">
    Agent Studio remains the place where the work is checked and published.
  </Card>
</CardGroup>
...
The Branch-based workflow makes it possible to inspect what was generated before publishing it to Live.
...
Once everything looks right, publish the Branch to Live. From the CLI, merge it into `main` with [`poly branch merge`](/adk/reference/branch_merge); on the new deployment system that publishes it. In the Agent Studio web UI, open the Branch and click **Publish** in the top-right corner.
