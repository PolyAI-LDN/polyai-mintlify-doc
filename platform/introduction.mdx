...
PolyAI's **Agentic Dialog Platform** runs production voice and chat agents for banks, hotels, healthcare providers, and other businesses where the agent is expected to resolve the conversation — bookings, billing, claims, escalations — rather than route it to a human. The same platform is available to every team building on PolyAI. Most agents run on [Raven](/agent-settings/raven), PolyAI's proprietary LLM: sub-300ms latency, 24+ languages, grounded in your knowledge.
