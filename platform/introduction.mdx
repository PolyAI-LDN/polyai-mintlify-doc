...
PolyAI's **Agentic Dialog Platform** runs production voice and chat agents for banks, hotels, healthcare providers, and other businesses where the agent is expected to resolve the conversation — bookings, billing, claims, escalations — rather than route it to a human. The same platform is available to every team building on PolyAI. Most agents run on [Raven](/agent-settings/raven), our proprietary LLM: sub-300ms latency, 24+ languages, grounded in your knowledge.

There are two main ways to start. Describe what you want in [Agent Builder](/agent-builder/introduction) and it scaffolds the flows, knowledge, and guardrails. Or work locally with the [ADK](/extend/adk) — a CLI, self-serve API keys, and a Git-based workflow. Either path gives every change a [shareable test environment](/widgets/test) so you can validate behavior and gather stakeholder review before promoting to production.
