...
To start from a description, use [Wren](/wren/introduction). It scaffolds the flows, knowledge, and guardrails from your prompt. To work locally, use the [ADK](/extend/adk): a CLI, self-serve API keys, and a Git-based workflow. Every change can go to a [shareable test environment](/widgets/test) for validation and stakeholder review before you publish it to Live.
