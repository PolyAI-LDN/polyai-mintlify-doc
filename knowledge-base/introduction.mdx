...
Well-structured entries make answers clear, behavior safe, and analytics meaningful.

## How retrieval works

The agent doesn’t see the whole KB every time. Instead, it uses **retrieval-augmented generation (RAG)** to find the best match for the user’s message.

1. The retriever compares the message to the `name`, `sample questions`, and `content`.
2. It returns the closest few topics.
3. The model selects one and generates a reply (and may trigger an `action`).

<Tip>
Make the `name` semantically close to the actual intent. Avoid vague labels like `"General_Info"` or `"Miscellaneous"` — they fire too often and confuse metrics.
</Tip>

### Simple FAQ

Used when the agent just needs to answer a question — no follow-up or action required.

### Single turn

<Icon name="message-square" /> One message in, one message out.
...
<Icon name="corner-down-right" /> Ask a clarifying question before giving the answer.
...
<Icon name="hand" /> agent replies, then offers to transfer.
...
<Icon name="arrow-right" /> Immediate transfer with no agent reply.
...
<Icon name="split" /> Transfer based on user clarification (e.g. group size, request type).
...
Used when you want to follow up with a link or info via text.

### Offer

<Icon name="phone" /> Offer to send a link by SMS.
...
<Icon name="list" /> Let the user choose which SMS to receive.

- `content`: Ask a branching question.
- `actions`: Map answers to different `sms_id`s.

**Example – Tee time booking**
![SMS_conditional-golf_booking](/images/kb-sms-conditional-golf-booking)
...
![sms-conditional](/images/kb-sms-conditional)
...
<Icon name="check-circle" /> Do this:
