...
You don't need to manually add style guidelines to your behavior prompt for voice. A **prompt decorator** is automatically applied per channel – for example, the Raven voice model includes built-in style guidelines, webchat applies a chat-formatting decorator, and conversational SMS (`sms.polyai`) applies an SMS decorator that enforces short, plain-text replies and segment limits.
