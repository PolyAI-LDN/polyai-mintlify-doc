...
- Access to edit your website's pages _or_ your tag manager (e.g., Google Tag Manager)
...
<Note>
The script tag shown below is an example. Always retrieve your actual script tag from **Agent Studio** (**Channels > Chat > Embed** tab) to ensure you use the correct unique code for your widget.
</Note>
...
After installing the widget script, you can customize the widget's position on the page using CSS. The widget appears inside a page element called `#poly-ai-chat`, which your web team can style using CSS.

Add a `<style>` block in the `<body>` section of your page to adjust the widget's position:
...
Adjust the `bottom` and `right` values to place the widget where you need it. This is especially useful for avoiding overlap with other fixed UI elements like cookie banners or navigation bars.

<Note>
Widget positioning is managed by your website team through CSS. PolyAI exposes the `#poly-ai-chat` element for this purpose — the position and layout are fully under your control.
</Note>

### Widget icon size

The widget icon uses standardized sizes based on the visitor's device:
...
Account for these dimensions when positioning the widget to ensure it doesn't overlap with other page elements.
...
2. [Adjust the widget position](/webchat/widget#widget-positioning) using CSS on the `#poly-ai-chat` element
...
- Place the widget in a **bottom corner** and keep it consistent across pages — see [widget positioning](/webchat/widget#widget-positioning) for CSS instructions
...
- Load the widget **in the background** (asynchronously) (the snippets above do this by default)
