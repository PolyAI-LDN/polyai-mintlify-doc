...
All disclaimer settings live in one place: **Agent Studio → Integrations → Widgets → Content** tab. There are two sections:
...
Some regulated markets require you to tell users plainly when they are talking to an AI. Control how your web chat discloses it's an AI, not a human. You configure everything in Agent Studio, and changes apply to your widget without a code deploy.

Both settings live in one place: **Agent Studio → Widget Configuration → Styling**.

### In-conversation AI label

The **AI label** dropdown chooses the tag shown next to the agent's name above every response, for example **Polly (Virtual Agent)**. Options:

- **Virtual Agent** (default)
- **AI Assistant**
- **AI**
- **Hide**: removes the in-conversation label entirely

<Info>
  Selecting **Hide** automatically turns on the **Powered by PolyAI** attribution below, so your widget always carries some form of AI disclosure.
</Info>

Pick the wording your legal team has approved for the markets you serve. The selected label renders inline with the agent's name on every response, so disclosure stays front and centre for users who need it.

## Powered by PolyAI attribution

The **Powered by PolyAI attribution** toggle adds a subtle "Powered by PolyAI" label under the widget window, footer-style, similar to the Web Calling widget.

- **On**: the attribution renders under the widget window.
- **Off** (default): no footer attribution.

Use this where you want a lighter-touch disclosure than a per-message label, but be aware this label alone does not meet the transparency requirements of some regions so you might wan tto apply it with an active opt in consent, or a persistant in widget disclaimer.

## Choosing the right setup

There's no single correct configuration; it depends on your regulatory environment and how visible you want AI disclosure to be. Keeping the AI label visible on every response is the clearest, most defensible form of disclosure and suits deployments where regulation requires it explicitly.

<Note>
  These settings help you meet AI disclosure requirements, but what you are required to display depends on your industry and the regions you operate in. Check with your legal or compliance team before choosing your setup.
</Note>

### Clean, minimal chat surfaces

| Setting | Recommendation |
| --- | --- |
| AI label | Hide |
| Powered by PolyAI attribution | On (enforced automatically) |

Use this where you want the conversation to read as clean as possible without a label on every message. Disclosure isn't lost: hiding the AI label automatically enforces the footer attribution on.

## Good to know

- **Changes preview live.** Edits in Widget Configuration → Styling appear in the on-page preview immediately, before you save and publish.
- **No code deploy needed.** Published changes reach your widget without any engineering work on your side.
- **Settings are audit-logged.** Changes to your AI label and attribution configuration are recorded, so your compliance team can evidence what was displayed and when.
- **Per-project, per-variant.** Configure independently for each widget variant you run.

## Related

<Columns cols={2}>
  <Card title="Disclaimers and privacy policies" icon="file-text" href="/messaging-channel/web-chat/disclaimers-and-privacy-policies">
    Configure your legal disclosures, consent and privacy policy links.
  </Card>

  <Card title="Chat configuration" icon="sliders" href="/messaging-channel/web-chat/chat-configuration">
...
This is written to stand as its own page (like disclaimers does), so once it's published you can drop the real URL into the release note's Links section in place of the placeholder.

### Powered by PolyAI footer

The **Powered by PolyAI** toggle renders a subtle attribution under the widget window, footer-style. It is off by default and works as a lighter-touch alternative to the per-message label when a cleaner conversation surface matters more than an inline tag.

<Note>
  Disclosure cannot be switched off entirely. Choosing **Hide** on the in-conversation label automatically turns the **Powered by PolyAI** footer on and locks it, so users always see at least one form of AI disclosure.
</Note>

### Where the settings apply

- **Per project, per variant.** Configure different disclosure for different variants of the same agent, for example one variant per market.
- **Live in the widget preview.** Changes update the preview immediately with no save or reload.
- **No code deploy.** Published changes propagate to the widget on your site without any engineering work.
- **Audit-logged.** Every change to the label or footer is recorded, so your compliance team can evidence what was displayed and when.
...
For full details on configuring languages for your widget, see [Web chat localisation](/messaging-channel/web-chat/localisation).
...
<Columns cols={2}>
  <Card title="Web chat localisation" icon="globe" href="/messaging-channel/web-chat/localisation">
