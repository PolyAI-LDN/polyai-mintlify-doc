...
Configure which LLM powers each channel in **Channels > [Voice configuration](/voice-channel/advanced/call-settings)** or **Channels > [Chat configuration](/messaging-channel/advanced/chat-configuration)**. For multichannel agents, **[Raven 3.5](/behavior/models/raven)** is the recommended model – it is the only Raven model tuned for both voice and chat, providing a consistent experience across channels.
...
<CardGroup cols={3}>
  ...
  <Card title="Voice settings" icon="microphone" href="/voice-channel/introduction">
