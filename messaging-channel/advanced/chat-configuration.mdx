...
- **Changes apply on save.** Messaging configuration takes effect immediately when saved, unlike agent changes, which reach customers only when you publish to Live.
- **Markdown is supported.** Chat messages render bold, italics, lists, links and code blocks across testing and review panels.

## Branching behavior

Chat configuration changes take effect immediately when saved. Unlike other project settings, webchat does not follow the Branch and publish system. There is no Sandbox, Staging or Live step. Use the Test Agent Chat panel to verify changes before saving.
