...
Use the Numbers page to add, delete, and assign phone numbers across your agent's environments. Numbers are grouped by environment – Staging and Live – so you can manage test and production numbers separately. Sandbox has no phone numbers because it holds many versions at once. Test in Sandbox with an in-app call or [simulation tests](/testing/simulation-tests).
...
- **Staging** – numbers for testing a Branch the way a customer would, before you publish (Branches and Sub-branches modes only)
- **Live** – numbers serving production traffic

If a Sandbox section still appears, it lists numbers left over from the old deployment system. Remove any that remain.
...
Variants are available in an environment once a version that contains them is running there. Publish to Live, or move the Branch to Staging. If no variants are available, publish first.
...
- **Unpublished variants** – your project now uses variants; publish so you can assign them
...
<CardGroup cols={2}>
  <Card title="Buy a number" icon="cart-shopping" href="/voice-channel/numbers/how-to-buy-number">
  ...
  <Card title="Outbound calling" icon="phone-arrow-up-right" href="/voice-channel/numbers/outbound-calling">
