...
Use speech recognition settings when your agent consistently mishears specific words — brand names, product codes, medical terms, or domain-specific vocabulary. Poor transcription causes wrong tool calls, failed lookups, and incorrect agent responses.

This page provides two tools for improving **ASR accuracy**:
...
1. Navigate to **Voice > Speech recognition**.
...
5. Add one or more regex rules within the correction:
   - **Replacement type**: **Full transcript** (matches the entire transcript as a literal string) or **Partial transcript** (matches a substring using regex).
...
<Tip>
**Use Partial transcript for regex patterns.** Full transcript mode performs a literal string match against the entire transcript — regex syntax is not evaluated. If you need regex features like character classes, alternation, or word boundaries, select **Partial transcript**. You can still anchor a partial transcript pattern to the full string using `^` and `$` if needed.
</Tip>
