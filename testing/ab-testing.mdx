...
- A Branch that you want to test against it (this becomes the variant). It must be synced with Live and in Staging, so A/B testing needs Branches or Sub-branches mode. Create the Branch with the standard [Branches flow](/environments-and-versions/branches-mode#making-a-change), then [move it to Staging](/environments-and-versions/environments#moving-a-branch-to-staging).
...
2. Open the **Publish** menu (top-right) and select **Publish as an A/B test**. The option is greyed out until the Branch is synced with Live and in Staging.
...
- You can keep working in other Branches and test them in Sandbox. Moving a Branch to or from Staging is blocked while the test runs, as is publishing to Live.
...
- The variant must be a Branch that is synced with Live and in Staging.
