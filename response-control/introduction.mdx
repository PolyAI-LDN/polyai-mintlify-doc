...
Define keywords and phrases to block or monitor in agent responses. This page covers two features:

- **[Stop keywords](#stop-keywords)** — Block or log specific phrases in agent output (content moderation)
- **[Pronunciations](#pronunciations)** — Control how your agent pronounces specific terms (voice tuning)
...
When a response matches a stop keyword pattern, the system halts the response and logs the occurrence.

## Getting started

### Accessing Response Controls settings

1. **Open Response Controls**: Go to **Channels > Voice > Response control** in the sidebar.
2. **Create a new phrase**:
   ...
   - Fill out the required fields:
     ...
     - **Regular Expression**: Enter a regex pattern to define the keyword or phrase.
      - **Say Phrases**: Toggle to control matching phrase behavior. When **TRUE** (active), matching output is halted — the agent will not say the phrase. When **FALSE**, matching output is allowed through but logged for analytics.
...
- **Say Phrases (Boolean)**: When TRUE, matching phrases are **blocked** (the agent will not say them). When FALSE, matching phrases are allowed through but logged for analytics.

<Note>
The name "Say Phrases" can be counterintuitive: setting it to **TRUE** means the control is *active* and will **prevent** the phrase from being spoken. Setting it to **FALSE** allows the phrase through.
</Note>
...
#### Steps to set up:
1. Go to **Channels > Voice > Response control** in Agent Studio.
...
![response-order](/images/response-order.png)
...
- **[Managed Topics actions](/managed-topics/how-to-setup-action/introduction)**
