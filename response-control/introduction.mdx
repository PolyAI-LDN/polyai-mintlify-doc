...
Define keywords and phrases to block or monitor in agent responses. Configure stop keywords and pronunciation rules.
...
2. **Create a new phrase**:
   ...
   - Fill out the required fields:
     ...
     - **Regular Expression**: Enter a text pattern (regular expression) to match the keyword or phrase.
...
- **Description**: Optional text to clarify the keyword's purpose.
- **Regular Expression**: Regex pattern that identifies the phrases or words to control.
- **Say Phrases**: Specifies whether matching phrases halt responses (TRUE) or are logged for analysis (FALSE).
...
You can use regular expressions (text patterns) for more precise control over which phrases are matched. For a detailed guide on regex, refer to **[Regex documentation](https://regex101.com/)**.
...
AI models often add unnecessary introductions before carrying out an action, which can slow down response times and, more importantly, annoy users. By applying stop keywords, you can ensure that agents immediately proceed with relevant information without unnecessary preamble or repetition.
