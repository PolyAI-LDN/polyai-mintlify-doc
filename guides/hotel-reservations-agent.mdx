...
Go to **Configure > Secrets** and add your OpenTable `client_id` and `client_secret`. Reference them in your function with `conv.utils.get_secret()`:

```python
opentable_creds = conv.utils.get_secret("opentable_credentials")
client_id = opentable_creds["client_id"]
client_secret = opentable_creds["client_secret"]
```
...
Here is what a successful conversation looks like once everything is wired up:

```text
Agent: Good afternoon, thank you for calling The Crown Hotel. How can I help?
Caller: Hi, I'd like to book a table for dinner tonight.
Agent: I can help with that. How many guests will be dining?
Caller: Four of us.
Agent: And what time were you thinking?
Caller: Around 7:30.
Agent: Let me check... I'm booking a table for 4 tonight at 7:30 PM.
       [make_reservation function executes — OpenTable API returns success]
Agent: You're all set — table for 4 tonight at 7:30. Is there anything 
...
1. `restaurant_booking_request` topic matched → triggered `restaurant_reservation` flow
2. Flow collected party size (4), date (today), time (7:30 PM) across three steps
3. Confirmation step read back details → caller confirmed
4. `make_reservation` function called OpenTable API → returned success
5. Agent delivered confirmation and exited the flow
