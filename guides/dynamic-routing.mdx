...
| Conditional handoff | `transfer_call` function | [Call handoff](/call-handoff/introduction) |
...
def route_caller(conv: Conversation):
    ...
    if response.status_code >= 500:
        ...
        if config.get("fallback_destination"):
            return {
                ...
                "transfer_call": {"destination": config["fallback_destination"]}
            }
        return
    ...
    if caller.get("tier") == "vip" and config.get("vip_routing_enabled", True):
        return {
            ...
            "transfer_call": {"destination": "VIP_SUPPORT", "reason": "VIP_CALLER"}
        }
