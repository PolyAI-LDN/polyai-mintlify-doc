...
The model is configured **per channel**, not globally. Select the model for each channel separately.

![llm-use](/images/model.png)

<Steps>
  <Step title="Open model settings">
    Navigate to **Channels > Voice > [Voice configuration](/voice/voice-configuration)** or **Channels > Chat > [Chat configuration](/webchat/chat-configuration)**.
  </Step>
