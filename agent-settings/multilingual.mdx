...
<AccordionGroup>
  <Accordion title="Full list of supported languages and codes">
...
Voice (TTS) coverage depends on the provider you select for each language. Some languages support fewer voices – see the [Voice Library](/voice/voice-library) for what's available per language.
...
## Controlling automatic language switching

A multilingual agent can switch the conversation language on its own when it detects that the caller is speaking or writing in another configured language. The agent only ever switches into a language you've added under **Configure > General**.

You can choose how aggressive that automatic switching is:

- **Permissive (default)** – the agent switches when the caller explicitly asks (for example, "can we speak in French?") and also when the caller clearly speaks or writes in another configured language without asking.
- **Strict** – the agent switches **only** when the caller explicitly asks to change language. Speaking or writing in another language without asking is treated as an ordinary message, and the agent keeps responding in the current language.

Use strict mode if you see false language switches caused by ASR mishears – for example, a short reply like "si" misheard as a different language, or muffled audio that the speech model transcribes into a foreign script. Strict mode prevents these from flipping the conversation language.

<Note>
Strict mode is configured per project on the PolyAI platform. Contact your PolyAI representative to enable it for a project.
</Note>

In both modes:

- The agent can only switch into languages configured under **Configure > General > Additional languages** (plus the main language). Requests to switch to an unconfigured language are rejected.
- You can still force a specific language up front using [DNIS routing](#forcing-a-language-by-dialled-number-dnis-routing) or by calling [`conv.set_language()`](/tools/classes/conv-object#set_language) from a function.
...
- **Filter by language** – open **Filter** and add a `Language is …` condition to narrow the table to a specific language. The condition shows as a chip above the table and can be saved into a [Custom View](/analytics/conversations/views).
  ...
  ![Conversations table filtered to French, showing the Language is French chip above the table](/images/analytics/conv-review-filter-chip-language.png)
  ...
  ![Filter builder side panel showing an Environment condition combined with a Language is French condition](/images/analytics/conv-review-filter-builder-language.png)
