...
Multilingual projects automatically track every language switch as a `CONVERSATION_LANGUAGE_CHANGED` metric. You don't need to declare this metric under **Configure > Metrics** – it is registered for you as soon as a project has one or more additional languages configured.

Each time the conversation language changes – whether through automatic detection or a call to [`conv.set_language()`](/tools/classes/conv-object#set_language) – the new IETF locale code (for example `"es-ES"` or `"fr-FR"`) is recorded as the metric value. The metric can change multiple times in a single conversation, so use it to:

- Filter [Conversations](/analytics/conversations/review) for calls where the caller switched languages mid-flow.
- Build [Dashboards](/analytics/dashboards/introduction) that segment volume or outcomes by final response language.
- Ask [Smart Analyst](/smart-analyst/introduction) questions such as *"What proportion of Spanish callers ended the conversation in English?"*

<Note>
If you have manually declared `CONVERSATION_LANGUAGE_CHANGED` in your metrics config – for example to restrict it to a specific set of expected values – your configuration is preserved and is not overwritten by the automatic declaration.
</Note>
