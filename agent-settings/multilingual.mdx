...
Use the full IETF locale code (e.g. `"es-ES"`, `"en-US"`, `"fr-FR"`), not the short form (`"es"`, `"en"`). See [`conv.set_language`](/tools/classes/conv-object#set-language) and [`conv.callee_number`](/tools/classes/conv-object#callee-number) for the full reference.
