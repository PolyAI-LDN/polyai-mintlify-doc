---
title: 'Raven'
description: "PolyAI's proprietary LLM family, built for customer service across voice and chat."
keywords:
  - raven
  - LLM
  - language model
...
Raven is PolyAI's proprietary LLM, purpose-built for real-time customer conversations. It delivers sub-300ms latency, stronger grounding, and more natural responses than general-purpose models, across 24+ languages. Raven runs the majority of PolyAI deployments.

Select it in [Voice configuration](/voice/voice-configuration) or [Chat configuration](/webchat/chat-configuration), or compare it with other models on the [Model](/agent-settings/model-use) page.

## Why Raven

General-purpose models (GPT, Claude) are trained for broad text tasks. They work for customer service, but require heavy prompting to behave reliably and are slower than a voice channel can afford.

Raven is built specifically for customer conversations:

* **Fast enough for live voice.** Sub-300ms latency keeps the conversation flowing without awkward pauses.
* **Trained on phone conversations.** Handles interruptions, mispronunciations, and partial utterances closer to how a person would.
* **Conversational behavior built in.** Staying on topic, grounding answers in your knowledge, and asking for clarification when unsure are baseline behaviors — not things you have to prompt.
