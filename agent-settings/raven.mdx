...
Raven is the engine behind PolyAI's voice agents — a proprietary LLM family purpose-built for real-time customer conversations. It delivers sub-300ms latency, stronger grounding, and more natural responses than general-purpose LLMs, across 24+ languages.

Raven powers the majority of PolyAI deployments across hospitality, healthcare, financial services, and more. You can select it in [Voice configuration](/voice/voice-configuration) and [Chat configuration](/webchat/chat-configuration), or compare it with other options on the [Model](/agent-settings/model-use) page.
...
Most models are trained to be good at everything - writing code, creative writing, answering questions on any topic, even generating images. It means:
...
**Stronger date and time logic** — Raven handles relative dates, scheduling scenarios, and format conversions that trip up general-purpose models.

**Reliable function calling** — Trained on real Agent Studio projects, Raven calls functions with the right parameters and doesn't confuse when to respond vs. when to act.
