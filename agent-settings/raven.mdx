...
Raven is the LLM we built ourselves, for one job: talking to your customers. Sub-300ms, 24+ languages, grounded in your knowledge instead of making things up. It runs most agents on the platform.

Pick it in [Voice configuration](/voice/voice-configuration) or [Chat configuration](/webchat/chat-configuration). If you want to compare it side-by-side with GPT, Claude, and the rest, see the [Model](/agent-settings/model-use) page.

## Why Raven

GPT and Claude are brilliant at a thousand things. Customer service is one of them — but you'll spend a lot of prompts coaxing it out, and on the phone they're slower than they need to be.

Raven only does customer service. So:

* It's fast enough for live phone calls (sub-300ms — you don't notice it thinking)
* It was trained on phone conversations, not chat threads, so it handles interruptions, mumbling, and "sorry, what?" the way a person would
* The behavior you want — staying on topic, grounding answers, asking back when it's not sure — is baked in rather than prompted in
