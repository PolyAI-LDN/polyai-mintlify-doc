...
- **Out-of-domain detection** – identifies when a request falls outside the agent's scope, enabling cleaner handoffs and knowledge gap tracking. When Raven flags a turn as out-of-domain, the platform writes a `RAVEN_OOD` boolean metric to the conversation so you can surface knowledge gaps in [Dashboards](/analytics/dashboards/introduction) and the [Conversations API](/api-reference/conversations/introduction) without adding any function logic
