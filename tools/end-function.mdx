...
The **End function** runs at the end of a conversation for final data processing, cleanup, and integration tasks. Because it runs asynchronously after the call has ended, you can perform complex operations–API calls, summaries, CRM writes–without affecting the caller experience.
...
<Warning>
**Critical:** Errors in the end function do not surface to the caller (the call is already over), but they can silently break downstream workflows. Always implement thorough error handling with try/except blocks and external logging, as shown in the example above. Test end function changes in Sandbox before deploying to production.
</Warning>
