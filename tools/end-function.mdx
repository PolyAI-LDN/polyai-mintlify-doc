...
<Warning>
**Critical:** Errors in the end function do not surface to the caller (the call is already over), but they can silently break downstream workflows. Always implement robust error handling with try/except blocks and external logging, as shown in the example above. Test end function changes in Sandbox before deploying to production.
</Warning>
