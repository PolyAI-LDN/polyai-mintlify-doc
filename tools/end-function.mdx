...
Below is a Python implementation of the **End function**. End functions use `@func_description` but do not accept LLM parameters, so `@func_parameter` is not needed.

```python
@func_description("Logs call summary, creates support tickets, and schedules follow-ups")
...
<CardGroup cols={3}>
  ...
  <Card title="Surveys (CSAT)" icon="star-half-stroke" href="/analytics/csat/introduction">
    Trigger post-call satisfaction surveys from the end function.
  </Card>
  <Card title="conv object" icon="code" href="/tools/classes/conv-object">
    Access conv.state and other data in the end function.
  </Card>
</CardGroup>
