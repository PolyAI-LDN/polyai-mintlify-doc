...
Controls how the `utterance` is delivered. Defaults to `"SPEECH"`, which speaks the utterance through TTS. Set to `"DTMF"` to transmit the utterance as DTMF keypad tones instead of speech – useful when the agent needs to send digits to an IVR or other automated system during [outbound calling](/telephony/outbound-calling).
