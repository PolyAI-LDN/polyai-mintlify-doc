...
Switches the conversation to a different variant. The value must match an existing variant name configured in [variant management](/knowledge/variants/introduction). Use this to route callers to location-specific or segment-specific conversation flows mid-call.
