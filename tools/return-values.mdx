...
##### Locale-specific utterances

For [multilingual agents](/agent-settings/multilingual), you can return a dictionary of locale-keyed strings instead of a single string. The agent selects the utterance matching the caller's current language:
...
Use [BCP 47 language tags](https://en.wikipedia.org/wiki/IETF_language_tag) as keys (e.g., `"en-US"`, `"es-US"`, `"fr-FR"`). This approach is useful when you need deterministic, pre-translated responses without branching on `conv.language` manually.

<Tip>
Locale-keyed utterances work anywhere `utterance` is accepted — including in combination with `handoff`, `hangup`, `listen`, and `variant`.
</Tip>
