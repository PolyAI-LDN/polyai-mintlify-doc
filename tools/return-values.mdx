...
For [multilingual agents](/agent-settings/multilingual), you can manage locale-specific utterances using one of two approaches:

**Using `conv.translations` (recommended):** Define translations in the Translation page, then reference them in functions:
...
The translation resolves automatically to the caller's current language. See [multi-language configuration](/agent-settings/multilingual) for setup details.

**Using a custom `Utterances` helper:** For projects that need centralized utterance management in code, create an enum mapping locale codes to strings:
...
Then resolve the utterance before returning it:

```python
return {
  "utterance": Utterances.CALLING_YOU_NOW(conv)
}
```

<Note>
The `utterance` return field expects a string. Always resolve locale-specific dictionaries to a single string before returning — don't pass the dictionary directly.
</Note>
