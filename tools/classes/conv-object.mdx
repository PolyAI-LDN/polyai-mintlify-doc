...
<AccordionGroup>
  ...
  <Accordion title="send_whatsapp">
    **Description**: Sends a WhatsApp message using the configured Twilio WhatsApp Business API integration. The phone number must be WhatsApp-enabled and approved by Meta.
    ...
    <Note>This is not a native PolyAI channel. It requires a configured Twilio WhatsApp Business API integration. Contact your PolyAI representative for setup.</Note>
  ...
  <Accordion title="send_email">
    **Description**: Send an email from the function.
    ...
    <Note>This function sends outbound emails from within a conversation (e.g., confirmations or notifications). Email is not a supported inbound channel for PolyAI agents.</Note>
