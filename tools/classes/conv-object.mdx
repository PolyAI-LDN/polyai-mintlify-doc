...
<AccordionGroup>
  ...
  <Accordion title="set_background_track">
    **Description**: Start playing an audio recording in the background of the call immediately, with precise timing tied to your function logic. Use this when you want background audio (such as hold music or a ringtone) to begin at an exact moment — for example, right after a disclaimer finishes — instead of relying on the static `delay_seconds` from the project-level background track config.

    Calling this from a function overrides any background track set in the project configuration for the rest of the call.

    **Parameters**:
    - `name` (str) – name of the audio recording to play. The file must already be uploaded under `voice/recordings/{name}.ulaw`.
    - `loudness` (int or float, optional) – playback volume in dB, between `-70` and `-20`. Defaults to `-40` (quieter values are softer).

    **Example**:
    ```python
    # Play hold music while looking up a customer's order
    conv.set_background_track("hold_music", loudness=-30)
    order = conv.api.orders.lookup(order_id=conv.state["order_id"])
    ```

    Available on voice channels only. Calling it on chat or webchat channels has no effect.
  </Accordion>
