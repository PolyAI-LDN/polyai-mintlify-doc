...
```python
@func_description("Checks if the caller has a previously saved cheese preference")
...
```python
@func_description("Books the selected cheese type")
...
```python
@func_description("Greets returning callers with their previous booking details")
def start_function(conv: Conversation):
