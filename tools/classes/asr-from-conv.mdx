...
description: Set and clear speech recognition biasing at runtime using conv.set_asr_biasing() for dynamic vocabulary.
tag: "Code"
---
...
- flow-level ASR settings (applied to every step in a flow)
- language-specific ASR settings (applied when the conversation is in a specific language)

Lower-priority layers are merged in order: language → flow → step → dynamic. Phrases set at a higher-priority layer override any matching phrase from a lower layer.

If the same phrase appears in multiple places, the value set by this type of biasing takes precedence.

<Note>
Flow-level and language-level ASR biasing are not self-serve in the Studio editor – contact your PolyAI representative to configure them. In the editor, apply biasing across a flow by either using global biasing (applies everywhere) or per-step biasing on each step within the flow. For runtime/dynamic biasing that persists across turns, use `conv.set_asr_biasing()` as described on this page.
</Note>
