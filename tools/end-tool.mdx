...
The end tool lives alongside your other functions in **Build > Tools**. It is not a separate page – the platform recognizes any function named `end_function` as the end tool and runs it automatically after every call.

To create or edit the end tool:

1. From your agent's main page, open **Build > Tools** in the sidebar.
2. If an `end_function` already exists, click it in the list to edit. Otherwise, click **Add Function**.
3. Name the function exactly `end_function` (lowercase, underscore). The platform identifies it by this name – no other registration step is required.
4. Write your Python in the **Function Definition** field. Leave **LLM Parameters** empty – the end tool is invoked by the runtime, not the LLM, and receives only `conv`.
5. Click **Save**. The function runs at the end of every conversation in that environment from the next call onwards.

See [Create tool](/tools/how-to-setup) for the full function editor walkthrough.
...
For LLM-based extraction of categorized reasons, sentiment, and other transcript-wide fields, see [Post-call reporting](/settings/post-call-reporting).
