...
`end_function` runs on every completed call. If it writes to external systems (CRM updates, ticket creation, data exports), errors or bugs can silently corrupt downstream data across all calls. Test changes in Sandbox before publishing.
...
![End tool card highlighted in the Start and end tool calls section](/images/tools/end-tool-location.png)
...
4. Save your changes. The function runs at the end of every conversation on that Branch from the next call onwards. It runs on Live once you publish.

![end_function editor view](/images/tools/end-tool-editor.png)
...
<Warning>
Errors in `end_function` can't surface to the caller but can silently break downstream workflows. Wrap logic in try/except, log externally, and test in Sandbox before publishing to Live.
</Warning>
