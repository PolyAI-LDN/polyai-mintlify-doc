...
<Warning>
Returning an `utterance` from `start_function` overrides the configured channel greeting (set in **Channels > Voice > Voice configuration** for voice, or the equivalent Chat configuration for webchat). If you use both, test carefully to ensure the correct greeting plays.
</Warning>
