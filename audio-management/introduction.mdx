...
### Limit barge-ins per call

Use **Max barge-ins** to cap how many times a caller can interrupt the agent within a single call. Once the limit is reached, barge-in is automatically disabled for the rest of the call, even if it remains enabled in the global, flow, or step config.

Set the limit in **Channels > Voice > Audio Management** under **Max barge-ins**. Leave it empty for no limit.

**When to set a limit:**
- Callers in noisy environments trigger repeated false barge-ins, derailing the conversation
- You want the agent to deliver critical information uninterrupted after a few interruptions
- You are tuning a Turbo-mode agent and want a safety net against runaway interruption loops

**How the count works:**
- Each turn where the caller interrupts the agent counts as one barge-in
- The count is per call and resets when a new call starts
- The current turn's interruption is included in the count before the next turn is evaluated

<Note>
Only turns flagged as barge-ins count toward the limit. Normal turn-taking, where the caller waits for the agent to finish, is not counted.
</Note>
