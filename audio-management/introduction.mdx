...
To clear several cached audios in one action — for example, after a voice change or major prompt rewrite — select the entries you want to remove and choose **Delete selected**. PolyAI processes each deletion independently and reports the result per entry, so a single problem entry does not block the others.

- Entries that delete successfully disappear from the list immediately.
- Entries that fail remain in the cache, and the UI surfaces the reason (for example, the entry no longer exists or could not be removed from storage). Retry the failed entries or refresh the page.

<Tip>
Bulk delete only removes audio from the cache. The next time the agent needs the phrase, it will regenerate it via TTS and re-cache it if the [caching threshold](#getting-started) is met.
</Tip>
