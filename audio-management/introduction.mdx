...
You can also manage the audio cache programmatically with the [Audio Cache API](/api-reference/agents/endpoint/audio-cache/list-audio-cache-entries). The API mirrors the in-app tab: list entries, download or replace WAV files, edit voice tuning settings, preview synthesis, and delete entries (including in bulk). Use it to script imports across agents or to keep your cache in sync with an external content source.
