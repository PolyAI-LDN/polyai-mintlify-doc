...
<Note>
  Audio cache endpoints are gated by the `audio_cache` resource permission on the API key. `read` is required to list, download, or synthesize previews; `write` is required to delete and bulk-delete; `patch` is required to replace an audio file. Entry IDs are numeric — requests with non-numeric IDs return `400 Invalid ID`. Audio files are limited to 6 MB.
</Note>
