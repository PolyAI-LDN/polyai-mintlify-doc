...
You can also manage the audio cache programmatically with the [Agents API](/api-reference/agents/introduction). The audio cache endpoints let you list cached entries, download or replace audio files, synthesize previews, and bulk-delete entries — useful for CI workflows, scripted refreshes, or migrating audio between agents.
