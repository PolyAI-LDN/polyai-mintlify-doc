...
PolyAI's integration with HotSOS connects virtual agents to the HotSOS system, allowing guest requests and maintenance issues to be logged directly into HotSOS in real-time. This reduces manual data entry and ensures tasks are routed efficiently to the appropriate teams.
...
<Warning>The integration currently only supports service orders from the [standardized list](#standardized-service-orders).</Warning>
...
3. **Verify room number detection:** Confirm whether your phone system supports automatic room number detection for calls made from guest rooms. If not, enable manual entry via the phone keypad as a fallback.
...
- Logs guest requests into HotSOS using predefined issue codes.
- Routes tasks to the appropriate teams without manual intervention.

### Room number detection
- **Automatic detection:** Retrieves the guest's room number for calls made from in-room phones.
- **Manual input fallback:** Guests can enter their room number using the phone keypad if automatic detection isn't available.

### Multi-item handling
- Handles up to **4 units** of a single item in one request.
- Supports up to **3 distinct item types** in a single request (e.g., “Two towels, shampoo, and a toothbrush”).

### Real-time integration
- Automatically syncs issue codes, room numbers, and quantities with HotSOS.
...
- **Access restriction:** Requests must come from in-room phones to ensure guest verification and security.
