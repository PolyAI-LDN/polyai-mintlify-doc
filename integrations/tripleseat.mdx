...
PolyAI acts as a conversational front end for your Tripleseat-powered locations (venues). When a caller or chat user asks about private dining, events, or large party bookings, the PolyAI agent:
...
<Steps>
  ...
  <Step title="Provide location and lead source details">
    ...
    - **Site ID** and **location ID** for each venue (location)
    - The **lead source** you want used for PolyAI-created leads (e.g., "Phone" or a custom source)

    <Note>If you have multiple venues under one site, each venue requires its own location ID mapping.</Note>
  </Step>
...
- **Location** - An individual venue (e.g., a specific restaurant or event space) where leads are recorded. Each location has its own **location ID** and belongs to a parent **site** (identified by a **site ID**).
- **Lead** - A booking inquiry created by PolyAI

<Note>
In Tripleseat, **venue** and **location** mean the same thing. Each venue (location) has a unique **location ID** and is grouped under a **site** with a **site ID**. When configuring the integration, you need both IDs for each venue.
</Note>
