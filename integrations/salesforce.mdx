...
Connect Salesforce directly from the **Integrations** page in Agent Studio using OAuth — no manual API setup required.
...
4. Click **Connect new account** and authenticate with Salesforce via OAuth
5. Enable Salesforce actions in your agent's prompt

### Available actions

| Action | Description |
|--------|-------------|
| **Get Records** | Retrieve Salesforce records such as contacts, accounts, opportunities, or cases using search criteria |
| **Update Record** | Update an existing Salesforce record such as a contact, account, opportunity, or case |

### Enabling actions

Reference Salesforce actions in your agent's prompts — either enable all actions or select specific ones. See the [Zoom integration](/integrations/zoom#enabling-zoom-actions) for detailed examples of this pattern.

---

## Manual setup (API-based)

For advanced use cases or custom API calls, you can integrate Salesforce manually.
...
Once you provide PolyAI with the credentials above, we'll configure the integration and contact you when it's ready.
