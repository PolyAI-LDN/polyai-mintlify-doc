...
<Info>**Generally available 19 June 2026.** The setup steps below are accurate; the **Webex Messaging** tile appears in **Configure > Integrations** when the integration ships.</Info>
...
1. In the Webex Control Hub, set up the integration under **Contact Center > Bring Your Own Channel**.
2. Add the channel by clicking the **PolyAI** button that appears and following the in-product configuration page – Webex will display the integration details PolyAI needs.
3. Configure routing and skills the way you'd handle any other Webex digital channel. The simplest setup routes the new conversation to a queue that has live agents subscribed.
4. Record the values Webex generates – you'll paste them into Studio. Typical fields include an **Organization ID**, a **Channel ID**, and an **API secret**.
