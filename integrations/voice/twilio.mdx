...
incoming customer calls to PolyA's voice agents using the Twilio telephony system.

If a [handoff](call-handoff/introduction) to a live agent is needed, the integration transfers the call
and accompanying data to Twilio Flex for human handling. All interactions are
managed through [SIP](https://www.twilio.com/docs/voice/sip) or a no-code [Twilio Studio](https://www.twilio.com/docs/studio) workflow.
...
PolyAI is live on the [Twilio Marketplace](https://www.twilio.com/marketplace) using a [media stream](https://www.twilio.com/docs/voice/media-streams)
...
- **Live-agent handoffs**: The PolyAI virtual agent passes contextual data from PolyAI to Twilio Flex, to be displayed in the [Flex UI](https://www.twilio.com/docs/flex/developer/ui).
- **Routing**: Supports Twilio Studio flows and programmable routing using TwiML syntax.
- **Metadata exchange**: Use SIP headers and APIs to share contextual information and accurately route tasks.
