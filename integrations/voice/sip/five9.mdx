...
<Tabs>
  <Tab title="Authentication token">
    PolyAI provides an authentication token included in the SIP INVITE message under the X-header (`X-PolyAi-Auth-Token`). This token identifies and distinguishes traffic for accurate assistant routing.
  </Tab>
  <Tab title="Dialed extension">
    Five9 and PolyAI agree on unique extension numbers for routing calls. This method offers flexibility and scalability, especially for clients managing multiple PolyAI assistants.
  </Tab>
</Tabs>
