...
- Save the handoff info into state to have the [`Get Handoff`](/api-reference/handoff/endpoint/get-handoff) API to show them when invoked.

## Store attributes
Attributes received from the API can **only** be accessed in the `start_function`.
Attributes are only available in the `start_function`. If not stored here, they cannot be accessed later in the conversation.
...
## Save handoff info
With this integration, the PolyAI agent always terminates the call (rather than performing a direct SIP transfer).
For this reason, handoff information must be configured in other ways — specifically, 
prepared to be exposed by the Handoff API.
