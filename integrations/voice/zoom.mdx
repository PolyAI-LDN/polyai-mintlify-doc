...
<Steps>
  ...
  <Step title="Create connections">
    Under the installed app, click the number shown under **Connections** (defaults to 1). You typically want **2 connections**, one for **Live** and one for **Staging**, named so it's obvious which is which.
...
<Tabs>
  <Tab title="Zoom Phone">
    ...
    3. Provide the numbers to PolyAI and specify which is **Live** and which is **Staging**.
  ...
  <Tab title="Zoom Contact Center">
    ...
    4. Set the **Phone Number** on the widget, and tell PolyAI whether it's the **Live** or **Staging** environment.
...
| Issue | Solution |
|-------|----------|
| Calls not reaching PolyAI | Verify the connection's number and environment (Live/Staging) mapping in the Zoom App |
