...
You can:
- Test against your own test systems
- Publish safely without changing code
...
Authentication is configured per environment (Sandbox, Staging, Live) on each API. This lets you point each environment at the appropriate credentials without changing code.

<Warning>
Authentication configuration is **not Branch-specific**. Auth settings on an API belong to the environment, not to the Branch you are working in. Editing them in one Branch or Sub-branch changes them for every other Branch in that environment. Take care when modifying auth configuration.
</Warning>
