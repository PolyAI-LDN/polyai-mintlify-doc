...
The Open platform has a self-serve integration set powered by Paragon, plus custom MCP servers for everything else. The categories below also surface in the docs, but most cards under Telephony, CRM, Hospitality, Healthcare, and Knowledge are enterprise-only. They require contracted SIP trunking, EHR provisioning, or per-tenant onboarding.
