...
The Open platform ships with a self-serve integration set powered by Paragon, plus custom MCP servers for everything else. The categories below also surface in the docs, but most cards under Telephony, CRM, Hospitality, Healthcare, and Knowledge are enterprise-only — they require contracted SIP trunking, EHR provisioning, or per-tenant onboarding.
...
For everything that's gated and why, see the [Open platform overview](/platform/open-platform#whats-enterprise-only).
