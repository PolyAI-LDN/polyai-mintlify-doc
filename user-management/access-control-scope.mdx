...
Real-time configuration and Branch management.

| Item | Controls |
|------|----------|
| **Real-time configuration** | The real-time configuration view |
| **Configuration builder** | The configuration builder |
| **Branch management** | Creating, renaming, syncing, merging, and deleting Branches and Sub-branches on the **Deployments** page, and restoring them from the **Archive** tab |
...
Environments, Branches, and widgets.

| Item | Controls |
|------|----------|
| **Deployment** | Environments, publishing, rolling back, A/B tests, Compare changes, and Live history |
...
Builds and tests the agent in a Branch. Reads the Deployments page but cannot publish.
...
Publishes, manages real-time configuration, and owns analytics. Does not change agent logic.
