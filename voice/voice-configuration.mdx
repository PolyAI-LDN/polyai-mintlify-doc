...
Safety filters are configured on a **per-channel basis**. The voice channel has its own safety filter settings, separate from the chat channel and the project-wide defaults. See [Safety filters](/agent-settings/safety-filters) for the full reference on categories, severity levels, and how filters interact with [Guardrails](/agent-settings/guardrails).
