...
def start_function(conv: Conversation):
    ...
    conv.goto_flow("Find out information")
    return str()
```
...
- **`conv.goto_flow("Find out information")`**: Routes the conversation to the specified flow.
...
- **Four voices at 0.25 each**: Even distribution across four voices.
- **Ten voices at 0.1 each**: Ensures variety while maintaining balance.

## Personalizing repeat caller experiences

Repeat callers are more likely to ask for a human agent if they hear the same AI voice every time. Switching voices on subsequent calls can make interactions feel less robotic, and it is proven reduce immediate escalations.
