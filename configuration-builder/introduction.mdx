...
The **Configuration Builder** lets you create settings forms so that managers can safely update options — like opening hours or on/off switches — across environments, without needing to republish the agent.

## How it works

Configuration Builder separates the **form layout** (what fields exist) from the **actual values** (what's filled in).
...
The form layout ensures all fields are filled in correctly. The data is what the agent actually uses during live conversations.
