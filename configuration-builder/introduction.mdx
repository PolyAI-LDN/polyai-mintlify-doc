...
The **Configuration Builder** lets you define JSON schemas that generate structured config forms. These forms allow managers to safely update settings—like opening hours, toggle flags, or feature switches—across environments, without requiring a deployment.
...
The builder sits **outside the agent's draft/publish system**.

This means:
- An empty config file that receives a schema will instantly expose real-time fields to fill.
- **Data values** can be edited per environment and take effect immediately.
- **Schema changes** must be created separately in each environment (see the warning below).
</Warning>

### Two audiences: builders and managers

<Info>
This page covers both schema definition (code) and value entry (UI). If you only need to fill in runtime values, skip to [step 2](#2-optional-add-environment-specific-values). For the full technical setup including Python code, continue reading from the top.
</Info>
