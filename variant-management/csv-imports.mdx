...
<Steps>
  <Step title="Prepare your CSV">
    ...
    - Include `is_default` values as required by the format (see [note above](#default-variant-behavior))
  </Step>
...
- **One default only** - Include exactly one variant with `is_default=true` in your CSV (see [note above](#default-variant-behavior))
