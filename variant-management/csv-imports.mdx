...
1. **Prepare your CSV**
   ...
   - Include `is_default` values as required by the format (see [known limitations](#default-variant-behavior))
...
- **One default only** - Include exactly one variant with `is_default=true` in your CSV, though note the [known limitations](#default-variant-behavior)
