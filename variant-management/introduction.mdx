...
### Per-variant integration credentials
Webchat handoff integrations (**Zendesk Sunshine** and **NICE CXone**) support a separate set of credentials per variant. Configure named credential sets under **Configure > Integrations** and assign one to each variant — useful when different variants hand off to different live-agent tenants (for example, one Sunshine account per brand). Variants with no explicit assignment fall back to the `default` credential set.
