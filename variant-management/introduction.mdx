...
**Variant Management** (found under **Build > Variant management** in the sidebar) allows you to manage content tailored for different sites—like a coffee shop with three stores or a hotel chain with branches in New York, London, and Tokyo.

![variant-1](/images/variant/introduction.png)

This setup allows a single agent to handle location-specific details like phone numbers, addresses, and operating hours. Variants streamline multi-site configuration and provide precise responses based on the customer's location.
