...
### Advanced functions
Use the [`conv.variant`](/function/conv-object#set-variant) object to retrieve variant attributes during conversations or to make decisions based on variant data. Use functions to set, retrieve, or act on variant-specific attributes.
...
<Warning> The first variant created will be used as the default for the assistant. If this variant is deleted, the next variant in the list will automatically become the new default. At present, there is no UI-based option to manually set a specific variant as the default.  </Warning>
