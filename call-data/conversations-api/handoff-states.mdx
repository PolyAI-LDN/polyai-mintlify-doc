...
The API does not trigger handoffs or change conversation state — it only returns what the agent stored. To trigger or configure handoffs, see [Call Handoffs](/call-handoff/introduction) (UI) or `conv.call_handoff` (code).
