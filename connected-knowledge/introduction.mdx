...
## Knowledge sources: auto-sync vs. file upload

PolyAI's Connected Knowledge auto-syncs from live sources, keeping your agent's information current without manual re-uploads. This is an operational advantage for enterprise teams managing large, frequently updated content libraries.

| Capability | PolyAI Connected Knowledge | File-upload-only platforms |
|------------|---------------------------|--------------------------|
| **Zendesk auto-sync** | Yes -- help center content syncs automatically | No -- manual re-upload when articles change |
| **Gladly auto-sync** | Yes -- knowledge source sync | No |
| **URL scraping with re-sync** | Yes -- click Update to re-scrape | One-time import only |
| **File upload** | Yes -- PDF, CSV, JSON, Office, and more | Yes |
| **Runtime merge with curated knowledge** | Yes -- Connected Knowledge and Managed Topics are merged at retrieval time | Typically a single flat knowledge store |
| **Priority system** | Managed Topics always take priority over Connected Knowledge | No priority mechanism |
| **Source attribution in conversation review** | Yes -- see exactly which source was used per turn | Varies |

For teams using Zendesk or Gladly as their primary knowledge base, this means the agent's knowledge stays current automatically as help articles are updated -- no manual sync step, no risk of stale answers.
