...
2. Fill in the form:
   - **Title**: A descriptive name for the SMS template (e.g., `reservation_confirmation`). This is the name you reference when triggering the template in functions.
   - **SMS Body**: The content of the message.
...
You can insert [variant attributes](/variant-management/introduction) into SMS templates for personalization using `${variant_attribute}` syntax. For example:

```plaintext
Hi ${customer_name}, your reservation at ${property_name} is confirmed for ${date}.
```
...
SMS can be triggered as part of a [function](/function/introduction) using [the `conv` object](/function/classes/conv-object):
...
- Use the **SMS widget** on the Performance Dashboard to monitor delivery rates.
