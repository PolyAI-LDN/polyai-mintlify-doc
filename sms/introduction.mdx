...
## Starting outbound SMS from an external system

If you want to start an SMS conversation from your own backend (for example, an appointment reminder triggered by a CRM event), use the [SMS API](/api-reference/sms/introduction). You provide the recipient number and the agent's first message; PolyAI picks one of the project's provisioned Twilio numbers (or uses the one you pin via `agent_number`), sends the message, and routes any reply back to the agent.

<Note>
The outbound SMS endpoint is not yet generally available on the public API gateway. Contact your PolyAI representative before integrating.
</Note>
...
curl -X POST https://api.us-1.platform.polyai.app/public_api/sms/outbound \
  ...
  -d '{
    ...
    "message": "Hi Alex — your appointment tomorrow at 10am is confirmed. Reply RESCHEDULE to change it."
  }'
```
