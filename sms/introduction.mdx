...
If you want to start an SMS conversation from your own backend (for example, an appointment reminder triggered by a CRM event), use the [SMS API](/api-reference/sms/introduction). You provide the recipient number and the agent's first message; PolyAI picks one of the project's provisioned Twilio numbers, sends the message, and routes any reply back to the agent.
