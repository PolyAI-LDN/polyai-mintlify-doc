...
For outbound SMS sessions where the user never replied, PolyAI closes the session silently without sending the notification, to avoid messaging users who didn't engage with the conversation. The notification still fires for outbound sessions once the user has sent at least one reply.
