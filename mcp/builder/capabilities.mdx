...
| --- | --- | --- |
| `agentId` | string | The agent. **Required.** |
| `environment` | string | Environment to list deployments for (sandbox, pre-release, live). `pre-release` is the Staging environment. One of `sandbox`, `pre-release`, `live`. **Required.** |
...
| `agentId` | string | The agent. **Required.** |
| `phoneNumber` | string | The phone number (E.164). **Required.** |
| `clientEnv` | string | Client environment (sandbox, pre-release, live). `pre-release` is the Staging environment. Default `live`. |
...
| `clientEnv` | string | Client environment (sandbox, pre-release, live). `pre-release` is the Staging environment. **Required.** |
...
| `environment` | string | Target environment: sandbox, pre-release (Staging), or live. **Required.** |
...
To narrow what a client can reach, scope the [API key](/mcp/authentication) itself. Create a [least-privilege key](/mcp/builder/security#restrict-the-tool-surface) limited to the agents and permissions that client needs, for example a read-only key for a monitoring assistant. The key's scope governs which calls succeed, so a narrowly scoped key can't create, delete, or publish even though the tools are still discovered.
