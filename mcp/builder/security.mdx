...
<AccordionGroup>
  ...
  <Accordion title="Review destructive actions" icon="triangle-exclamation">
    Tools that delete, publish, or roll back should require explicit user intent. So does [`promote-a-deployment-to-the-next-environment`](/mcp/builder/capabilities#promote-a-deployment-to-the-next-environment) on projects that have not yet moved to the new deployment system. Enable **confirm before running tools** in your MCP client so these are never executed silently, and read the [deployment steps](/mcp/builder/walkthrough#6-roll-back-if-needed) before publishing to Live.
  </Accordion>
