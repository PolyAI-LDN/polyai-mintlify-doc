...
```plaintext
After the caller confirms that they would like to receive an SMS message with further details, use {{SMS_template}} to send the SMS out.
```

**Example with a fallback:**

```plaintext
Offer to send the caller a text message with booking details. If they agree, use {{booking_confirmation_sms}}. If they decline, continue the conversation normally.
```
