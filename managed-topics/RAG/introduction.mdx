...
<Tip>
Write clear, specific topic names and realistic sample questions for best results. These are key signals the retriever uses to find the right match. You can add up to **20** sample questions per topic — more questions help the retriever find the right match. For best retrieval and generation quality, use PolyAI's [Raven](/agent-settings/model-use) model — it is optimized for grounding responses in retrieved knowledge.
</Tip>
