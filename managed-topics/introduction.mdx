...
<Tip>
If you need to add custom logic to your topics — such as connecting to external services, looking up information, or changing behavior based on conditions — see the [Tools](/function/introduction) section under **Build** in the sidebar.
</Tip>
...
The agent **does not** see all MTs at once. Instead, it uses [a smart matching system](/managed-topics/RAG/introduction) to find the best match for the user’s message.
...
2. It returns the best-matching topics to the language model. Only a small number of each topic's sample questions (typically 3) are included for the model to review.
3. The [language model](https://en.wikipedia.org/wiki/Large_language_model) picks the best match and generates a reply (and may trigger an **[action](/managed-topics/how-to-setup-action/introduction)**, **[function](/function/introduction)**, or **[flow transition](/flows/transition-functions)**).
