...
Without managed topics, your agent has no controlled knowledge — it either guesses from a general model or retrieves unstructured text with no guarantee of accuracy. Managed topics solve this by giving you precise control over what the agent says, when it says it, and what it does next.

Each topic has three parts: **sample questions** (how callers phrase the request), **content** (the answer), and optional **actions** (handoffs, SMS, function calls, flows). Topics cover the questions callers ask most — return policies, store hours, appointment availability, account lookups.

Managed Topics are found under **Build > Knowledge > Managed Topics tab**.

<CardGroup cols={2}>
  <Card title="Precise answers" icon="bullseye">
    You write the answer. The agent delivers it as written, grounded by retrieval — not hallucinated.
  </Card>
  <Card title="Actionable" icon="bolt">
    Topics can trigger [functions](/tools/introduction), [flows](/flows/introduction), [SMS](/managed-topics/how-to-setup-action/send-sms), and [handoffs](/managed-topics/how-to-setup-action/handoff).
  </Card>
  <Card title="Retrieval-ranked" icon="magnifying-glass">
    [RAG](/managed-topics/RAG/introduction) matches the caller's message to the best topic using weighted scoring on name, sample questions, and content.
  </Card>
  <Card title="Always wins" icon="trophy">
    When both managed topics and [connected knowledge](/connected-knowledge/introduction) match, managed topics take priority.
  </Card>
</CardGroup>

<Note>
**Use Managed Topics when** you need precise control over what the agent says, or when the answer should trigger an action. **Use [Connected Knowledge](/connected-knowledge/introduction) instead** when you want to expose large volumes of external content (help articles, PDFs, FAQs) without curating individual topics.
</Note>
