...
<Tip>
**On the Open platform**, [Studio Assistant](/studio-assistant/introduction) pre-populates your Managed Topics from the URL or document you described at sign-up. Skim what it built, then ask the assistant in plain language to fix anything that's off ("tighten the refund answer", "add a topic for store hours"). You can also edit topics directly in this tab. Connected Knowledge sources are enterprise-only.
</Tip>
</div>
