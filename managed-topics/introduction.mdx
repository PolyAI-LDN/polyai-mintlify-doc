...
Managed Topics define what your agent knows and how it responds. Each topic has sample questions, an answer, and optional actions (handoffs, SMS, function calls). Topics cover the questions callers ask most – return policies, store hours, appointment availability, account lookups.
...
Managed Topics are not a simple FAQ list or file-upload knowledge base. They are a structured knowledge architecture that gives you precise control over what your agent says and does.

### Managed Topics vs. file-upload knowledge bases

Many voice AI platforms offer a "knowledge base" that accepts file uploads (PDFs, text files, URLs) and uses basic RAG to retrieve answers. Managed Topics go further:

| Capability | PolyAI Managed Topics | File-upload knowledge bases |
|------------|----------------------|---------------------------|
| **Action triggering** | Handoffs, SMS, function calls directly from topic matches | No -- retrieval only |
| **Flow integration** | Trigger multi-step conversation workflows from any topic | No |
| **Weighted retrieval** | Name, sample questions, and content weighted separately | Flat document chunking |
| **Per-environment activation** | Toggle topics on/off per environment for phased rollouts | Typically all-or-nothing |
| **Multi-user live editing** | Real-time collaboration on topic content | File re-upload required |
| **Priority control** | Managed Topics always override Connected Knowledge | No priority system |
| **CSV bulk operations** | Import/export with activation status | Varies |

### When to use what

Use this decision tree to choose the right knowledge surface:

1. **Does the answer need to trigger an action** (handoff, SMS, function call, flow)? → **Managed Topics**
2. **Do you need precise control** over exactly what the agent says? → **Managed Topics**
3. **Is the content large, frequently updated, and hosted externally** (help center, Zendesk, Gladly)? → **[Connected Knowledge](/connected-knowledge/introduction)**
4. **Do you need both structured responses and broad FAQ coverage?** → **Both** -- Managed Topics for critical paths, Connected Knowledge for long-tail content. Managed Topics always take priority when there is a conflict.
...
<Tip>
[Raven](/agent-settings/raven) is recommended. It grounds answers in retrieved topics, says "I don't know" instead of hallucinating, and converts topic content into natural responses without example utterances.
</Tip>
...
### Using a function call (code)

Alternatively, add a function call action to the topic that calls `conv.goto_flow()`:
...
- Give the agent **one task per turn** – do not mix text responses and function calls in the same round
...
- After answering a simple FAQ, ensure the agent asks if the user needs more help – either in the topic content or via a global [rule](/agent-settings/rules)
...
- Mixing text and function calls in the same turn.

### Common anti-patterns

**Mixing text and function calls in one turn:** For best results, split utterances and function calls into separate turns – give the agent one task per turn. When a topic requires both a spoken response and an action, structure it so the response happens in one turn and the action in the next.
...
**Missing follow-up prompt:** Without a follow-up question like "Is there anything else I can help with?", the agent may hallucinate a next step. Add this to each simple FAQ's content or configure it as a global [rule](/agent-settings/rules).
