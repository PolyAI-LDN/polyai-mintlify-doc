...
Use Managed Topics to give your agent answers to the questions callers ask most often — return policies, store hours, appointment availability, account lookups. Each topic contains sample questions, an answer, and optional actions. When topics are well-maintained, your agent resolves more calls without escalation; when they drift out of date, containment drops and callers hear wrong or missing information.
...
3. The [LLM](https://en.wikipedia.org/wiki/Large_language_model) selects the best match and generates a reply (and may trigger an **[action](/managed-topics/how-to-setup-action/introduction)**, **[function](/tools/introduction)**, or **[flow transition](/flows/transition-functions)**).
...
**Example – Send a link** <img src="/images/kb-handoff-sms.png" alt="SMS_offer" />
...
**Example – Conditional SMS** <img src="/images/kb-sms-conditional.png" alt="SMS_conditional" />
...
**Mixing text and function calls in one turn:** Instructing the agent to both say something *and* call a function in the same turn is unreliable. The model may do one or the other, but not always both. Instead, split utterances and function calls into separate turns — give the agent one task per turn.
