...
Configure personality and role in **Build > Agent**. The same page also contains the [Behavior](/behavior/general/rules) section for hard rules and constraints.

<Note>
  **Where is the greeting?** Configured per channel — **Channels > Voice > Voice configuration** or **Channels > Chat > Chat configuration**. Legacy projects without channel-specific settings still configure it on this page.
</Note>
...
- **Voice greeting** – set under **Channels > Voice > [Voice configuration](/voice-channel/advanced/call-settings)**.
