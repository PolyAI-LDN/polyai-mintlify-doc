...
For example, if your Studio URL is `https://studio.uk.poly.ai/acme-uk/acme-team-4/agent`, then `account_id` is `acme-uk` and `project_id` is `acme-team-4`. Both the slug form (visible in the URL) and the prefixed form (`ACCOUNT-xxxxxxx` / `PROJECT-xxxxxxxx`) are accepted in API paths.
...
### Account and project scoping

API keys are scoped to a single account. When an endpoint includes a `project_id` in the URL path, the platform verifies that the project belongs to the account associated with your API key.

If the `project_id` belongs to a different account, the request is rejected with HTTP `403` and the error code `FORBIDDEN`:
...
If you receive this error, confirm that the `account_id` and `project_id` in the URL match the account that issued the API key. See [Finding your `account_id` and `project_id`](#finding-your-account-id-and-project-id).
