...
Regional endpoints, API overview, versioning, and authentication.
...
- Alerts API – configure alert rules for operational metrics and receive webhook notifications when thresholds are breached.
...
- Outbound Calling API – trigger outbound calls and check their status programmatically.
- Webhooks API – manage webhook endpoints for receiving event notifications.
- WebRTC Gateway API – establish browser-based voice connections via WebRTC.

Most APIs share the same regional base URLs; each has its own path and authentication model described on its page.
...
Most PolyAI API endpoints use API key authentication. Some APIs (such as Chat and Outbound) use different authentication headers — see each API's page for details.
