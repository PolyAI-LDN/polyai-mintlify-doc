...
v2 uses the *same schema* and the *same event-sourced backend* as v3, but retains the legacy authentication model.

Note on v2 Conversations API: v2 has the same schema as v3 but differs in authentication methods. Customers on v2 can continue using v2. New customers should use v3.
...
- All new integrations must use v3.
...
- Legacy (v1/v2) keys
  May already exist and follow the legacy authentication model.

- Conversations API v3 keys
  Project- and region-scoped, managed from the Data access page in Agent Studio.
  Keys are provisioned by PolyAI. Contact your representative to:
  • request a key
  • confirm its scope
  • align on any custom metrics or data-access patterns

Some specialised APIs (for example the External Events API) use dedicated, separately-scoped keys.
