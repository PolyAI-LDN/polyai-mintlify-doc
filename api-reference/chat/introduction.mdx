...
Use the Chat API to integrate PolyAI agents into existing <Tooltip tip="Non-voice chat systems, including in-browser chats and widgets, as well as SMS and email conversations.">Webchat</Tooltip>
...
* `operation`: The operation to perform, e.g., `create`, `respond`, `end`.

## Authentication

All requests to the Chat API must include the following headers:

* `X-Api-Key`: Your API key.

* `X-Token`: The connector token for the agent you want to chat with.
