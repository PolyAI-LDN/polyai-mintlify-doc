...
Use the Chat API to integrate PolyAI agents into existing <Tooltip tip="Non-voice chat systems, including in-browser chats, widgets, and SMS conversations.">Webchat</Tooltip> systems, and use its endpoints to get programmatic access to automated chat interactions. This API structures non-voice web conversations, messages, and chat sessions using a simple set of REST endpoints.
