...
| `authToken`            | Offer only | Authentication token for the voice agent                                     |
<<<<<<< Updated upstream
| `mode`                 | No         | `agent` (default) or `echo` (debug only)                                     |
=======
| `mode`                 | No         | `agent` (default) or `echo`. Echo mode is restricted to debug gateways; production rejects it with a `FORBIDDEN` error |
>>>>>>> Stashed changes
...
| `FORBIDDEN` | Request not permitted (for example, `echo` mode on a production gateway) |
