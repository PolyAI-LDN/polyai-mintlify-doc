...
<ResponseField name="mode" type="string">
  ...
  | `echo` | Debug only. Echoes audio back to the caller. Available in debug environments only; production gateways reject this mode with a `FORBIDDEN` error. |
...
| `FORBIDDEN` | The requested action is not permitted on this gateway (for example, requesting `echo` mode on a production gateway) |
