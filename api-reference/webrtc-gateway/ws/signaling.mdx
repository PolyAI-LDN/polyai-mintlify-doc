...
<<<<<<< Updated upstream
  Either `agent` (default) or `echo` (debug only – echoes audio back to the client).
</ResponseField>

=======
  Either `agent` (default) or `echo`. Echo mode echoes audio back to the client and is restricted to debug environments. Production gateways reject `echo` requests with a `FORBIDDEN` error.
</ResponseField>
...
| `FORBIDDEN` | The request is not permitted in this environment (for example, `echo` mode requested on a production gateway) |
