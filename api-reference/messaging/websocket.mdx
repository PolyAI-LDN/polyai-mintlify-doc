...
| `cursor` | No | Resume from a specific point in the conversation. Pass the sequence number as a non-negative integer (e.g. `cursor=12`). Default: `0` (full history). The `seq:<n>` prefix is also accepted for backwards compatibility. |
