...
<Note>
  An invalid `cursor` value (for example, a non-numeric string) does not reject the connection. The server logs a warning and defaults to `seq:0`, replaying the full conversation history.
</Note>
