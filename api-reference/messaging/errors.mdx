...
<Note>
A malformed `cursor` no longer rejects the connection. The server defaults to `0` (full replay) and logs a warning. Pass a non-negative integer (e.g. `cursor=12`); the `seq:` prefix is also accepted for backwards compatibility.
</Note>
