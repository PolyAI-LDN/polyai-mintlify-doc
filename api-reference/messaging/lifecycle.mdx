...
| **Connection drops** | Reconnect with `cursor=seq:<last_sequence>` using the same `session_id` and `access_token` |
...
sequenceDiagram
    ...
    Client->>Server: WebSocket connect (session_id, access_token, cursor=seq:12)
