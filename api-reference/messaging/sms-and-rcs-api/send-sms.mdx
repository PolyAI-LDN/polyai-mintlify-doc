...
Sends an outbound message to the specified user number and opens a new conversation session. The endpoint handles both **SMS** and **RCS** on the same route: for SMS, send to a plain E.164 number; for RCS, prefix the recipient number with `rcs:` and pass a Twilio Messaging Service SID for your RCS sender in `messaging_service_sid`. If the recipient's device or carrier doesn't support RCS, delivery automatically falls back to SMS. <br /><br />The first outbound message replaces the agent’s default greeting message, so it becomes part of the conversation context. This means the agent can see and respond based on that original message when the user replies. This works the same for both raw and templated outbound messages. 

For templates, the system first interpolates the template (whose ID is provided and used to fetch the template itself) and parameters into raw text; after that, it follows the same flow as any other outbound message. The agent only receives the resulting message text and does not distinguish whether it originated from a template. If the user replies, the agent handles the conversation as a normal SMS or RCS session.
