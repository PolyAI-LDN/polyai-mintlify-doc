...
Sends an outbound message to the specified user number and opens a new conversation session. The endpoint handles both **SMS** and **RCS**: if the recipient's device and carrier support RCS, the message is delivered as RCS with richer formatting and read receipts; otherwise it falls back to SMS automatically. The message behaves like a greeting, so it is fully part of the conversation context that the agent can access. If the user replies, the agent handles the conversation as a normal SMS or RCS session.
...
<ParamField body="agent_number" type="string">
  Sender phone number in E.164 format. Must be one of the numbers provisioned for your project. If omitted, the API picks one automatically from the numbers available for the connector token's environment.
</ParamField>
...
- **SMS vs RCS delivery** — the endpoint auto-selects RCS when the recipient's device and carrier support it, and falls back to SMS otherwise. No client changes are needed to enable RCS; ensure your project has RCS enabled and its brand/agent is registered.
