...
<Note>
**`no_data` vs `unknown`:** Use `no_data` to detect when your agents aren't receiving traffic (which may itself be a problem). `unknown` indicates that the state could not be determined — for example, the rule has not yet been evaluated.
</Note>

<Info>
New alert rules start in the `unknown` state and transition to `ok` or `alert` after their first evaluation.
</Info>
...
<Note>
Replace `api.us-1.platform.polyai.app` with the base URL for your deployment region. See [Getting started](/api-reference/introduction#regional-api-endpoints) for the full list of regional base URLs.
</Note>

### 2. Register a webhook endpoint

```bash
curl -X POST https://api.us-1.platform.polyai.app/v1/webhook-endpoints \
  -H "x-api-key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl https://api.us-1.platform.polyai.app/v1/alerts \
  -H "x-api-key: YOUR_API_KEY"
```
