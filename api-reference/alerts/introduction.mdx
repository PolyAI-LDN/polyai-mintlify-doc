...
```bash
curl -X POST https://api.{region}.platform.polyai.app/v1/alert-rules \
...
<Note>
Replace `{region}` with your deployment region: `us-1`, `uk-1`, or `euw-1`. See [Getting started](/api-reference/introduction#regional-api-endpoints) for the full list of regional base URLs.
</Note>

### 2. Register a webhook endpoint

```bash
curl -X POST https://api.{region}.platform.polyai.app/v1/webhook-endpoints \
  -H "x-api-key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl https://api.{region}.platform.polyai.app/v1/alerts \
  -H "x-api-key: YOUR_API_KEY"
```
