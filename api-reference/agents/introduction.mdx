...
All endpoints authenticate with a credential sent in the `x-api-key` header. Two credential types are supported:

- **API keys** are workspace-scoped and bound to fixed permissions — best for system-to-system integrations. See [API keys](/secrets/api-keys).
- **Personal access tokens (PATs)** authenticate as a user and inherit that user's permissions — best for local scripts and coding agents. Create one from your [Account settings](/secrets/personal-access-tokens).

```bash
curl https://api.us.poly.ai/v1/agents \
  -H "x-api-key: YOUR_API_KEY_OR_PAT"
```

<Note>
API keys for the Agents API are not yet available through self-service. Email [developers@poly.ai](mailto:developers@poly.ai) to request access. PATs are self-service from the **Personal access tokens** tab in Agent Studio.
</Note>
