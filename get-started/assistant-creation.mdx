---
title: 'Creating an Assistant'
description: 'Step-by-step guide on how to create an assistant on our platform'
---

Follow these steps to create an assistant on our platform:

## Step 1: Start the Creation Process
- After landing on the landing page, click the top right “Create Assistant” button to start your assistant creation process.

## Step 2: Set Name, Voice, and Greeting Line
- You will first see a page where you need to:
  - **Name your assistant**
  - **Choose a voice**
  - **Set the greeting line**

- Once you are happy with these settings, click "Continue".
  - You may listen to the voice or configure it further. See the [Voice Configuration](#) section for more details.
  - [How to set a good greeting line](#)
  - [How to choose a good voice](#)

## Step 3: Choose Personality, Role, and Additional Info
- Next, you will see the page for choosing the personality, role, and additional info for your assistant.

### What does personality mean?
- It defines the assistant's personality. See [here](#) for more details.

### What does role mean and how are they different?
- The role defines the function or purpose of the assistant. Your assistant will act according to this role.
  - See [here](#) for more details.

### What should we put for additional info?
- Provide additional info that you would like your assistant to be aware of regarding its operation.

## Step 4: Knowledge
- **What does that mean?**
  - Knowledge refers to the information and context the assistant will have access to.

- **What examples make sense?**
  - Provide relevant examples that the assistant can use to perform its tasks.

- **How does it work?**
  - This section may be expanded into a full section detailing the workings of the knowledge feature.

- Provide some examples but link to the [how-to section](#).

## Step 5: [Optional] Buy a Number or Telephony Integration
- A brief introduction and a link to the [telephony integration section](#).

By following these steps, you can create a personalized and effective assistant tailored to your needs.
