...
![general](/images/quickstart-general.png)
  ...
  <Step title="Choose a language model">
    Navigate to **Settings → Model** to select which LLM will power your agent. The default is **GPT-4o Mini**, but others like **Claude 3.5 Haiku** or **PolyLLM** are also available depending on use case.
...
You can now also configure **Actions**, RAG (retrieval-augmented generation), and inline messaging via the **Knowledge Base UI**.
...
You can now test your agent without deploying by:
...
<Info>
  ...
  You can now assign **multiple voices** to your agent using the **Multi-voice UI** under Voice settings. Voices can be distributed across calls by percentage.
</Info>

These testing tools are available on the Studio home screen.
  </Step>
...
Full page: Environments & Versions
...
<Info>
To tune your agent’s latency and style of reply, try adjusting the **Interaction Style** slider under [**Audio Management**](/audio-management/introduction). Choose between **Turbo**, **Swift**, **Balanced**, or **Precise**.
</Info>
