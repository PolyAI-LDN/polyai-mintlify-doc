...
<Note>
  You're in [Open platform mode](/platform/open-platform). This quickstart walks the self-serve 60-day free-trial path: Studio Assistant scaffolds the agent, you embed via PolyPhone. Click the **Open platform — exit** pill in the top-right corner for the full enterprise quickstart.
</Note>
  ...
  <Step title="Test in-browser (~2 min)">
    <Tabs>
      <Tab title="In-browser call">
        ...
        [PLACEHOLDER - Agent Debugging side panel open with Voice section showing Call agent option highlighted, Sandbox environment selected, ready to begin a test call]
      </Tab>
      <Tab title="Webchat preview">
        ...
        [PLACEHOLDER - Webchat side panel with Chat configuration showing Sandbox environment, agent variant selector, and a conversation in progress with matched topics displayed below the user's message]
      </Tab>
    </Tabs>
  ...
  <Step title="Embed or share (~1 min)">
    ...
    <Tabs>
      <Tab title="PolyPhone widget">
        ...
        See [PolyPhone](/polyphone/introduction) for styling, behavior, and channel-mix options.
      </Tab>
