...
<Steps>
  ...
  <Step title="Test your agent (~2 min)">
    Click **Test** in the top-right corner of the agent page to open the **Agent Debugging** side panel. From there, pick how you want to debug:

    ![test agent debugging side panel](/images/deployment/test-agent-debugging.png)
    ...
    <Tabs>
      <Tab title="In-browser call">
        ...
        1. Click **Test** in the top-right corner
        2. In the **Agent Debugging** panel, choose **Call agent** under **Voice**
        3. Select **Sandbox** from the environment dropdown
        4. Begin speaking to your agent
      ...
      <Tab title="Webchat">
        ...
        1. Click **Test** in the top-right corner
        2. In the **Agent Debugging** panel, choose **Webchat** under **Messaging** (or **Chat with agent** to debug a voice agent via chat)

        The chat window shows tool calls and topic citations alongside the conversation. Click the **gear icon** to toggle these.

        ![Webchat side panel with Chat configuration, environment selector, and agent variant dropdown](/images/deployment/test-webchat-panel.png)

        Matched topics and knowledge sources appear inline beneath each user turn so you can see exactly what fired:
