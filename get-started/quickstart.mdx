...
<Note>
  You're in [Agent Builder mode](/platform/open-platform). This quickstart walks the self-serve 60-day free-trial path: Studio Assistant scaffolds the agent, you embed via PolyPhone. Click the **Free trial — exit** pill in the top-right corner for the full enterprise quickstart.
</Note>
...
<Steps>
  ...
  <Step title="Test in-browser (~2 min)">
    ...
    <Note>
      **Phone testing** (calling a real PSTN number) requires enterprise telephony provisioning and isn't available on Agent Builder. Use the in-browser call or the [PolyPhone widget](/polyphone/introduction) instead.
    </Note>
  </Step>
...
For everything that's included on Agent Builder and everything that's enterprise-only, see the [Agent Builder overview](/platform/open-platform).
