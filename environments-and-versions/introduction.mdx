...
Most voice AI platforms use a simple publish-to-production flow. PolyAI's four-stage pipeline exists because voice agents handle real customer calls -- and a broken deployment means dropped calls, incorrect information, or failed handoffs affecting live callers.

Four stages reduce production risk:

- **Draft** -- iterate freely without affecting any environment. Changes are invisible until published.
- **Sandbox** -- test with real voice calls in an isolated environment. Catch issues before anyone outside your team is affected.
- **Pre-release** -- user acceptance testing (UAT) with a separate phone number. Stakeholders validate before production.
- **Live** -- production, with instant rollback to any previous version if issues arise.

Each environment can have its own phone number, and topic activation is environment-specific -- so you can test a new topic in Sandbox without it appearing in production. If something goes wrong in Live, roll back to a known-good version in seconds, not minutes.
