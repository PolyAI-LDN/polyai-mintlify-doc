...
Test agent changes before they reach live callers. Draft → Sandbox → Live.

<Note>
**New to environments?** Start with the [Environments tutorial](/learn/guides/get-started/environments) for a hands-on introduction.
</Note>

<Warning>
**Saving is not the same as going live.** Saved changes are drafts. Drafts must be **published** to Sandbox before they can be **promoted** to Live. Unpublished changes don't appear in any environment.
</Warning>
...
<Note>
**Pre-release is enterprise-only.** Enterprise accounts get an extra UAT staging environment (Pre-release) between Sandbox and Live for user acceptance testing before production. On the Open platform you promote directly from Sandbox to Live.
</Note>
...
Promotion moves a version from Sandbox to <Tooltip tip="The production environment. This is where the agent handles live traffic.">Live</Tooltip>.
...
2. Click the overflow menu (three vertical dots) next to the version in the **Sandbox** tab.
...
<Tabs>
  <Tab title="Confirm">
    [PLACEHOLDER - Promote to Live confirmation modal in Open platform mode: confirmation checkbox, version name, and Promote button; promoting from Sandbox to Live]
  </Tab>

  <Tab title="Complete">
    [PLACEHOLDER - Deployments page in Open platform mode after a Promote to Live succeeds: version now appears in the Live tab with a green status indicator]
  </Tab>
</Tabs>
...
2. Select a version and click **Compare** to view differences between environments. <span className="full-only">Compare across **Sandbox**, **Pre-release**, and **Live**.</span><span className="simplified-only">Compare across **Sandbox** and **Live**.</span>
