...
- **Widgets.** Chosen under **Integrations > Widgets**. Point a widget at Staging to test it against a Branch, or at Live for your website. Moving a widget to a different environment generates a new script tag, so re-embed it afterwards. See [Configure a widget](/widgets/configure).
