...
<AccordionGroup>
  ...
  - **[ASR Service](/speech-recognition/introduction)**: Converts the audio into text using [automatic speech recognition](https://en.wikipedia.org/wiki/Speech_recognition).
  - **ASR Processing**: Searches for transcription issues and applies any relevant corrections.
  - **Transcript Text → Corrected Transcript**: The corrected transcript is passed to [Retrieval](/managed-topics/RAG/introduction).
  ...
  - **TTS Service**: Converts text chunks into speech using [text-to-speech synthesis](https://en.wikipedia.org/wiki/Speech_synthesis). Configure voices in [voice settings](/voice/introduction).
