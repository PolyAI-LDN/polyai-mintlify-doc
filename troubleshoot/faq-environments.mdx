...
<AccordionGroup>
  <Accordion title="How do I test changes safely before going live?" icon="flask">
    Work in a [Branch](/environments-and-versions/introduction), your own isolated copy of the agent branched off Live. In [Simple mode](/environments-and-versions/simple-mode) there are no Branches: test your working copy the same way, then publish.
    ...
    4. **Move it to Staging** if you need to test through a real channel, such as a phone number, widget, SMS or email. This step is optional.
    5. **Publish.** Your Branch merges onto Live and becomes the newest version in Live's history.

    You can [compare changes](/environments-and-versions/diffs) before publishing, and [roll back](/learn/maintain/version-management#rolling-back) in one step if issues arise.
  </Accordion>
  <Accordion title="Do I have to go through Staging?" icon="arrow-right-arrow-left">
    No. A Branch publishes straight to Live once you have tested it. Move it into Staging only when you need a real channel, such as a phone number, widget, SMS or email. Only one Branch can sit in Staging at a time.
  </Accordion>
