...
<AccordionGroup>
  <Accordion title="How should I structure a topic?" icon="sitemap">
    ...
    - **Sample questions**: You can add up to **20** sample questions per topic. More sample questions help the retriever find the right topic, but only a small subset (typically the top 3) are passed into the LLM context at query time. Write your most representative questions first.
...
<AccordionGroup>
  ...
  <Accordion title="Can I use state or variables in actions?" icon="database">
    Yes. You can use [variables](/function/variables) and state from [functions](/function/introduction) to conditionally control what happens. For example, you can show different content or trigger different actions depending on whether the user is authenticated or which location they're calling from.
