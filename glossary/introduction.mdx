...
An isolated workspace branched from a Branch, so several people can split a piece of work without conflicts. A Sub-branch only ever merges back into its parent Branch, with visual conflict resolution. See [Deployments](/environments-and-versions/introduction).
...
The environment where every Branch and Sub-branch lives while you work on it. Draft has no telephony or API connections, so nothing you build there can reach a real system. See [Deployments](/environments-and-versions/introduction).
...
Where a workspace runs: `draft` (no telephony or APIs), `staging` (optional, real phone number and staging API connections), or `live` (production). See [Deployments](/environments-and-versions/introduction).
...
An isolated workspace branched off Live, where you build and test changes. Publishing merges it back onto Live. See [Deployments](/environments-and-versions/introduction).
...
The old name for the [Draft](#draft) environment. Sandbox no longer has its own phone number or API connections; it holds every Branch and Sub-branch while you work on them. See [Deployments](/environments-and-versions/introduction).
...
An optional environment where you can test any Branch with a real phone number and staging API connections, one Branch at a time. Staging is the new name for Pre-release. See [Deployments](/environments-and-versions/introduction).
