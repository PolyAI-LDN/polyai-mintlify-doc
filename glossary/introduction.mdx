...
The context your agent runs in: `sandbox` (where Branches are built and tested, with shared API and app connections), `staging` (optional, one Branch on your channels), or `live` (production). See [Deployments](/environments-and-versions/introduction).
...
An optional environment where you can test with a real phone number and API connections, one Branch at a time. Staging is the new name for Pre-release. See [Deployments](/environments-and-versions/introduction).
