...
An isolated workspace branched from a Release, so several people can split a piece of work without conflicts. A Branch only ever merges back into its parent Release, with visual conflict resolution. See [Deployments](/environments-and-versions/introduction).
...
The environment where every Release and Branch lives while you work on it. Draft has no telephony or API connections, so nothing you build there can reach a real system. See [Deployments](/environments-and-versions/introduction).
...
- **Build-time approvals** – every change to a deployed agent is built and tested in a [Release](/environments-and-versions/introduction) before a human publishes it onto Live, so each version is reviewed before it reaches production traffic.
...
The production environment where the agent handles real customer traffic. You never edit Live directly; publishing merges a Release onto it, and every publish is a version you can roll back to. See [Deployments](/environments-and-versions/introduction).
...
The old name for the [Draft](#draft) environment. Sandbox no longer has its own phone number or API connections; it holds every Release and Branch while you work on them. See [Deployments](/environments-and-versions/introduction).
...
An optional environment where you can test any Release with a real phone number and staging API connections, one Release at a time. Staging is the new name for Pre-release. See [Deployments](/environments-and-versions/introduction).
