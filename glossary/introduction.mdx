...
An AI-driven voice agent built on PolyAI that interacts with customers in real time across voice (telephony), chat (webchat), and SMS. Each agent is a self-contained project with its own knowledge, flows, tools, voice configuration, and deployment environments. You set global behavior (greeting, personality, and role) in the Agent settings page, and the agent uses these alongside its knowledge to generate contextual, grounded responses. See [Agent settings](/behavior/introduction), [Quickstart](/get-started/quickstart).
...
A conversation that ends before the user's request is resolved and without a handoff to a human, for example when the caller hangs up mid-conversation.
...
An isolated copy of the agent branched off Live, where you build and test changes. Publishing merges it back onto Live. See [Deployments](/environments-and-versions/introduction).
...
Splitting large bodies of text into smaller pieces for retrieval. Sources are scraped, chunked, and matched against user input via RAG. See [Sources](/knowledge/sources/introduction), [RAG](/knowledge/faqs/RAG/introduction).
...
The context your agent runs in: `sandbox` (where Branches are built and tested, with shared API and app connections), `pre-release` (Staging, which is optional and holds one Branch on your channels), or `live` (production). See [Deployments](/environments-and-versions/introduction).
...
- **Build-time approvals** – every change to a deployed agent is built and tested in a [Branch](/environments-and-versions/introduction) before a human publishes it onto Live, so each version is reviewed before it reaches production traffic.
...
The AI model powering the agent's understanding and response generation. PolyAI's proprietary [Raven](/behavior/models/raven) model family is recommended for most deployments. It is designed for conversational AI with strong grounding and natural speech. Third-party models (GPT, Claude) are also supported. See [Model](/behavior/models/model-use).

## Live

The production environment where the agent handles real customer traffic. You never edit Live directly; publishing merges a Branch onto it, and every publish is a version you can roll back to. See [Deployments](/environments-and-versions/introduction).
...
The old name for [Staging](#staging). See [Deployments](/environments-and-versions/introduction).
...
The environment where every Branch and Sub-branch lives while you work on it. Branches in Sandbox share one set of API and app connections, and Sandbox has no channel connections, so testing there happens over an in-app call or chat. Previously, Sandbox was a standalone deployment with its own phone number. See [Deployments](/environments-and-versions/introduction).

## Staging

An optional environment where you test one Branch at a time over a real phone number or widget, using the connections configured for Staging. Available in Branches and Sub-branches modes. Staging is the new name for Pre-release, and its id in code stays `pre-release`. See [Environments](/environments-and-versions/environments#staging).

## Sub-branch

An isolated copy of the agent branched from a Branch, so several people can split a piece of work without conflicts. A Sub-branch only ever merges back into its parent Branch, with visual conflict resolution. See [Deployments](/environments-and-versions/introduction).
...
Testing in Staging to validate agent behavior before production. See [Deployments](/environments-and-versions/introduction).
