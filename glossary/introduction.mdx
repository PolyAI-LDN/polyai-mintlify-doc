...
A call or chat that ends before the user's request is resolved and without a handoff to a human — for example, the caller hangs up mid-conversation. PolyAI does not surface a dedicated "abandonment rate" metric out of the box; abandoned conversations can be identified in [Conversation Review](/analytics/conversations/review) (short duration, no handoff, no task completion) or via the [Conversations API](/api-reference/conversations/introduction) using `total_duration`, handoff fields, and [PolyScore](/analytics/polyscore). For a business-specific definition, track it as a [custom metric](/settings/metrics).

## AHT (Average Handling Time)

The mean time spent handling a conversation end-to-end. PolyAI does not publish a single "AHT" metric; the closest out-of-the-box equivalents on the [Standard dashboard](/analytics/dashboards/standard) are **Average Call Duration** (mean call length in seconds) and **Total Automated Minutes** (total time handled by the agent). For a precise AHT definition that matches your contact center's formula, compute it from the Conversations API `total_duration` and `polyai_duration` fields, or define a [custom metric](/settings/metrics).
...
- For caller-level views, use the [Conversations API](/api-reference/conversations/introduction) to group by caller identifier and apply your own recontact logic, or define a [custom metric](/settings/metrics).
...
The global level of [ASR biasing](#asr-biasing), configured on the Speech Recognition page. Biases the ASR model toward recognizing specific words and phrases on every turn of the conversation for better domain-specific transcription accuracy. See [Keyphrase boosting](/speech-recognition/introduction).
