...
The context your agent runs in: `sandbox` (where Branches are built and tested, with shared API and app connections), `staging` (optional, one Branch on a real channel), or `live` (production). See [Deployments](/environments-and-versions/introduction).
...
The environment where every Branch and Sub-branch lives while you work on it. Branches in Sandbox share one set of API and app connections, and Sandbox has no channel of its own, so testing there happens over an in-app call or chat. Previously, Sandbox was a standalone deployment with its own phone number. See [Deployments](/environments-and-versions/introduction).
