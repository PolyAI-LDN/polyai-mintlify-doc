...
The context your agent runs in: `sandbox` (no telephony or APIs), `staging` (optional, real phone number and API connections), or `live` (production). See [Deployments](/environments-and-versions/introduction).
...
The environment where every Branch and Sub-branch lives while you work on it. Sandbox has no telephony or API connections, so nothing you build there can reach a real system. Previously, Sandbox was a standalone deployment with its own phone number. See [Deployments](/environments-and-versions/introduction).

## Staging

An optional environment where you can test with a real phone number and API connections, one Branch at a time. In Simple mode, your working copy is automatically in Staging. Staging is the new name for Pre-release. See [Deployments](/environments-and-versions/introduction).
