...
<Accordion title="Infrastructure details (advanced)">
- **Load balancing**: Incoming calls are distributed across multiple media servers
- **Redundant media processing**: Multiple media servers ensure continuous service availability
...
Voice conversations require capabilities beyond text-based AI. PolyAI handles these natively:

- **Turn detection (endpointing)** — Detects when a caller has finished speaking and is waiting for a response. A pause mid-sentence ("I'd like to book... um... for Saturday") should not trigger a response, but a pause after a complete thought should. PolyAI uses a fusion of audio signals and text analysis to make this decision in real time.
- **Interruption handling (barge-in)** — When a caller speaks over the agent, the system distinguishes genuine interruptions ("Actually, never mind — transfer me") from backchannel affirmations ("Yeah", "Uh-huh"). True interruptions stop the agent and process new input. Backchannels are acknowledged without interrupting the response.
- **Background noise filtering** — Real-world calls include noise (TVs, cars, other conversations). Audio processing filters background noise so the speech recognition model receives clean input.

### 3. Agent service — Raven LLM

The agent service is the core of the system, powered by [Raven](/agent-settings/raven), PolyAI's proprietary LLM for voice conversations. Raven is not a wrapper around a third-party LLM — it is a purpose-built model trained for real-time spoken dialogue.
...
| **Voice-optimized responses** | Trained for spoken dialogue — natural length, pacing, phrasing | Trained for text — often too verbose for voice |
...
The agent service receives transcribed user input and coordinates:
...
- **Knowledge retrieval**: Uses RAG (Retrieval-Augmented Generation) to pull relevant information from both tabs of the [Knowledge](/managed-topics/introduction) area – [Managed Topics](/managed-topics/introduction) and [Connected](/connected-knowledge/introduction)
- **Action execution**: Triggers any necessary [function calls](/tools/introduction) or API integrations
...
- **[Functions](/tools/introduction)**: Custom logic and external integrations
