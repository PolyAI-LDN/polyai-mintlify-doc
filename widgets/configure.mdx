...
<ParamField path="Variant" type="dropdown" required>
  The [variant](/knowledge/variants/introduction) this widget connects to. Use variants for location-specific deployments. A/B tests run from a Branch, not a variant. See [A/B testing](/testing/ab-testing).
</ParamField>

<ParamField path="Environment" type="dropdown" required>
  Staging or Live. Sandbox can hold many Branches at once and has no channel connections, so a widget cannot target it. Test a Branch in Sandbox over an in-app call or chat instead. Moving a widget to a different environment generates a new script tag, so re-embed it afterwards.
</ParamField>
...
- **Save and publish**: publish your changes to the selected environment.
...
2. Click **Save and publish** to publish your changes to the selected environment.
...
- **Per environment**: isolate a Staging widget for QA from your Live widget.
