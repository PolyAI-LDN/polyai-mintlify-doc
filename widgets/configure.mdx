...
<ParamField path="Primary / theme colour" type="string">
  Accent colour for the launcher and primary actions. Pick from the brand swatches or enter a hex code. Example: `#c3da28`. Stored as `theme_color` for Phone widgets and as `color` for Chat widgets — same control, same effect.
</ParamField>

<Tip>
Check contrast against the launcher's white iconography to keep the call/chat button legible. The launcher is a 60 px circle on desktop, 44 px on mobile.
</Tip>
...
<Note>
Phone widgets don't render a header logo or agent avatar — the call card is intentionally minimal so the focus stays on the live audio. Use the agent name and primary colour to brand it.
</Note>
...
<Note>
For Phone widgets, transient call-state strings ("Connecting…", "In call", "Mute", "Unmute", "Call ended", "Start new call", and the generic error message) are not editable in v1. Submit a feature request if you need to localise them.
</Note>
...
### Chat-only: launcher and behaviour
...
| Auto-open | `data-auto-open="true"` on the script tag | Opens the chat as soon as the widget initialises — useful for dedicated chat pages. |
...
| Prompt bubble | Default text + background colour in the editor; runtime calls via `WebchatAPI.showBubble({...})` | Shows a speech bubble next to the launcher to draw attention or run a campaign. |
...
Phone widgets don't have these knobs — the launcher renders with a fixed size, the call card is fullscreen on mobile only, and behaviour is driven by call state rather than host-page config. See [Install on your site](/widgets/install) for the full script-tag attribute reference.
