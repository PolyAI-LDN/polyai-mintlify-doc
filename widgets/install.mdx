...
Embedding PolyPhone is one script tag, generated for you in Agent Studio.

## Step 1: Get your script tag

1. Open your PolyPhone widget under **Configure > Polyphone** in Agent Studio.
...
You only embed the script once. Updates to styling, content, or the agent itself take effect on the next page load — no re-embed.

<Note>
The script must be served over **HTTPS**. The host page also needs to be in a [secure context](https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts) because browsers block microphone access on insecure origins. The call button stays disabled on `http://` pages.
</Note>
...
<Note>
Always copy the actual script tag from **Configure > Polyphone > Embed** in Agent Studio. The one in this guide is illustrative.
</Note>
