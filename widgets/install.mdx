...
<Frame caption="Embed tab with the widget script tag">
  <img src="/images/widgets/embed-tab.png" alt="Embed tab with the widget script tag ready to copy" style={{ maxWidth: '720px', width: '100%', margin: '0 auto', display: 'block' }} />
</Frame>

The tag is bound to the widget's environment. Moving the widget between Staging and Live generates a new tag, so you re-embed it. Changing the variant, branding or content leaves the tag unchanged. See [Re-embedding](#re-embedding).
...
You only need to re-embed when **the script tag itself changes**. Moving the widget to a different environment (Staging or Live) generates a new tag. The Embed tab in Agent Studio shows a **Snippet changed** banner if the published script differs from the version your team has on the site. Hand the new tag to your dev team or update your tag manager configuration.
