...
Chat widgets accept optional `data-*` attributes on the `<script>` tag — read once at load time, no JavaScript required. Phone widgets don't use these; their behaviour is configured entirely in the editor and driven by call state.
...
| `data-auto-open` | `true`, `false` | `false` | Opens the chat as soon as the widget initialises. |
