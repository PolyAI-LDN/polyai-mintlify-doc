...
The install is a single script tag, generated for you under **Widgets > Embed**. Paste it just before `</body>` on your site:
...
| Where you set it up | Widgets | Voice > Numbers | Widgets | Voice |
...
Web Calling and Numbers are two ways into the same voice agent. They share the agent's spoken [voice](/voice-channel/introduction) (set once, under **Voice**) and the same conversation logic. Switching a visitor between them doesn't require a separate flow.
...
<span className="full-only">Phone and Chat widgets share a single Studio surface, which is why both live under **Widgets**. Pick **Phone** or **Chat** when you create one.</span>
...
- [Studio Assistant](/studio-assistant/introduction), ask it to adjust Web Calling behavior in plain language
...
- [Variants](/knowledge/variants/introduction), per-domain or per-region widget configuration
