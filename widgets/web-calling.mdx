...
Web Calling uses whichever voice you've set under **Configure > Agent voice**. The same voice plays whether someone reaches you through the embedded widget or a [shareable page link](/widgets/share).
...
| Docs | This section | [Numbers](/voice-channel/numbers/introduction) | [Webchat](/messaging-channel/introduction) | [Voice](/voice-channel/introduction) |

Web Calling and Numbers are two ways into the same voice agent. They share the agent's spoken [voice](/voice-channel/introduction) (set once, under **Channels > Voice**) and the same conversation logic. Switching a visitor between them doesn't require a separate flow.
...
- [Voice](/voice-channel/introduction), pick the voice your agent speaks with
- [Agent Builder](/studio-assistant/introduction), ask it to adjust Web Calling behavior in plain language
...
- [Voice](/voice-channel/introduction), the agent's TTS voice, shared by Web Calling and telephony
- [Numbers (telephony)](/voice-channel/numbers/introduction), the phone-network route into the same voice agent
- [Webchat](/messaging-channel/introduction), the text channel that uses the same widget framework
- [Variant management](/knowledge/variants/introduction), per-domain or per-region widget configuration
