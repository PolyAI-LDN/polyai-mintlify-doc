...
| Need | Notes |
|---|---|
| An Agent Studio project with a variant available in Staging or Live | Widgets connect to Staging and Live only. Point the widget at Staging to test, then at Live for real traffic. Sandbox has no channel connections, so test there over an in-app call instead. |
