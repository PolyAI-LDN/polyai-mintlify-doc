...
<span className="full-only">2. [Adjust the widget position](/widgets/install#widget-positioning-chat-widgets) using CSS on the `#poly-ai-chat` element (Chat widgets).
3. </span><span className="simplified-only">2. </span>Test on mobile, where banners often cover more screen space.
