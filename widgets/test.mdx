...
- Anyone with the link can place a call against the configured environment, so point the widget at **Staging** for share-outs unless you specifically want stakeholders on **Live**. Sandbox has no widget connection, so test a Branch in Sandbox over an in-app call instead.
- Calls placed via the preview count against your usage just like calls on Live.
