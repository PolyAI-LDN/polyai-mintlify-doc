...
<Note>
**Use FAQs when** you need precise control over what the agent says, or when the answer should trigger an action (SMS, handoff, tool call). **Use [Connected Knowledge](/knowledge/sources/introduction) instead** when you want to expose large volumes of external content (help articles, PDFs, FAQs) without curating individual topics.
</Note>

<Tip>
**Use [Studio Assistant](/studio-assistant/introduction) to maintain topics in natural language.** Ask it to *"add a topic for store hours"*, *"tighten the refund answer"*, or *"create topics from `https://example.com/faq`"* — it edits FAQs on a branch you review before merging. You can also edit topics directly in this tab.
</Tip>
...
- `actions`: If accepted, call `start_sms_flow`. See [SMS setup](/messaging-channel/sms) for details.
