...
Update handoff destinations, add new transfer points, and manage SIP routing – all without taking the agent offline. Test changes safely in Sandbox before publishing to Live.
...
| Test a handoff | In-app call in Sandbox (or call the Staging number) → Trigger transfer → Verify | 5 min |
...
4. **Save**.
5. Make a quick in-app test call in *Sandbox* and confirm the transfer works, then [publish the Branch](/environments-and-versions/introduction).
...
1. Make an in-app call in *Sandbox*, or move the Branch to *Staging* and call the Staging number.
2. Trigger the scenario that should transfer.
3. Confirm the call operates how you are expecting.
4. Publish the Branch to *Live*.
...
- [Branch management](/learn/maintain/version-management) - Test in a Branch, publish to Live, roll back
