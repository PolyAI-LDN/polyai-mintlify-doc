---
title: 'Maintaining Sources'
description: 'Sync and manage external knowledge sources – websites, files, and integrations – to keep content current.'
---
...
Sources can become outdated when:
...
Unlike the FAQs tab (which you edit directly), Sources must be **synced** to pull in the latest content.
...
- Before publishing to Live or moving a Branch to Staging
...
Sources can be enabled or disabled per environment (Sandbox, Staging, Live).
...
- **Gradual rollout** - Enable in Sandbox, then Staging, then Live as you gain confidence
...
- **Sync before major releases** - Ensure all sources are current before publishing to Live
...
2. **Publish** your Branch to Live to make the synced content available to customers
...
5. Publish to Live if changes were made
...
4. Optionally, move the Branch to Staging for user acceptance testing
5. Publish to Live when season begins
...
- [Environments](/environments-and-versions/introduction) - Understanding Sandbox, Staging and Live