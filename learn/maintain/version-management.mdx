...
<Note>Run your [test sets](/analytics/test-suite/introduction) before promoting to catch regressions early.</Note>
...
Rollback when a new version has critical bugs, performance degrades, [PolyScore](/analytics/polyscore) trends decline, or customer complaints spike. Rollback is just promoting an older version:
...
4. Promote to Live → [monitor closely](/learn/maintain/health-checks#post-deployment-monitor)
...
- **Monitor after promotion** — watch the first 30 minutes closely, check metrics and [PolyScore](/analytics/polyscore) for 24 hours. Set up the [Alerts API](/api-reference/alerts/introduction) for automated post-deployment monitoring.
...
- [Health checks](/learn/maintain/health-checks) — post-deployment monitoring routines
- [Performance monitoring](/learn/maintain/performance-monitoring) — tracking version performance
- [PolyScore](/analytics/polyscore) — quality-based rollback decisions
- [Alerts API](/api-reference/alerts/introduction) — automated post-deployment alerting
