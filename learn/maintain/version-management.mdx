...
2. **Pre-release** - User Acceptance Testing (UAT) environment
   ...
   - Test with select users or a small group
...
<Tip>You can also roll back by promoting an older version directly. The dedicated **Rollback** option in the options menu does the same thing — it promotes a previous version back to the selected environment. See [Rolling back](/environments-and-versions/introduction#rolling-back-to-a-previous-version) for the step-by-step UI flow.</Tip>

### Rollback best practices

- **Act quickly** - Don't let bad versions run longer than necessary
- **Communicate** - Notify everyone involved
...
2. **Find last good version** - Check **Deployments > Environments**
3. **Promote immediately** - Don't wait for scheduled time
4. **Notify others** - Alert everyone involved
5. **Monitor** - Verify the rollback fixed the issue
6. **Root cause analysis** - Analyze what went wrong
...
- **Use consistent format** - Follow a standard naming convention
...
2. Refresh the page
3. Check **Deployments > Environments** for active version
...
Coordinate with your collaborators on which Sandbox instances to use for each branch.
