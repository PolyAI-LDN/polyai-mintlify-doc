...
<Tip>
**Faster route for many of these:** open the failing conversation, then ask [Agent Builder](/studio-assistant/introduction) to fix it — *"the agent told the caller we offer same-day delivery; we don't, update the shipping topic"*. See [Iterate after a call](/learn/iterate-open-platform) for the full loop.
</Tip>
...
| **Audio is silent or distorted** | Voice config mismatch or bad network. | Check **Channels > Voice > Voice Configuration** for the correct TTS voice. Test your network or try another device. | [Voice config ↗](/voice-channel/advanced/call-settings) |
