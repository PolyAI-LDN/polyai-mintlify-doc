...
| Pre-deployment | Before each publish | 20-30 min | Test sets, manual testing, integration checks |
| Post-deployment | After each publish | 30 min | Live calls, key metrics, function logs |
...
6. Review [version history](/learn/maintain/version-management#rolling-back) – document major changes

## Pre-deployment check

Before publishing a Branch to Live (or moving it to Staging):

1. Run all test sets – investigate any failures before publishing
...
5. Compare to the current Live version using [Compare changes](/environments-and-versions/diffs)
6. Have a rollback plan ready (identify last known good version)

**Publish if:** All tests pass, no critical bugs, performance is acceptable.
**Do not publish if:** Test failures exist, performance degraded, or integrations are failing.

## Post-deployment monitor

After publishing to Live (or moving a Branch to Staging):
