...
| Update API credentials | Build > Tools > Secrets → Edit secret → Save | 3 min |
...
<Tip>You can reference state variables inside delay responses using the `$` symbol. For example: `Still checking availability at $branch_name...`</Tip>

<Note>Delay timing starts when the function begins executing, not when the user stops speaking. Account for LLM and ASR latency.</Note>
...
- [Functions overview](/function/introduction) - Learn about function capabilities
- [Using secrets in functions](/secrets/how-to-invoke-from-kb) - Secure credential management
- [Function logging (conv.log)](/function/classes/conv-log) - Structured logging for debugging
- [Delay controls](/function/delay-control) - Managing latency in functions
