...
| Limit barge-ins per call | **Channels > Voice > Audio Management** → Max barge-ins |
...
Use **Max barge-ins** on the same page to cap interruptions per call. Once the limit is reached, barge-in is disabled for the rest of the call — useful in noisy environments where false triggers stack up. Leave it empty for no limit. See [Audio management](/audio-management/introduction#limit-barge-ins-per-call) for details.
