---
title: "Maintaining Managed Topics"
description: "Add, edit, and bulk-manage Managed Topics content."
---

Edit, add, or bulk-update Managed Topics content. Test in Sandbox before promoting to Live.

## Quick reference

| I need to... | Action | Time estimate |
|--------------|--------|---------------|
| Edit a single answer | Click topic → Edit answer → Save | 2 min |
| Add a new topic | + Topic → Add questions & answer → Save | 5 min |
| Update 10-20 topics | Export CSV → Edit → Import CSV | 15 min |
| Update 50+ topics | Export CSV → Edit → Import CSV | 30 min |
| Fix wrong topic triggering | Add sample questions to correct topic | 3 min |
| Identify knowledge gaps from calls | [Smart Analyst](/smart-analyst/introduction) or [Annotations](/analytics/conversations/annotations) | 10-15 min |
| Check if a topic edit improved quality | [PolyScore](/analytics/polyscore) → filter by date range | 5 min |
| Merge duplicate topics | Combine content → Delete duplicate | 5 min |
| Test changes | Agent Chat in Sandbox | 5 min |
| Promote to Live | Publish → Promote to Pre-release → Promote to Live | 5 min |

<Tip>
   Always test in _Sandbox_ first and promote only when you're happy. Details are in [Environments & versions](/environments-and-versions/introduction). 
</Tip>

## Edit an existing topic

1. Click the topic you want to change.
2. Update text directly in the _Answer_ panel.
3. Click **Save**.
4. Use **Publish** and run [test sets](/analytics/test-suite/introduction) in sandbox before publishing.

Common fixes:

| Issue                            | What to do                                                      |
| -------------------------------- | --------------------------------------------------------------- |
| The agent gives a partial answer | Add clarifying details to the _Answer_ field.                   |
| It triggers the wrong topic      | Add a few of the user's actual phrasings as _Sample questions_. Check [Conversation Diagnosis](/analytics/conversations/diagnosis) to see which topic was matched. |
| It overlaps another topic        | Merge the content or re-title one topic to be more specific.    |

## Add a new topic

1. Click **\+ Topic**.
2. Enter a clear title.
3. Add at least three _sample questions_—think of real user wording.
4. Write the _Answer_ in plain language.
5. Save and preview.

## Bulk edits

Large updates are faster in a spreadsheet.

1. Click **Export CSV** to download a CSV file containing all of your Managed Topics.
2. Edit or add rows in your spreadsheet app.
   - Keep column headers exactly as exported.
   - Leave IDs intact for topics you're editing; new rows will create new topics.
3. Save as CSV.
4. Click **Import CSV** and upload the file.
5. Review the diff, then **Confirm import**.

Keep column headers exactly as exported. New rows (without an ID) create new topics; rows with an existing ID update that topic.

## Promote your changes

1. Open **Deployments > Environments**.
2. Select the new _Sandbox_ version.
3. Click **Promote to pre-release**.
4. Confirm.
5. Click **Promote to Live**
6. Confirm.

## Common workflows

### Weekly content update

1. Review [annotations](/analytics/conversations/annotations) for "Missing topic" flags and use [Smart Analyst](/smart-analyst/introduction) to surface knowledge gaps (5 min)
2. Add or update 3-5 topics (15 min)
3. Test in Agent Chat (5 min)
4. Publish to Sandbox (2 min)
5. Promote to Pre-release for UAT (3 min)
6. Promote to Live (3 min)

**Total time:** ~30 minutes

### Seasonal content refresh

1. Export current topics to CSV (2 min)
2. Update seasonal information (hours, policies, etc.) (20 min)
3. Import updated CSV (5 min)
4. Test seasonal queries in Agent Chat (10 min)
5. Publish and promote (5 min)

**Total time:** ~45 minutes

### Fixing a knowledge gap

1. Identify the gap from [Conversation Review](/analytics/conversations/review), [annotations](/analytics/conversations/annotations), or [Smart Analyst](/smart-analyst/introduction) (5 min)
2. Create new topic or update existing (5 min)
3. Add sample questions matching user phrasing (3 min)
4. Test with actual user queries (5 min)
5. Publish and promote (5 min)

**Total time:** ~25 minutes

<Tip>Save a failing conversation as a [test case](/analytics/test-suite/introduction) before making changes, then re-run it after to confirm the fix works.</Tip>

## Related pages

- [Smart Analyst](/smart-analyst/introduction) - Identify knowledge gaps at scale with deep sampling
- [PolyScore](/analytics/polyscore) - Measure quality impact of knowledge changes
- [Test suite](/analytics/test-suite/introduction) - Regression testing after edits
- [Annotations](/analytics/conversations/annotations) - Track "Missing topic" flags
- [Conversation Diagnosis](/analytics/conversations/diagnosis) - Debug topic matching
- [Connected knowledge](/learn/maintain/knowledge-connected-knowledge) - Sync external sources
- [Performance monitoring](/learn/maintain/performance-monitoring) - Identify knowledge gaps
- [Version management](/learn/maintain/version-management) - Publish and promote changes
- [Managed Topics overview](/managed-topics/introduction) - Complete feature documentation

## When to ask for help

Contact your PolyAI representative or account manager if you need to:

- Add Python functions or external API calls
- Localize your Knowledge for multi-language support
- Migrate content between separate projects
- Undo a large import that has gone wrong