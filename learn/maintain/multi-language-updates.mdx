...
By default, the agent detects the caller's language automatically through ASR and responds in that language. The agent can switch when the caller explicitly asks to change language, and also when the caller clearly speaks or writes in another configured language.

If you see false language switches – for example caused by short utterances or muffled audio that ASR mishears as a foreign language – ask your PolyAI representative to enable **strict** language switching for the project. In strict mode the agent only switches when the caller explicitly asks (e.g. "can we speak in French?"). See [Controlling automatic language switching](/agent-settings/multilingual#controlling-automatic-language-switching) for the full behaviour.
