...
|---|---|
| Check overall performance | [Dashboards](/analytics/dashboards/introduction) |
| Check call quality scores | [PolyScore](/analytics/polyscore) in Conversation Review |
| Review customer satisfaction | [CSAT dashboard](/analytics/csat/introduction) |
| Find slow responses | [Conversation Review](/analytics/conversations/review) → filter by latency |
| Debug ASR accuracy | [Conversation Review](/analytics/conversations/review) → [Diagnosis](/analytics/conversations/diagnosis) → check transcription |
| Find knowledge gaps | [Dashboards](/analytics/dashboards/introduction) → unhandled queries |
| Check function errors | [Conversation Review](/analytics/conversations/review) → filter by errors |
| Categorize calls in bulk | [Agent Analysis](/agent-analysis/introduction) |
| Track custom business metrics | [Custom metrics](/settings/metrics) |
...
Go to **Configure > [Dashboards](/analytics/dashboards/introduction)** for high-level metrics: call volume, average duration, handoff rate, top intents, and performance trends. Filter by date range, environment, variant, or version. Check the [Safety dashboard](/analytics/dashboards/safety) for compliance and risk monitoring.

### Conversation Review

Go to **Analytics > [Conversations](/analytics/conversations/review)** to drill into individual calls. Search, filter, listen to recordings, review transcriptions, and check the [Diagnosis](/analytics/conversations/diagnosis) panel for technical details (function logs, knowledge retrieval, LLM prompts, timing breakdown). Use [annotations](/analytics/conversations/annotations) to flag issues for follow-up.
...
**Fixes:** add custom vocabulary in [Speech Recognition](/speech-recognition/introduction), adjust ASR sensitivity, add clarification prompts for ambiguous input. Flag ASR errors in [annotations](/analytics/conversations/annotations) to track patterns.

### Knowledge gaps

1. Check **Configure > Dashboards** → unhandled queries
2. Review common questions without answers in Conversation Review
3. Use [Smart Analyst](/smart-analyst/introduction) deep sampling to identify gaps at scale — try: *"What questions are we not handling well?"* or *"Where does the assistant give incorrect or incomplete answers?"*
...
**Fixes:** add knowledge for common handoff reasons, adjust handoff rules, add self-service options before handoff. Review [CSAT](/analytics/csat/introduction) for handed-off calls — if satisfaction is high after handoff, the issue may be agent capability rather than handoff rules.
...
| **Quality** | Fix pronunciations, improve response clarity, monitor [PolyScore](/analytics/polyscore), test with real users |

## Related pages

- [Smart Analyst](/smart-analyst/introduction) — AI-powered conversation analysis with deep sampling
- [PolyScore](/analytics/polyscore) — automated AI quality scoring for every voice conversation
- [CSAT surveys](/analytics/csat/introduction) — measure customer satisfaction with post-call surveys
- [Agent Analysis](/agent-analysis/introduction) — categorize calls at scale with custom LLM prompts
- [Custom metrics](/settings/metrics) — define and track business-specific metrics
- [Safety dashboard](/analytics/dashboards/safety) — compliance and risk monitoring
