...
|--------------|--------|---------------|
| Check daily performance | Configure > [Dashboards](/analytics/dashboards/standard) → Standard → Review key metrics | 5 min |
| Check call quality scores | [PolyScore](/analytics/polyscore) in conversation table → Sort by score | 5 min |
| Review customer satisfaction | [CSAT dashboard](/analytics/csat/introduction) → Check trends | 5 min |
| Listen to recent calls | Analytics > [Conversations](/analytics/conversations/review) → Filter → Listen | 10-20 min |
...
| Categorize a batch of calls | [Agent Analysis](/agent-analysis/introduction) → Create group | 15 min |
...
| Tag problematic calls | Open call → Add [category/annotation](/analytics/conversations/annotations) | 2 min |
| Check containment rate | Configure > [Dashboards](/analytics/dashboards/standard) → Standard → Containment metric | 2 min |
| Define a new metric | [Custom metrics](/settings/metrics) → Add metric | 10 min |
| Review safety issues | [Safety dashboard](/analytics/dashboards/safety) → Check flags | 10 min |
...
| Anything bespoke (e.g. bookings, revenue) | [Custom dashboards](/analytics/dashboards/custom) – ask your PolyAI representative if you do not see one |
...
4. Check the [Diagnosis](/analytics/conversations/diagnosis) panel:
   - Did the agent serve the new answer? Check topic citations and sources.
   - Did the caller stay in self-service or ask for a human?
5. If something looks off, tag it with an [annotation](/analytics/conversations/annotations).
...
These tags surface in QA reports and help you and the PolyAI team spot patterns.
...
1. Open [Standard dashboard](/analytics/dashboards/standard)
2. Review key metrics vs. baseline:
   - Call volume
   - Containment rate
   - Average handle time
   - Handoff rate
   - [PolyScore](/analytics/polyscore) averages
3. Identify any anomalies
4. Drill into [Conversation Review](/analytics/conversations/review) if needed
...
- [Annotations](/analytics/conversations/annotations) - Tag and flag calls for follow-up
...
|-----------|----------------|
| Metrics flat after multiple tweaks | Your PolyAI representative – may need flow changes |
| Consistent ASR errors | PolyAI support – include call IDs |
| Dashboard missing a metric you need | Your PolyAI representative |
| Safety dashboard shows spikes | Contact PolyAI support immediately |
