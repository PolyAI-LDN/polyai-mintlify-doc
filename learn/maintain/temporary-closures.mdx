...
<Steps>
  ...
  <Step title="Activate the topic">
    Make sure the topic is active in your Branch. If you have an existing opening hours topic, consider deactivating it temporarily so the closure topic takes priority.
  </Step>
  ...
  <Step title="Revert when done">
    Deactivate the closure topic and reactivate your normal hours topic. Publish again.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Select the Live environment">
    Switch to the **Live** tab. Changes here take effect immediately. No publish is needed.
  </Step>
...
<Steps>
  ...
  <Step title="Test in Sandbox">
    Make an in-app call in Sandbox and trigger a transfer to check it routes correctly. To test from a real phone, move your Branch to **Staging** (Branches and Sub-branches modes only) and call the Staging number.
  </Step>
  <Step title="Publish">
    Publish to Live.
  </Step>
  <Step title="Revert when done">
    Change the route back to the original number. Publish again.
  </Step>
</Steps>
...
- [Environments](/environments-and-versions/introduction) - Publish changes
