...
description: "Read the transcript, find the failure, ask Agent Builder to fix it. The loop that gets the agent from working to actually good."
keywords:
  - iterate
  - improve agent
  - conversation review
...
<Note>
  This page covers the iterate-after-a-call loop with [Agent Builder](/studio-assistant/introduction): read the transcript, ask Agent Builder to fix it, share the link. Agent Builder is available to **project admins on enterprise clusters** as well as free-trial accounts — the workflow is the same in both.
</Note>

The first version of your agent won't be the final one. Real callers say things your prompts didn't anticipate, mispronounce reference numbers, and go off-script in ways no demo script covers. The fastest way to a good agent is to read the transcripts, find what broke, ask Agent Builder to fix it, and send the share link to a fresh pair of ears.
...
Agent Builder proposes a plan. Read it. Approve when it looks right, or send feedback if it doesn't.

### 4. Test the fix

Open the agent in the browser (**Test** in the top-right) and replay the scenario that broke. If it works, promote the new version from Sandbox to Live. If not, send Agent Builder another revision; the branch is still open.

### 5. Send the link to someone new

You know what you fixed, so you're biased. Send the [share link](/polyphone/share) to someone who isn't in the loop and ask them to try the thing you broke. They'll find the next thing.
...
<CardGroup cols={2}>
  ...
  <Card title="Prompting Agent Builder" icon="message-pen" href="/studio-assistant/prompting">
