...
description: "Read the transcript, find the failure, ask Wren to fix it. The loop that gets the agent from working to actually good."
keywords:
  - iterate
  - improve agent
  - conversation review
...
Open the agent in the browser (**Test** in the top-right) and replay the scenario that broke. If it works, [publish](/environments-and-versions/introduction#publishing) the Branch to Live with the **Publish** button in the top-right. If not, send Wren another revision; the Branch is still open.
...
<CardGroup cols={2}>
  ...
  <Card title="Prompting Wren" icon="message-pen" href="/wren/prompting">
