...
The worst version of this: the agent says "I'm sorry, let's start over" and drops all collected information. That's a design failure.
...
- This adds one turn to the call. That's worth it. Undoing a wrong booking costs far more.
...
<Quiz questions={[
  {
    ...
    correct: 2,
    explanation: "Guideline 4: Support correction. The user corrected a single value. The agent should update the time and keep everything else — not restart the entire flow.",
  }
]} />

### 6. Act efficiently

Every unnecessary turn costs time, patience, and containment rate. Cut aggressively:

- If the user already gave information, don't ask for it again.
- If only one option makes sense, don't present it as a choice — just do it.
...
The most common efficiency failure: the agent explains *why* before doing the thing. "In order to look up your booking, I'll need your reference number. Could you please provide that?" Just ask: "What's your booking reference?"
...
Err on the side of informality. Voice agents that sound like legal documents or corporate emails feel wrong to talk to.
...
The test: can *every* user who has a legitimate reason to call actually complete their task? If any common user profile is locked out, the design fails guideline 1 (do the thing) as well.

### 10. Adapt to the user

Use what you know. If the system has context about the user — their account, their recent activity, their location — use it to skip steps and personalise the conversation.
...
- Caller is on a mobile number you recognise → "Is this about the account ending in 4821?"
- Return caller within 24 hours → "Are you calling back about the same issue?"

This saves time and signals competence. But don't be creepy — if you surface information the user didn't expect you to have, briefly explain how you know ("I can see from your account that...").
