...
description: "Understand Draft, Published, and Environment stages to safely promote changes to production."
icon: "code-branch"
---
...
<Quiz questions={[
  {
    q: "How many versions can be active in a single environment at any time?",
    options: [
      "Up to three — one per promotion stage",
      "Unlimited — versions run in parallel",
      "One — only one version per environment is active",
      "Two — a primary and a fallback",
    ],
    correct: 2,
    explanation: "Only one version per environment is active at any time. Multiple environments can point to the same version, but each environment runs exactly one version.",
  }
]} />
