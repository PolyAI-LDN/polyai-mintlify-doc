...
<Tabs>
  <Tab title="Agent">
    ...
    <Warning>
      If your project uses a [start function](/tools/start-function) that returns an `utterance`, it overrides the greeting field. Changes to the greeting box won't affect the live agent in that case. Check with your development team before editing the greeting.
    </Warning>
...
It helps to understand what the LLM receives on each turn. You can inspect this directly in **Conversation Review → Diagnosis → LLM requests**.
...
This means your Personality, Role, and Rules fields are literally part of the prompt on every turn. Write them as you would write any prompt instruction — clearly, specifically, and with hard constraints where you need them.
...
<Check>
  - **Agent** section clearly defines the agent's role and scope
...
<Quiz questions={[
  {
    q: "If a start function returns an utterance, what does it override?",
    options: [
      "The agent's language setting",
      "The Greeting field",
      "The first Managed Topic",
      "The agent's display name",
    ],
    correct: 1,
    explanation: "The start function's utterance overrides the Greeting field. Changes to the greeting text box won't affect what callers hear if a start function is returning its own utterance.",
  }
]} />
