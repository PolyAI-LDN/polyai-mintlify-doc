...
<CardGroup cols={2}>
  ...
  <Card title="Behavioral rules" icon="shield-halved">
...
<Tabs>
  <Tab title="Agent">
    ...
    <Note>
      **Advanced:** Some projects use a [start function](/tools/start-function) that returns an `utterance`, which overrides the greeting field. If you edit the greeting and nothing changes, this may be why. You'll learn about tools and return values in [Level 2](/learn/guides/advanced/using-functions). For now, if the greeting isn't responding to edits, open **Build → Tools** to see if a `start_function` is defined.
    </Note>
  ...
  <Tab title="Behavior">
    The [Behavior](/agent-settings/rules) section defines hard constraints that must always be followed.
...
<Check>
  ...
  - **Behavior** includes hard constraints
