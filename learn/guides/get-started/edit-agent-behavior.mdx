...
<Tabs>
  <Tab title="Agent">
    ...
    <Note>
      **Advanced:** Some projects use a [start function](/tools/start-tool) that returns an `utterance`, which overrides the greeting field. If you edit the greeting and nothing changes, this may be why. You'll learn about functions and return values in [Level 2](/learn/guides/advanced/using-tools). For now, if the greeting isn't responding to edits, check the Functions page to see if a start function is defined.
    </Note>
