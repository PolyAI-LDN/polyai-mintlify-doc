...
<Tabs>
  <Tab title="Personality and Role">
    ...
    **Custom personality example (with "Other" selected):**
    ...
    <Note>
      The **Greeting** is configured per-channel under **Voice → Agent voice** (and equivalent Channels settings) rather than on the Agent page. The greeting goes directly to TTS without LLM processing. Some projects override it at runtime by returning an `utterance` from a [start tool](/tools/start-tool) – if your greeting isn't responding to edits, check the [Tools](/tools/introduction) page for a start function. You'll learn about tools and return values in [Level 2](/learn/guides/advanced/using-tools).
    </Note>
  </Tab>
