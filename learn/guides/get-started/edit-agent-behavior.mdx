...
<Tabs>
  <Tab title="Personality and Role">
    ...
    **Custom personality example (with "Other" selected):**
    ...
    <Note>
      The **Greeting** is configured per-channel – under **Channels > Voice > [Voice configuration](/voice-channel/advanced/call-settings)** for voice and **Channels > Chat > Chat configuration** for webchat – not on the Agent page. The greeting goes directly to TTS without LLM processing. Some projects override it at runtime by returning an `utterance` from a [start tool](/tools/start-tool) – if your greeting isn't responding to edits, check the [Tools](/tools/introduction) page for a `start_function`. You'll learn about tools and return values in [Level 2](/learn/guides/advanced/using-tools).
    </Note>
  </Tab>
