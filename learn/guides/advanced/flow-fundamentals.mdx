...
<Info>
  **Level 2 — Lesson 9 of 10** — Understand flows, steps, and transitions for structured multi-turn interactions.
</Info>

Flows give you fine-grained control over multi-turn conversations. They enforce a specific sequence of steps, prevent the model from improvising, and let you collect, validate, and act on information reliably.

Use flows when the conversation requires a specific order of operations — collecting a booking reference, verifying identity, or routing based on user input. Without flows, these interactions rely on the LLM to guess the sequence, which leads to inconsistent outcomes.
...
<Steps>
  <Step title="Enter the flow">
    A start function calls `conv.goto_flow("flow_name")`. This can be triggered from agent behaviour, a knowledge base topic, or another function.
  </Step>
...
<Quiz questions={[
  {
    ...
    options: [
      "It uses a special LLM mode that locks behaviour",
...
2. Create one from any behaviour menu (e.g., inside a managed topic action)
...
The start function is what triggers entry into the flow. You must expose it somewhere — in agent behaviour, a knowledge base topic, or another function. If a flow has no entry point, it exists but cannot be reached.
...
<Quiz questions={[
  {
    ...
    options: [
      "The model is too smart and optimises the flow",
...
- The model returns to normal behaviour (agent rules, knowledge base, global functions)
...
<ProgressTracker lessonKey="l2-7-flow-fundamentals" lessonNum={9} totalLessons={10} level="Level 2" />
