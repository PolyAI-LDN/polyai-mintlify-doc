...
<FillBlank
  prompt='To make a function visible to the LLM, you reference it in a topic, flow, or Behavior field using the syntax: @________'
  answer={["function_name", "@function_name"]}
  hint="What prefix makes a function reference?"
  explanation='Use @function_name in Behavior, a topic action, or a flow step. Agent Studio highlights it and registers the function in the LLM request.'
/>
...
This is visible in Conversation Diagnosis. When a function is called, you will see two requests instead of one. Enable the **Tool calls** toggle to see what parameters were passed and what was returned.
