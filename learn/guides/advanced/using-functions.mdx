...
You can build a basic agent using only the Agent tab — personality, role, and rules. But you will quickly hit a wall.

Without functions, the agent cannot:

- retrieve user-specific data (account balance, booking status)
- execute actions (make a booking, send a confirmation)
- save information from the conversation
- integrate with any external system

Prompt engineering alone is also not enough for large enterprise projects with dozens of user journeys. Putting all of that into a single rules box gives the LLM too much context at once and makes it harder to reason reliably about any single scenario.

Functions solve both problems. They let you integrate with external systems **and** give you fine-grained control over what the LLM knows and does at each step of the conversation.
...
When the LLM is deciding whether to call a function, it sees:

1. The base system prompt (personality + role + rules from the Agent tab)
2. Any function definitions that have been referenced — including name, description, and all parameter names, descriptions, and types
...
This means your function name and description are part of the prompt. Write them with the same care as any other prompt text.
