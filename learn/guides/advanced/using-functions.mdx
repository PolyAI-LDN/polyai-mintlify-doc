...
## Creating a tool in Agent Studio

In Agent Studio, tools are defined as Python `function`s – **"tool" and "function" refer to the same feature**, with "tool" being the UI name and "function" the Python identifier. This page uses them interchangeably.

To create a tool, go to **Build → Tools** and click the **+** button.
...
<Steps>
  <Step title="Create a function">
    In **Build → Tools**, create a tool (Python function) called `get_store_hours` with no parameters. Return a string like `"The store is open Monday–Friday, 9am to 6pm."`.
  </Step>
