...
Automatic Speech Recognition (ASR) determines what the agent *hears*. Everything the agent does next depends on accurate speech recognition: Knowledge retrieval, routing, response control, and handoff logic. Small transcription errors here can silently break otherwise well-designed agents.
