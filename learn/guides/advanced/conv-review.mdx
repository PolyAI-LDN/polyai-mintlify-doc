...
- Sample questions from one topic accidentally matching another
...
>
> This means the agent can't tell which topic to pick. The fix is to restructure or rename the topics, not just edit the text.
...
Entities are specific pieces of information (like dates, names, or numbers) that the agent extracts from what the caller says.

Inspect entities to:
- Confirm values were actually captured
- Detect silent failures (nulls)
- Spot information the agent incorrectly assumed or invented
