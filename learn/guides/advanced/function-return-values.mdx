---
title: "Tutorial: Function return values"
sidebarTitle: "Function return values"
description: "PolyAcademy Level 2 — What your functions can return and how each return type affects agent behavior."
...
<Info>
  **Level 2 — Lesson 3 of 8** — This lesson covers the different values a function can return and when to use each one. Understanding [return values](/tools/return-values) gives you precise control over what the agent says and when.
</Info>
...
<FillBlank
  prompt='You want the agent to say "Goodbye!" and then end the call. Complete the return value: return {"utterance": "Goodbye!", "______": True}'
  answer={["hangup"]}
  hint="Which key ends the call?"
  explanation='The hangup key ends the call after the utterance is played. Always pair it with an utterance so the user hears something before the call ends.'
/>
...
<Quiz questions={[
  {
    ...
    correct: 1,
    explanation: "When you return both keys, the utterance is played immediately (buying time while data loads), and the content is stored in history for the LLM to reference on the next turn. This lets you say something natural now while giving the LLM the structured data it needs to respond intelligently.",
  }
]} />

<CardGroup cols={2}>
  <Card title="← Previous: Using functions" icon="arrow-left" href="/learn/guides/advanced/using-functions">
    Lesson 2 of 8
  </Card>
