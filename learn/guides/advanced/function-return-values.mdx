...
Depending on the model configuration, the LLM may return either a function call **or** text — not both in the same response. This means you cannot rely on the LLM to generate a goodbye message at the same time as calling a hangup function.
